...
4. [Module Reference](#4-module-reference) — 39 modules, numbered sequentially (not nested) for simplicity:
   1. Accounts
   2. Transactions
   3. Transfers
   4. Exchange Rates
   ...
   36. Fines & Penalties
   37. Member Deposit Tracking
   38. Payment Confirmations
   39. Bonds & Investments Overhaul (settlement value, coupon adjustments, termination workflow)
...
**Financial vs. performative, at a glance (v1.36.0):** Admin, Director, Treasurer, Assistant Treasurer, and Shareholder see company financial information (account balances, share price, investment figures) by default — every other role above is "performative," adding functionality without money-figure visibility unless separately granted. Full mechanics below.
...
- **`blockFinanceRestricted`** — Auditor **and** Administrative Officer. Applied to every other finance-adjacent route file (Accounts, Transactions, Transfers, Grants, Loans, Investments, Requisitions, Savings, Side Fund, Fines, Deposits, Dividends, Reports, Search, Shares, Certificates, Exchange Rates, System, `/users/shareholding`).
...
**Financial vs. performative roles (v1.36.0) — `requireFinancialAccess(permission)` and `FINANCIAL_ROLES` (`middleware/auth.js`).** `blockFinanceRestricted` is a deny-list of exactly two roles (Auditor, Administrative Officer) — every OTHER role, including Secretary/Assistant Secretary/Coordinator, was implicitly treated as "a real member, fine to show company financial data to," on the historical assumption noted in `requireAssignedRole`'s own doc comment. A handful of read-only, dashboard-style endpoints were written under that assumption with **no permission gate at all**, just `authenticate` (each carried its own "used by dashboard, any authenticated user" comment): `GET /accounts/summary`, `GET /savings/settings`, `GET /side-fund/settings`, `GET /shares/price(/history)`, `GET /exchange-rates/current(/history)`, `GET /investments/performance-summary`, `GET /mmf/performance-summary`.

The system's actual policy is narrower: only **Treasurer, Assistant Treasurer, Shareholder, Director, and Admin** (`FINANCIAL_ROLES`) see company financial information by default. Every other role — Secretary, Assistant Secretary, Coordinator, Administrative Officer, and any future role added purely to *"add functionality without being a shareholder/officer with money authority"* — is "performative": full access to whatever it's actually for (Events, Documents, Acknowledgements, its own Service Fee records, etc.), but no account balances, share price, exchange rates, or investment/MMF figures, unless an Admin separately grants the specific permission via Settings → Roles & Permissions (the same "extra roles that allow seeing other views" escape hatch every other permission in this system already has).

`requireFinancialAccess(permission)` is the fix, applied to exactly the endpoints listed above: passes if the caller holds one of `FINANCIAL_ROLES` **or** the named permission — an ALLOW-list, unlike `blockFinanceRestricted`'s deny-list, so it doesn't need updating every time a new performative role is added. `AuthContext.js`'s matching `hasFinancialAccess(permission)` is the client-side mirror, used by `DashboardPage.jsx` (account balance cards, the Best/Worst Investment card), `TopBar.jsx` (the balance strip), and `Sidebar.jsx` (Savings/Side Fund nav items) to decide what to even render or fetch — cosmetic on its own, but paired with the real backend gate this time, unlike the sidebar's older `!isAdminOfficer`-only checks it partially replaces. See Section 23.6/7.1 for the specific leak this closed and Section 7.3 for the related Transfer Approve-button fix in the same release.

**Zero-role accounts (v1.21.1) — `requireAssignedRole` (`middleware/auth.js`).** Registering and verifying an email is not the same as being approved: a freshly verified account holds **no role at all** until an Admin assigns one (via Users → Assign Role, or by approving a role request made at registration). `blockFinanceRestricted` and `blockAuditor` are deny-lists for two *specific* known roles — they do nothing to stop a zero-role account, since it isn't Auditor or Administrative Officer either. `requireAssignedRole` is the allow-list half of that: it rejects any request from a user with an empty roles array, and is applied to every route file that has "open to any authenticated user" endpoints (Accounts, Transactions, Transfers, Grants, Loans, Investments, Events, Documents, Reports, System, Dividends, Savings, Side Fund, Fines, Deposits, Requisitions, Shares, Exchange Rates, Certificates, Search, Audit, Staff Access, Service Fees, Categories, plus `GET /users/shareholding`). Deliberately **not** applied to `GET/PATCH /users/me`, `GET /users/me/role-request`, `GET /users/roles`, `notifications.js`, or `settings.js`'s company-branding read — a pending account still needs to see/edit its own profile, check whether its role request has been reviewed, and render a branded page.
...
> **Note:** this section (8.1–8.6) predates Bonds, Projects/Milestones, and the v1.40.0 overhaul, and was never updated when those were built — it describes an earlier, simpler shape of this module and is kept here for historical continuity rather than as an accurate current reference. For the actual current `investments` schema (including `investment_type='BOND'`, `face_value`/`coupon_rate`/`coupon_frequency`, `settlement_value`, `supplementary_budget`, and the termination workflow columns), the real bond coupon math (`cms/src/utils/bondSchedule.js`), and everything added in v1.40.0, see **Section 39**. Projects/Milestones DO have a working frontend today (`InvestmentDetailPage.jsx`) contrary to what 8.6 below says — that gap has since been closed.
...
##### 9.1 Purpose
A general-purpose internal request/approval workflow — originally for expense requisitions (a member/officer requests funds for something), expanded to also carry acknowledgement-style requests: **Contribution Acknowledgement**, **Savings Deposit**, (v1.26.0) **Side Fund Contribution**, and (v1.37.0) **Fine Payment** — the mechanism by which a regular Shareholder now records money they've already handed over, since direct posting of any of these was restricted to Treasurer/Assistant Treasurer. One workflow engine, five purposes.

##### 9.2 Data model

**`requisitions`**: id, reference_id, requisition_type (EXPENSE / CONTRIBUTION_ACKNOWLEDGEMENT / SAVINGS_DEPOSIT / SIDE_FUND_CONTRIBUTION / FINE_PAYMENT — the last two widened onto the CHECK constraint in v1.26.0 and v1.37.0 respectively), requested_by, amount_requested, amount_approved, currency_id, category_id, description, purpose, contribution_date (the date the underlying payment/deposit was actually made — required for all four acknowledgement-style types), required_by_date, priority, status (PENDING/APPROVED/REJECTED), account_id (target account — for EXPENSE and, since v1.37.0, FINE_PAYMENT, the only two types with no single canonical account to auto-resolve), fine_id (v1.37.0, only set for FINE_PAYMENT — which specific outstanding fine this settles), transaction_id, reviewed_by, reviewed_at, review_notes, created_at.

For all four acknowledgement-style types, the requester submits amount + `contribution_date` + a free-text purpose/notes field describing how they paid; no month/period picker is offered even for the side fund type — the existing oldest-unpaid-first cascade in `applySideFundPayment` sorts out which period(s) a side fund acknowledgement covers. FINE_PAYMENT additionally requires `fine_id`, since — unlike the other three types — a member can hold several simultaneous outstanding fines, so the requisition must name exactly which one it settles (Section 36.3).
...
- **Approving a FINE_PAYMENT requisition (v1.37.0)** requires a Treasurer-chosen `account_id` (like EXPENSE, unlike the other two acknowledgement types) and routes into `finesService.clearFine` (Section 36.3) — the same shared crediting core the Treasurer's own direct "Clear Fine" action uses, including the hard currency-match check.
...
- **Historical reversals from before v1.41.0 are only partially recoverable — see `migration_v1.41.1.sql`.** v1.41.0's reversal fix only runs at the moment a NEW reversal happens; it does nothing for a contribution that was already reversed sometime in the past, before this fix existed. Reported directly: "the latest migration does not affect the reversals in the past which is a problem because this value is important to have accurately." `migration_v1.41.1.sql` does a one-time backfill for those: the envelope balance (`side_fund_config.current_balance`) is corrected exactly and safely for every such transaction (a single aggregate number, always fully recoverable from the ledger). Per-due detail generally is **not** — before v1.41.0, `side_fund_dues.transaction_id` was last-write-wins and `side_fund_member_credit` kept no per-transaction history at all, so there's often no reliable way to know which specific due (or how much of a banked credit) a given old reversed payment actually applied to. The migration deliberately does not guess: it corrects what it safely can, and prints a `RAISE NOTICE` naming any due that still happens to point at the reversed transaction, for a human to review and correct by hand if that member's standing looks wrong — silently "fixing" this automatically would risk being confidently wrong, which is worse than flagging it.
- None currently open beyond the above.
- **Fixed in v1.41.0 — reversing a side fund contribution's transaction left `side_fund_dues`/`side_fund_member_credit` untouched.** Same root cause as the v1.40.1 Deposits fix: `reverseTransaction` was a generic, ledger-only operation with no branch for `SIDE_FUND_CONTRIBUTION_IN`. The harder part here is that a single payment can cascade across several dues oldest-first and bank any leftover as credit, and `side_fund_dues.transaction_id` is last-write-wins (a due settled across separate payments only remembers the most recent one) — so there was no reliable way to answer "what did transaction X actually do?" Solved with a new append-only `side_fund_payment_applications` table, written by `sideFundService.applySideFundPayment` itself (one row per due touched, plus one row for any credit banked), which `reverseTransaction` now reads to undo exactly what that transaction did — decrementing `side_fund_config.current_balance`, rolling back each due's `amount_paid`/`status`, and reversing any banked credit (with a `side_fund_credit_ledger` entry recording why) — regardless of how many dues/members one transaction touched (this also correctly covers Bulk Pay Dues, one transaction spanning many members). Guarded the same way as the Deposits fix: if the envelope balance or a member's banked credit has already been spent/drawn down since, the reversal is rejected with a clear error rather than going negative. A transaction from before this version (no `side_fund_payment_applications` rows) still has its envelope balance corrected, but returns a warning that the per-due/credit detail couldn't be rolled back automatically.
...
- **Historical reversals from before v1.41.0 — mostly recoverable, one gap — see `migration_v1.41.1.sql`.** Same underlying limitation as Side Fund above: v1.41.0's fix only applies to reversals happening from now on. `migration_v1.41.1.sql` backfills already-reversed transactions from before that: deposits via `approveSavingsDeposit`/`createFixedTermSavings` and all handouts are **fully** recoverable, since `member_savings.transaction_id`/`savings_handouts.transaction_id` already existed before v1.41.0 — the migration marks them `REVERSED` and corrects `savings_balances` exactly, the same way the code does for a reversal happening today. The one gap: a deposit credited via the old `creditSavingsContribution` direct-credit path (Record Contribution's "Savings Portion") wrote **no** per-entry record at all before v1.41.0 added one — for a transaction that predates that fix, there is no `member_savings` row and no user-identifying column on `transactions` itself, so the migration cannot determine which member it belonged to. These are reported (amount, date, description) for manual lookup and correction, not silently skipped or guessed at.
- **Fixed in v1.41.0 — reversing a savings deposit or handout's transaction left `savings_balances`/`member_savings`/`savings_handouts` untouched.** Same underlying gap as the v1.40.1 Deposits fix and the v1.41.0 Side Fund fix above. Three of the five savings-crediting code paths (`approveSavingsDeposit`, `createFixedTermSavings`, and — new in this version — `creditSavingsContribution`, the Record Contribution "Savings Portion" slice, which previously updated only the aggregate balance with no per-entry record at all) now all write/link a proper `member_savings` row via `transaction_id`, and `savings_handouts` already had one. `reverseTransaction` now reads that link and rolls it back: for a deposit, a FLEXIBLE entry's `savings_balances.principal_balance` is decremented back (guarded against going negative, e.g. if some of it was already paid out via a handout since) and a FIXED_TERM entry just flips status, since it never touched `savings_balances` in the first place; for a handout, the principal/interest are restored. Both `member_savings.status` and `savings_handouts.status` gained a `'REVERSED'` value (mirroring the existing `shareholder_contributions.status` convention) plus `reversed_at`/`reversed_by` columns. **Deliberately left out of scope**: the two rarer "exit payout" legs that also credit Savings — a Side Fund exit payout and a Deposit exit refund — write no `member_savings` row and the `transactions` table itself has no user-identifying column to fall back on, so there's currently no way to reverse either of those specifically; they're rare, one-time, admin-driven events rather than routine recurring contributions, so this is a known, accepted gap rather than a silent one.
...
*Sections 14.3–14.7 rewritten for v1.33.0 — shares are no longer a raw sum of contributed money. See 14.7 for the full before/after and the reasoning behind the change.*
...
**`share_price_history`** and **`currency_exchange_rates`** (v1.33.0 — same date-effective pattern as `primary_account_floor_limits`): each row has `effective_from`/`effective_to`, so a query for "the price/rate in effect on date X" is a simple range lookup, never today's current value. As of v1.33.0 these two tables are no longer purely for display — they're read for every contribution, past and future, to work out how many units it bought. See `sharePricingService.js`.

**`shareholder_contributions.account_id`** (v1.33.0): which account the contribution's money was actually paid into — Primary by default, but any non-Savings account can be picked (Section 4.2's `POST /transactions/contributions`). The money stays exactly where it's recorded; this field only affects which currency gets converted when computing units, nothing about the ledger posting itself.

##### 14.3 Business rules / key logic — the unit-price model (v1.33.0)
- **Shares are real units bought at a price, not raw money.** For every `APPROVED` contribution, `computeShareUnitsPerUser()` (`transactionsController.js`) works out how many units it bought: convert the contribution's amount from its own account's currency into the share price's currency, using the exchange rate *in effect on the contribution's own date* (a no-op if already the same currency), then divide by the share price *in effect on that same date*. This is exactly the two formulas requested: contribution × rate ÷ price when the currencies differ, which collapses to contribution ÷ price when they don't (rate = 1) — one code path, not two.
- **Units bought in the past never change.** Once a contribution has been converted into units, that unit count is locked in permanently — a later change to the share price or exchange rate never retroactively reprices old contributions. Only a member's *current value* (units × today's price) moves with the market; the unit count itself only grows via new approved contributions. This matches standard mutual-fund/unit-trust/cooperative-society share accounting.
- **Always derived, never directly edited**: as covered in Section 2.3, every approved contribution triggers `recalculateShareholding()` (`transactionsController.js`, extracted as its own function in v1.30.1, rewritten in v1.33.0 to sum units instead of raw money) — a full recompute across *all* shareholders (units per member ÷ grand total units = `percentage`, units themselves become the new `shares_held`) — there is no manual "set a member's percentage" admin action, by design, to prevent the registry from ever disagreeing with the underlying contribution history.
...
- **FX coverage is required up front, not discovered at recompute time.** `creditShareholderContribution` calls `convertToShareCurrency` as a pre-check the moment a contribution is recorded — if no exchange rate has ever been configured covering that currency pair as of that date, the contribution is rejected immediately with a clear error, rather than being accepted and only failing later during a recompute.
...
##### 14.4 API endpoints
Shareholding itself has no dedicated route file — it's read via `GET /api/users/shareholding` (Section 6, Users module) and updated automatically as a side effect of `creditShareholderContribution` and `reverseTransaction` (Transactions module, Section 4.2). There is no direct write endpoint for day-to-day use.

The one exception (v1.33.0), both Admin-only, in `routes/shares.js`:

| Method | Path | Permission | Notes |
|---|---|---|---|
| GET | `/shares/recalculate-preview` | Admin | Read-only — computes what a full recalculate WOULD change (old vs proposed `shares_held`/`percentage` per member) without writing anything. |
| POST | `/shares/recalculate` | Admin | Actually commits the recompute, using the exact same computation the preview just showed (`buildRecalculatePreview` in `sharesController.js`), so preview and commit can never disagree. |

This pair exists for one specific situation: a company whose `share_price_history`/`currency_exchange_rates` tables had no row old enough to cover contributions made before anyone first set a price/rate by hand (see 14.7 and `migration_v1.33.0.sql`). Ordinary day-to-day contributions never need this — they already trigger their own recompute automatically.

##### 14.5 Frontend
Surfaced in multiple places: a "Shareholding" tab/section (likely under Users or a Reports view) listing every shareholder with their current shares/percentage, sortable; individual percentage figures also feed the Dividends and Share Certificates modules' snapshot logic. The Record Contribution modal (`TransactionsPage.jsx`) now has an optional Account selector (defaults to Primary if left blank). The Accounts page (`AccountsPage.jsx`) has a new Admin-only "Shareholding Recalculate" card that opens a preview/commit modal — the old-vs-proposed comparison table, sorted by largest change, with a confirm button that only appears once the preview has loaded.

##### 14.6 Known issues (Shareholding)
- The full-registry recompute on every single contribution (rather than an incremental per-user update) is O(number of shareholders) work on every contribution post — fine at the club's current membership scale, but worth keeping in mind if membership grows into the hundreds+ (cross-reference the earlier scaling-advice conversation about database/computation growth). Unchanged by v1.33.0 — only the per-contribution math changed, not the recompute's shape.
...
- A `price_per_share`/exchange rate must exist covering every contribution's date, all the way back to the very first one ever recorded, or that contribution has no valid conversion to compute units from. `migration_v1.33.0.sql` exists specifically to backfill this for companies with pre-existing contribution history (see 14.7).

##### 14.7 v1.33.0 — from raw money to real units
**Before v1.33.0**: `shares_held` was literally `SUM(shareholder_contributions.amount)` for a member — no division by share price at all. Share price was purely a display-side valuation multiplier (`shares_held × price_per_share`) shown on a member's profile; setting a new price never touched `shareholding_registry`. This meant a member who contributed the same currency at different times always got exactly one "share unit" per currency unit contributed, regardless of what the share price was that day — not standard practice for a company issuing priced shares.

**After v1.33.0**: shares are real units, bought at whatever price was in effect on each specific contribution's date, following the two formulas below (identical logic, since formula 2 is just formula 1 with rate = 1):
1. `units = (contribution amount × exchange rate on that date) ÷ share price on that date` — when the contribution's account currency differs from the share price's currency.
2. `units = contribution amount ÷ share price on that date` — when they're the same currency.

Units bought in the past are permanently locked in — a later share price or exchange rate change never retroactively re-prices old contributions, only today's valuation of them.

**Migration path for existing data**: because `share_price_history`/`currency_exchange_rates` never shipped with a seeded starting row, a company whose first real contributions predate the first time someone manually set a price/rate has no valid historical price/rate to convert those early contributions with. `cms/migration_v1.33.0.sql` backfills exactly that — a single idempotent `founding price`/`founding rate` row dated to each company's earliest `APPROVED` contribution (edit the two company-specific constants at the top before running: ZWECK TUKULA = UGX 100,000, INVESTABO GLOBAL = UGX 50,000; founding EUR→UGX rate = 4,000 for both). It deliberately does not touch `shareholding_registry` itself — run the migration first, then use Settings → Accounts → "Shareholding Recalculate" (Admin only, Section 14.4/14.5) to review the before/after preview and commit the one-time historical recompute.
...
| POST | `/:id/archive` | `DOCUMENT_ARCHIVE` | Move a DRAFT/FINAL document to ARCHIVED (Company Archive) |
| DELETE | `/:id` | `DOCUMENT_DELETE` (v1.46.0 — see 16.6) | Soft-remove an already-archived document from the archive |
...
- **Resolved in v1.46.0 — there was no way to remove a document from the Company Archive once it landed there.** Reported directly: "i cannot remove documents from the archive once i have put them there which i should be able to do." `archiveDocument` (`POST /:id/archive`) was always a one-way DRAFT/FINAL → ARCHIVED move with no counterpart, and the Company Archive tab's own copy said outright "these are permanently stored." New `deleteDocument` (`DELETE /:id`, `documentsController.js`) is the missing "take it back out" action, gated by a new `DOCUMENT_DELETE` permission (not auto-granted to any role, including Admin — must be granted in Settings → Roles → Permissions). Eligibility deliberately matches whatever the Company Archive VIEW itself already shows — either `status = 'ARCHIVED'` (a document that went through the regular lifecycle and was later archived) OR `related_record_type = 'COMPANY_ARCHIVE'` (a document uploaded directly into the archive, which never goes through a separate Archive step and would otherwise be permanently stuck) — not just the narrower `status = 'ARCHIVED'` alone, which would have left most direct archive uploads undeletable. This is a **soft** removal — a new `DELETED` status value (`migration_v1.46.0.sql`, widening the existing `documents.status` CHECK constraint), never a real `DELETE FROM documents` row-delete, since the row is referenced by several enforced foreign keys with no `ON DELETE` clause (grants, grant_conditions, loans_received/given, staff_document_grants, audit_engagement_documents, report_log — any of these would simply error) and by two *unenforced*, polymorphic `target_type`/`target_id` links (`document_signatures`, `document_stamps_applied`) that a hard delete would silently orphan rather than error on, corrupting the audit trail of who signed/stamped something that no longer exists. `getAllDocuments` excludes `DELETED` from every list view the same way it already excluded `SUPERSEDED`, so a removed document simply disappears — from the Company Archive tab, the general "All Documents" list, and its own status filter — with the underlying row and its full history intact in the database. A Remove/Trash-icon action was added both to the Company Archive card grid and to the "All Documents" table's row actions (shown only once a document is actually `ARCHIVED`, with a confirmation prompt first). Verified end-to-end against `@electric-sql/pglite`: an active DRAFT document is correctly refused; an ARCHIVED document and a direct-to-archive DRAFT upload can both be removed; a document already removed can't be removed again; and a removed document disappears from both the general list and the Company-Archive-filtered view.
- **Resolved in v1.46.0 — generating a Member Portfolio Summary saved an unwanted document record.** Reported directly: "when someone generates or tries to print their portfolio summary it is saved as document record in the system which isn't supposed to happen." `MemberPortfolioPage.jsx` had two buttons doing almost the same thing — "Print" (`handlePrint`, purely client-side: renders `memberPortfolioTemplate()` and opens the browser's print dialog, no backend call at all) and "Generate Portfolio Summary" (`handleGenerateDocument`, which additionally called `documentsAPI.generate(...)`, persisting a `FINANCIAL_REPORT_INDIVIDUAL` document every time someone printed their own portfolio). The second button and its handler were removed entirely — printing/saving-as-PDF (via the browser's own print dialog) is the only action on this page now, and it never touches the `documents` table. Documents of this type generated before this fix are unaffected and still preview/reopen correctly from the Documents page (the `FINANCIAL_REPORT_INDIVIDUAL` renderer entry in `GENERATED_RENDERERS` was kept for exactly that reason) — this only stops new ones from being created going forward.
...
- **Monthly report emails now attach a PDF with a brief summary in the body, instead of dumping the whole report inline (v1.46.0).** Requested directly, alongside the equivalent question about share-certificate emails (already true — see below). Before this version, `sendGeneralReportToAllMembers`/`sendIndividualReportsToAllMembers` (`reportService.js`) sent the ENTIRE report — every account, every transaction category, every contribution row — as the raw email body HTML, with no attachment at all. Now the full detail (the exact same `renderGeneralReportHTML`/`renderIndividualReportHTML` output as before) is rendered to a PDF via a new shared `pdfService.renderHtmlToPdfBuffer()` — factored out of `certificateService.js`'s own Puppeteer rendering (Section 4.13/29), which now just delegates to it — and attached to the email; the email body itself shrank to a short bullet-point summary (`buildGeneralReportSummaryHtml`/`buildIndividualReportSummaryHtml`): total income/expenses and active investment/loan/grant/upcoming-event counts for the General report, shares held/contributions-this-period/total-contributed for the Personal report. The General report's PDF is rendered ONCE and the identical buffer reused for every recipient (it's the same document for everyone); the Personal report's PDF is necessarily rendered once per member, since each one is genuinely different — the same per-recipient Puppeteer cost the monthly certificate pipeline already accepts for exactly the same reason (Section 7.4's documented Puppeteer cold-start note applies here too, not a new risk). **Share-certificate emails already matched this exact "brief body + PDF attachment" pattern from the start** (`certificateService.emailCertificateToUser`, Section 4.29/30) — nothing needed to change there. Verified end-to-end against `@electric-sql/pglite` (with `pdfService`/`sendEmail`/`notify` stubbed, since this sandbox has no real headless Chrome or SMTP): each recipient's email carries exactly one PDF attachment and a summary body that does NOT contain the full report's own section headings (proving the full detail moved to the PDF, not just got prepended to); the General report's PDF is rendered exactly once total, the Personal report's once per member; and a member with no shareholding row or no contributions that period still gets a correctly-degraded (not broken) summary.
...
##### 18.2 Data model
**`notifications`**: id, user_id, title, message, notification_type, is_read, related_record_type, related_record_id, **link** (the frontend route the bell item navigates to when clicked), created_at.
...
- **Fixed in v1.41.1 — v1.41.0's link fix didn't reach notifications already sitting in the table.** A notification's `link` is written once, at creation time, into the row itself — `notify()`/`notifyMany()` never recomputes it later, and the frontend never re-derives it from the notification's type on click. So v1.41.0's code fix only ever affected *new* notifications going forward; every notification already in the `notifications` table kept whichever broken link it was created with, and kept bouncing to the Dashboard exactly as before — reported directly, with a screenshot of a "New requisition awaiting your approval" notification still doing it. `migration_v1.41.1.sql` runs a one-time `UPDATE notifications SET link = ...` pass over existing rows, matching the exact same broken patterns v1.41.0 fixed in code (`/dashboard`, `/reports/me`, `/certificates`, and the `/grants/:id`/`/events/:id`/`/requisitions/:id`/`/transactions/:id`/`/transfers/:id` shapes) and rewriting them to the same corrected destinations. Safe to run more than once — a row that's already correct simply won't match. The same pass also caught two links v1.41.0 missed entirely: `transfersController.js`'s `TRANSFER_APPROVAL_PROGRESS` and `TRANSFER_REJECTED` notifications were still using `/transfers/${transfer.id}` (only the third, `TRANSFER_POSTED`, had actually been fixed) — corrected in code alongside the data backfill.
- **Fixed in v1.41.0 — some notification `link`s bounced to the Dashboard instead of the page they should attend to.** Reported directly: "let all notices that appear on it link properly and accurately to the pages one needs to attend to ... i have noticed that a few work well and some just take you to the dashboard." Root cause: `App.js`'s router has a catch-all route (`path="*"` → `<Navigate to="/" replace />`) — any `link` pointing at a route that doesn't actually exist silently redirects to the Dashboard rather than erroring, which is exactly what happened for several hardcoded links written against routes that were never built (or were built with a different shape) — `/dashboard` (redundant, correct route is `/`), `/reports/me`, `/grants/:id`, `/events/:id`, `/certificates`, `/requisitions/:reqId`, `/transactions/:transactionId`, and `/transfers/:id`. Only these `:id`-parameterized detail routes actually exist in the router: loans received/given, investments, MMF, capital goals, and a user's own portfolio — grants/events/transactions/transfers/requisitions are list-only pages with no per-record deep link support. Each broken link was repointed to the correct **existing** page — the redundant dashboard links to `/`, the Reports self-view link to `/portfolio`, and grants/events/certificates/requisitions/transactions/transfers to their respective list pages (`/grants`, `/events`, `/profile`, `/requisitions`, `/transactions`, `/transfers`) — rather than building new detail routes/pages for each, which was out of scope for this fix. Links that already pointed at a valid list page (Savings, Side Fund, Fines, Deposits, Service Fees, Payment Acknowledgements, Audit, Documents) were left unchanged since they already worked, just without a deep link to one specific row.
- **Fixed in v1.41.0 — the bell dropdown wasn't fully visible on small screens.** The dropdown's width was already responsive, but had no overall height cap — only its two inner scrollable sections did (260px/380px), so header + both lists + footer could total roughly 700-730px, taller than many phone viewports, leaving the footer (and sometimes list content) unreachable. Fixed by capping the outer dropdown at `min(640px, calc(100vh - 88px))` with its own `overflowY: auto`, as a flex column with the header/footer pinned (`flexShrink: 0`) and the two inner lists still independently scrollable within that outer cap — a nested-scroll layout rather than one single long scroll region.
...
##### 18.5 Frontend
The bell dropdown lives inside `TopBar.jsx` itself (not a separate page) — unread count badge, dropdown list, "Mark all read," footer text that is now conditional on Auditor status (Section 3). As of v1.41.0 the outer dropdown container is height-capped and independently scrollable so it stays fully usable on small screens (18.3 above).

##### 18.6 Known issues (Notifications)
- **Several list-only modules still have no per-record deep link** (Grants, Events, Transactions, Transfers, Requisitions) — a notification for one of these opens the right list page, not the specific row it's about. Building dedicated `:id` detail routes/pages for these was out of scope for the v1.41.0 link fix (which only repointed flat-out broken links to the nearest correct existing page) and would be a larger, separate undertaking if deep linking to one exact record is wanted later.
- Otherwise none specific beyond the general pattern already documented — the best-effort/non-blocking design is a deliberate and sound choice, not a gap.
...
- **Fixed in v1.46.0 — clicking Generate more than once on `GenerateDocumentPage.jsx` created the exact same document multiple times.** Reported directly: "after one has filled in the fields to generate a document using the templates established, the page is unchanged and one may press the generate button more than once to generate the same document multiple times with the same content." Filling in the form used to call `documentsAPI.generate(payload)` (a real, persisting POST) directly from the submit handler, with nothing to stop — or even visually discourage — a second click from doing it all again. The form's single Generate step is now split into two: **Preview** (`handlePreview`) renders the document locally via the same `exportUtils.js` template function used before, entirely client-side — no backend call, nothing saved, safe to click any number of times while adjusting fields — and **Confirm & Save to Library** (`handleConfirmGenerate`), which appears only once a preview exists and is the one and only place `documentsAPI.generate()` is ever called. Confirm disables itself the instant it succeeds (a `generated` flag, checked alongside the existing `loading` guard) — the only way to save a second time is to deliberately change a field and preview again, which is a genuinely new document, not an accidental duplicate. This also directly delivers the second half of the same request — "let the system show a preview of the entered information before actually fully generating the document in case they may have to change something" — since the preview is now mandatory and happens before anything is persisted, not (as before) as a side-effect shown after saving. `MEETING_AGENDA`/`MEETING_MINUTES`/`RESOLUTION`'s v1.45.0 attendee/chairman/secretary pickers (Section 29.9) are unaffected — they still resolve at Confirm time from the same `pendingPayload` built at Preview time.
#### 22. AUTHENTICATION
...
- **Fixed in v1.35.1 — a transient network error (e.g. the backend mid-restart) forced a full logout, not just a failed request.** Two places conflated "the server explicitly rejected this token" with "we couldn't reach the server at all," treating both identically: `AuthContext.js`'s `initAuth()` (runs on every page load/refresh) wiped the cached session and set `user = null` on *any* failure from the profile-refresh call, not just a 401/403 — a plain connectivity blip during app start had exactly the same effect as a genuinely expired token. `axios.js`'s response interceptor had the same conflation one level down: if the `/auth/refresh` call itself couldn't reach the backend, the resulting network error was treated the same as the refresh token actually being invalid, clearing the session and hard-redirecting to `/login`. Both now check `err.response?.status` and only clear the session on an explicit 401/403 from the server — a network error now just fails that one request and leaves the cached session alone, so the next successful request (or the user simply waiting a few seconds) recovers on its own instead of forcing a full re-login. Reported as "continuous network error that always takes me to the login page" correlating with a backend that was restarting roughly every 16 minutes with no crash or error in its own logs — root-caused and fixed immediately after in v1.35.2 (Section 7.1): the app's own rate limiter was throttling Render's health check into a 429, and Render restarted the "unresponsive" instance. Code-only, no schema migration.
...
##### 23.1 Purpose
Member/officer account management — profile data, role assignment, activation/deactivation, the shareholding view (Section 4.14), and (v1.34.0) the full Member Portfolio snapshot exposed via this module's routes.
...
**`user_roles`** (join table): user_id, role_id, assigned_by, assigned_at — a user can hold multiple roles simultaneously (e.g. Treasurer + Director). Uniqueness (v1.35.3) is enforced by a **partial** index, `user_roles_active_role_unique ON (user_id, role_id) WHERE revoked_at IS NULL` — a member can't hold the same role twice at once, but a role can be assigned, revoked, and re-assigned to the same person any number of times over their membership; there is no table-wide `UNIQUE(user_id, role_id)` anymore.
...
| GET | `/:id/portfolio` | `isSelfOrHasPermission('USER_VIEW_ALL')` | v1.34.0 — full Member Portfolio snapshot (see 23.7) |
| GET | `/:id/deletion-check` | `USER_MANAGE` | v1.35.0 — read-only: can this account be permanently deleted? (see 23.8) |
| DELETE | `/:id` | `USER_MANAGE` | v1.35.0 — permanently delete an account with zero activity on record (see 23.8) |

##### 23.5 Frontend
`cms-frontend/src/pages/users/UsersPage.jsx` — member directory table with role badges, active/inactive status, Admin-only "Edit"/"Assign Roles"/"Deactivate" actions, and (v1.34.0) a "View Portfolio" link on every row for anyone holding `USER_VIEW_ALL`, plus (v1.35.0) a "Delete Permanently" action for `USER_MANAGE` holders; a personal "My Profile" page (`ProfilePage.jsx`) for self-service edits, accessible to every role including Auditor. The photo camera-icon button opens the OS file picker, then (v1.28.3) `PhotoCropModal` for framing before the cropped result is uploaded — `reloadProfile()` calls `AuthContext`'s `refreshUser()` afterward, so the TopBar/sidebar avatar update immediately without needing a page reload.
...
- **Fixed (v1.35.3)**: Assign Role failed with the generic "A record with this value already exists" for any member being given back a role they'd previously held and lost — even though their Current Roles correctly showed nothing active. `user_roles` had carried a table-wide `UNIQUE(user_id, role_id)` since the very first schema; `assignRole`'s own "already holds this role?" check only looks at active rows (`revoked_at IS NULL`), so it correctly says yes-go-ahead, but the INSERT that follows then collided with the constraint anyway, since a REVOKED row for that same pairing still counted. First surfaced after using the new Permanent Deletion feature (v1.35.0) and then trying to hand that same role to someone who'd held it before — the deletion itself was working correctly; it just led to a role reassignment that hit this separate, pre-existing bug. Fixed in `migration_v1.35.3.sql` by replacing the constraint with a **partial** unique index (`WHERE revoked_at IS NULL`) — see 23.2.

##### 23.7 Member Portfolio (v1.34.0)
A dedicated detail page, `MemberPortfolioPage.jsx`, showing everything about one member's standing in the company — the "one page that answers every question about this member" the Users list itself was never meant to be. Reached two ways: an Admin (or anyone else holding `USER_VIEW_ALL`) clicking "View Portfolio" on any row of the Users page (route `/users/:id/portfolio`), or any logged-in member tapping their own photo/name/roles at the top of the Sidebar — previously just static display, not clickable at all — which opens the same page for themselves only (route `/portfolio`, no `:id`, defaults to the logged-in user). Both routes render the identical component; access control is enforced entirely server-side by the same `isSelfOrHasPermission('USER_VIEW_ALL')` gate the plain `GET /:id` profile lookup already used, so a member without that permission who tries to view someone else's portfolio gets a 403 regardless of what the frontend shows.

**What it shows** — assembled by a new `portfolioService.buildMemberPortfolio(userId)` (`cms/src/services/portfolioService.js`), which queries each domain independently (these tables don't share one natural join key) and returns a single snapshot object: profile + every role ever held (current and revoked, with who assigned/revoked and when); shareholding — current shares/percentage, today's value at the current share price, full contribution history, all-time approved total; savings — principal/accrued-interest/total-interest-paid balance plus recent deposits and handouts; dividends received (all-time total plus every distribution); side fund standing — current membership status and the full dues history, with an overdue total called out; every payment ever paid out to this member across all types, unified through `payment_acknowledgements` (Section 35) — dividends, service fees, reimbursements, savings handouts, side fund payouts, no separate query per type needed; and every transaction they've been involved in, whether as the member the money is for (`transactions.contributed_by`) or as the staff member who created or approved the posting (`created_by`/`approved_by`) — a transaction can match more than one of these at once (e.g. a Treasurer recording their own contribution), and the page labels every way it matches rather than picking just one.

**Printable summary** — two ways to get a printable copy, both driving the same `memberPortfolioTemplate()` (`exportUtils.js`) off the portfolio data already on screen, no re-typing anything into a form:
- **Print** — an instant browser print, nothing saved, same pattern as the Transaction Ledger export.
- **Generate Portfolio Summary** — saves a real, audit-logged Document (`document_type` `FINANCIAL_REPORT_INDIVIDUAL`, which was already an allowed value in both `document_templates`/`documents` CHECK constraints since the schema was first written, just never finished) via the exact same generic `POST /documents/generate` endpoint Receipts/Resolutions use (Section 16) — downloadable and re-printable later from the Documents module, not just a one-off. `migration_v1.34.0.sql` seeds the one `document_templates` row this needs; `DocumentsPage.jsx`'s `GENERATED_RENDERERS` map gained a `FINANCIAL_REPORT_INDIVIDUAL` entry so a previously-generated summary can still be reopened the same way every other generated document type is.

**Deliberately separate from `reportService.generateIndividualReport()`** (Section 17's monthly report pipeline) — that one is a period-scoped (one calendar month), contribution-only report built for the automated email pipeline (`sendIndividualReportsToAllMembers`); this is the always-current, all-time, every-domain snapshot meant to be looked up on demand. The two share no code and can drift in what they show without one needing to track the other.

##### 23.8 Permanent Deletion (v1.35.0)
Deactivate (23.3) has always been, and remains, the correct tool for "this member shouldn't have access anymore" — it's safe, reversible, and never touches a single financial or historical record. This is a deliberately narrower, separate action for exactly one situation: someone registered more than once (typically a different email each time, out of confusion about which one they meant to use) and the extra account was never actually used for anything. `userDeletionService.js` exists specifically to make that possible without ever risking real data.

**Why a plain `DELETE FROM users` isn't safe.** Virtually every table in this schema — contributions, transactions, dividends, savings, side fund activity, documents, the audit log, and dozens more — has a foreign key back to `users(id)`. A user who has ever contributed, been assigned a role and logged in, approved something, or even just been named as the "recorded by"/"approved by" on someone else's record cannot be deleted without either Postgres rejecting it outright or cascading the delete into real financial/governance history — something this system never does anywhere else (`audit_log` is explicitly designed to be append-only and never deleted, Section 25.2).

**How the safety check works.** Rather than hand-maintain a list of "every table that might reference a user" — this schema has roughly 90 such foreign-key columns across roughly 60 tables, and any hand-written list would silently go stale the moment a new table is added — `getUserFootprint(userId)` reads PostgreSQL's own catalog (`pg_constraint`/`pg_attribute`) for every foreign key that points at `users(id)`, then checks each one for rows belonging to this user. This stays correct automatically as the schema grows; nothing here needs updating when a future migration adds a new table with a `users(id)` reference. The one deliberate inclusion worth calling out: `users.created_by` (who created this account) is itself such a foreign key, so if the account being checked ever created *other* accounts, that counts as real activity and blocks the delete too.

**The one allowlist.** A genuinely unused account can still have picked up a small amount of purely self-contained bookkeeping along the way — a role request, a role assignment that was never actually exercised, its own bell notifications, agreeing to the membership agreement. None of these have any effect on anyone else or on company finances, so they're cleaned up automatically as part of the delete rather than blocking it: `notifications`, `role_requests`, `user_roles`, and `member_consents` rows for this user are deleted; `audit_log` rows are kept exactly as they are except `user_id` is set to `NULL` — honouring the "never delete a log row" rule while still letting the foreign key go away. Every other table found by the catalog scan blocks the deletion if it has even one matching row.

**The flow, end to end.** `GET /:id/deletion-check` is read-only and safe to call at any time — it returns `{ clean, blocking: [{ table, column, count }] }`. The Users page's "Delete Permanently" action runs this the moment its confirmation modal opens: if `blocking` is non-empty, the modal lists exactly which tables have activity and tells the admin to use Deactivate instead, with no delete button shown at all. Only when the account comes back clean does a "Delete Permanently" button appear, and `DELETE /:id` re-runs the exact same check inside the deletion's own database transaction before doing anything irreversible — closing the gap where a fresh contribution could theoretically land between the admin opening the modal and clicking confirm. A best-effort attempt is also made to remove any uploaded photo/signature file from storage; a failure there never blocks the deletion itself.

##### 23.9 Known issues (Permanent Deletion)
- The `pg_constraint` introspection query is standard PostgreSQL catalog SQL and was reviewed carefully by hand, but this sandbox has no live database to execute it against — it has not yet been run against a real deployment. Run `GET /:id/deletion-check` against a genuinely unused account (and, as a sanity check, against your own heavily-active account — it should come back firmly not-clean) before relying on the delete button for the first time.
...
| `REACT_APP_VERSION` | `1.38.0` | `1.38.0` (kept in sync manually — a pure display string, nothing reads it programmatically) |
...
v1.47.0 — converted every route below from hardcoded `requireRoles([...])` to the permission system (`SERVICE_FEE_VIEW`/`SERVICE_FEE_MANAGE`), the same shape as every other finance module. Like every permission in this system, neither is auto-granted — see 28.6.

| Method | Path | Who | Description |
|---|---|---|---|
| GET | `/agreements` | `SERVICE_FEE_VIEW` | List all agreements |
| GET | `/agreements/:id` | `SERVICE_FEE_VIEW` | Full detail incl. payment history and amendment history (v1.47.0) |
| POST | `/agreements` | `SERVICE_FEE_MANAGE` | Create a new agreement |
| PATCH | `/agreements/:id` | `SERVICE_FEE_MANAGE` | Edit / end an agreement — v1.47.0: changing `monthly_amount` now requires `reason` + `effective_from` and is recorded as a new row in `service_fee_agreement_amendments` (never overwritten) |
| POST | `/agreements/:id/pay` | `SERVICE_FEE_MANAGE` | Record a monthly payment (defaults to the standard monthly amount) — v1.39.0: creates a pending Payment Confirmation (Section 38) rather than posting immediately; also requires `payment_method` (Cash/Bank Transfer/Mobile Money) |
...
| GET | `/reimbursements` | `SERVICE_FEE_VIEW` | Review queue, optional `?status=` filter |
| GET | `/reimbursements/:id/receipt` | Owner or reviewer | Download the attached receipt |
| POST | `/reimbursements/:id/approve` | `SERVICE_FEE_MANAGE` | Approve — posts and pays the transaction in one step |
| POST | `/reimbursements/:id/reject` | `SERVICE_FEE_MANAGE` | Reject with a required reason |
...
- **`/service-fees`** (`ServiceFeesPage.jsx`) — three tabs: "My Service Fee" (everyone — own agreement, payment history, a "Request Reimbursement" action, own reimbursement history), "Agreements" (anyone holding `SERVICE_FEE_VIEW` — list, clickable rows; create/record-payment/amend/terminate require `SERVICE_FEE_MANAGE`), "Reimbursement Requests" (`SERVICE_FEE_VIEW` to see the queue — the request-list monitoring guarantee added in v1.47.0 — but the Approve/Reject buttons only render for `SERVICE_FEE_MANAGE`, with a pending-count badge on the tab itself).
- **`/service-fees/agreements/:id`** (`ServiceFeeAgreementDetailPage.jsx`, v1.47.0) — a single agreement's own page, reached by clicking its row in the Agreements table (mirrors Loans' `LoanDetailPage.jsx`): header stats (current monthly amount, total paid to date), the Record Payment/Amend/Terminate actions relocated here from the flat table (same actions, same `SERVICE_FEE_MANAGE` gate — nothing new), the full payment history, the new amendment history (previous → new amount, reason, effective date, who changed it), and a Download Summary button (`serviceFeeAgreementTemplate`, `exportUtils.js`) for a printable statement covering both histories.
...
3a. (v1.47.0) Grant `SERVICE_FEE_VIEW`/`SERVICE_FEE_MANAGE` to whichever roles should administer Service Fees (Admin, Treasurer, Assistant Treasurer previously had this by hardcoded role — now nobody does until granted).
...
- **v1.39.0**: `recordPayment` no longer posts its transaction immediately — it creates a pending Payment Confirmation instead (Section 38), and the fee recipient must confirm it before the transaction (and the `service_fee_payments` history row) actually exists. `approveReimbursement` is unchanged, still posting immediately followed by the original Payment Acknowledgement signoff.

---

#### 29. Digital Consent & Multi-Signatory Approval (v1.23.0; widened v1.44.0 — see 29.8; person-specific signatories v1.45.0 — see 29.9)
...
- **`signature_requirements`** — Admin-configured: `document_type` + `role_id` + `is_active`. A document type with zero active rows has no multi-signature requirement. Originally just the 4 types named above; **widened in v1.44.0 (29.8) to all 14 document types** the system has (every `documents.document_type` value plus `SHARE_CERTIFICATE`) — being eligible doesn't change anything for a type on its own, an Admin still has to actually select roles for it.
- **`document_signatures`** — the generic signing-slot table, reused for two different kinds of "thing being signed" via `target_type`:
  ...
  Each row is one required slot, either role-based (`required_role_id` — whoever currently holds the role may fill it) OR, since v1.45.0 (29.9), person-based (`required_user_id` + a free-text `position_title` like `'Chairman'` — only that exact person may fill it, regardless of role). Exactly one of the two is set, enforced by a CHECK constraint. Also: `status` (`PENDING`/`SIGNED`), `signed_by`, `signature_snapshot_path` (a **copy** of the signer's `users.signature_path` taken at the moment they sign — this is what makes a later signature redraw safe), `signed_at`.
...
| POST | `/documents/:id/sign` | Any role-assigned, consented member *(was `DOCUMENT_APPROVE` — decoupled in v1.44.0, 29.8)* | Fill the caller's role's pending slot on a document — `signSlot` itself enforces that the caller actually holds a required role |
| GET | `/documents/:id/signatures` | Any authenticated user | Current signing status for a document |
| GET | `/documents/pending-signatures` | Any authenticated user *(v1.44.0, 29.8)* | Everything — documents AND certificate rounds — currently awaiting the caller's own signature |
| GET | `/certificates/rounds` | Treasurer / Assistant Treasurer / Admin | List every signing round (management view) |
| GET | `/certificates/rounds/:id` | Treasurer / Assistant Treasurer / Admin, OR a current pending signatory of that round *(widened in v1.44.0, 29.8)* | One round + its signature status |
...
- **Documents page** — a Signatures icon (amber while pending, green once fully signed) on any document row whose type is eligible opens a modal listing each required role, who (if anyone) signed, their signature image, and a Sign button if the current user's role has an open slot. Reused for certificate rounds too (v1.44.0 — see 29.8).
- **Reports page → Certificate Signing Rounds panel** (Treasurer/Assistant Treasurer/Admin) — expandable list of every round, each showing per-role signature status and a Sign action for eligible signatories. This stays the *management* view (browse/audit every round ever opened) — a required signatory who isn't one of those three roles now reaches an individual round through the Pending My Signature tab instead (29.8), not through this panel.
...
##### 29.8 Role-wide access, visibility gating, and a unified pending-signature view (v1.44.0)

Requested directly: signatures should appear on every approved document and wherever a signature is required per the roles a member holds; with more than one Director, any of them can sign and their own specific name/signature is what appears; every document requiring a signature (including share certificates) should surface in one dedicated place to sign from; a document can't be posted to the general Documents view until every signatory has approved it; share certificates should carry the required signatures even though they live outside the general Documents list; the signer's name and the signing date should auto-fill; and redrawing a signature later must never alter an already-signed document.

Investigating what v1.23.0 already did against this list turned up that most of it was already true — `signSlot` matches on role (not person), records `signed_by`/`signed_at` automatically, and snapshots the signature file at sign time so a later redraw never touches an already-signed record (29.2/29.3). "Any of several directors can sign" already worked exactly as asked, since a slot belongs to the *role* Director, not to a specific director. What was genuinely missing or actively broken, fixed here:

1. **Signing a document required `DOCUMENT_APPROVE` on top of being a configured signatory role.** If an Admin added a role as a required signer in Settings → Signatories without separately granting that role `DOCUMENT_APPROVE`, its members could see the pending slot but got a 403 trying to fill it, for a reason nothing in the UI explained. `POST /documents/:id/sign` no longer carries that permission gate (`routes/documents.js`) — `signSlot`'s own "does the caller currently hold a role with a pending slot here" check is sufficient, exactly the pattern `POST /certificates/rounds/:id/sign` already used successfully. No permission change is needed in Settings → Roles for a newly-added signatory role to actually be able to sign.
2. **Certificate signing rounds could only ever be viewed/signed by Treasurer, Assistant Treasurer, or Admin** (`GET /certificates/rounds/:id`), regardless of who was actually configured as a required signatory. A Director-only signatory role was notified ("your signature is needed") but had no page that would load for them — a real functional gap, not just a documentation one. `getRoundById` (`certificatesController.js`) now allows either those three management roles (unchanged, for browsing/auditing any round) OR a caller who currently holds one of that specific round's still-PENDING required roles. `GET /certificates/rounds` (the full list) stays management-only, since it's a browse-everything view, not a "what do I need to act on" one.
3. **New `GET /documents/pending-signatures`** (`signatureService.getMyPendingSignatures`) — the first single place in the system that shows both regular documents and certificate rounds together, since neither the Documents page nor the Reports page ever combined the two. One row per target the caller currently holds a pending slot for, `role_names` collecting every one of the caller's own matching roles on that target (a person can hold more than one required role). Powers the new **Pending My Signature** tab on the Documents page (badge shows the outstanding count), which reuses the existing `SignaturesModal` — generalized to branch between `documentsAPI`/`certificatesAPI` under the hood via a `targetType` field, rendering identically either way since both are backed by the same `document_signatures` table.
4. **A document with open signature slots was fully visible to everyone in the general Documents list**, just flagged amber instead of green. `getAllDocuments` now hides a `DRAFT` document that still has at least one `PENDING` required slot from anyone who is not: its own creator, an Admin, or a current holder of one of its still-pending required roles. It reappears for everyone automatically the moment the last required signature lands — no separate "publish" step, and a document type with no signature requirement configured is completely unaffected (the hiding condition is specifically "has an open PENDING slot," which such a document never has). This check does not apply to `GET /documents/:id` (a direct link still opens if you know the ID) — only to the general list, matching what was actually asked.
5. **Only 4 document types could ever require a signature.** `SIGNABLE_DOCUMENT_TYPES` (`signatureService.js`, now the single source of truth — `settingsController.js` previously kept its own separately-maintained, easily-driftable copy of the same list) widened from 4 to all 14, mirroring `STAMPABLE_DOCUMENT_TYPES`'s own already-wider list. The `signature_requirements.document_type` CHECK constraint was widened to match (`migration_v1.44.0.sql`). Purely opt-in either way — nothing changes for a document type until an Admin actually selects required roles for it in Settings → Signatories, which now offers all 14 types instead of 4.

No new tables. Verified end-to-end against a real embedded Postgres (`@electric-sql/pglite`) driving the actual shipped controller handlers: a newly-eligible type (`CONTRACT`) can require a signature and enforces it correctly; one of two directors signs with zero permissions granted at all and the document still finalises correctly, recording that specific director's own name; the other director is correctly rejected from signing the same already-filled slot; an open-slot document is hidden from an unrelated shareholder but visible to its creator, an Admin, and the pending signatory; `getMyPendingSignatures` returns the right items and stops listing something once it's signed; a Director-only certificate-round signatory (holding none of the three management roles) can now view and sign a round that would previously 403, while an unrelated shareholder is still correctly forbidden.

##### 29.9 Person-specific signatories from template attendee fields (v1.45.0)

Requested directly: on the Meeting Minutes/Agenda/Resolution templates, the Chairperson/Secretary/attendee fields should be fillable from a dropdown of system users, or a typed free-text name for someone not in the system; picking a real system user as Chairman or Secretary should mean that specific person's signature is recorded on the document once they approve it.

Everything in 29.1–29.8 above requires a ROLE — a slot is filled by whoever currently holds it, not by one named individual. That model doesn't fit "the person named as Chairman on THIS meeting must sign THIS document," since the person chairing a specific meeting may not hold any role the system tracks (e.g. a guest chair, or a Shareholder standing in) and, conversely, someone who does hold the Secretary role today might not be the one who actually took these particular minutes. This version adds a second, independent way to require a signature — naming one specific person — that coexists with the existing role-based one on the exact same document.

1. **`document_signatures.required_role_id` is now nullable; a new `required_user_id` + `position_title` pair covers the person-based case** (29.2, schema/`migration_v1.45.0.sql`). A CHECK constraint keeps exactly one of the two set per row, and the old single `UNIQUE(target_type, target_id, required_role_id)` constraint is replaced by two partial unique indexes (one per kind) since a plain composite UNIQUE stops deduplicating once one of its columns is legitimately `NULL` half the time. `ensurePersonSignatureSlots(client, targetType, targetId, people)` (`signatureService.js`) is the person-based counterpart to the existing `ensureSignatureSlots` — idempotent, same `ON CONFLICT ... DO NOTHING` shape. `signSlot` now matches a caller against a pending slot by **either** their currently-held roles **or** their own user ID directly (`required_user_id = $callerId OR required_role_id = ANY($callerRoleIds)`) — a person named as Chairman can sign their slot even if they hold no relevant role at all, and conversely someone who *does* hold the Secretary role but wasn't the person named on this specific document cannot fill that slot in their place. `getSignatureStatus`/`getMyPendingSignatures`/`notifyPendingSignatories` were all extended in parallel to surface person-based rows the same way role-based ones already were, using `position_title` (falling back to the person's own name) as the display label everywhere a role name used to be — existing frontend code reading `role_name` needed no change.
2. **Confirmed intent, via clarifying questions before building this:** selecting a person from the dropdown does **not** auto-sign anything — that person still has to actively log in and sign it themselves, exactly like every other signature in this system; naming them makes them specifically the required signer for that one document, even if they don't currently hold the corresponding role; this treatment applies to Meeting Minutes, Meeting Agenda, and Resolutions; and a free-typed name (not matched to a real system user) is printed-only, with a blank manual-signature line, and never blocks the document from being finalised.
3. **`documentsController.approveDocument`** (`extractPersonSignatories`) reads `chairperson_user_id`/`secretary_user_id` straight out of the document's own `template_data` (set at generation time — see 21/frontend below) for these three document types, and — in the same transaction as the existing role-based `ensureSignatureSlots` call — also opens a person-based slot for each one that resolved to a real system user. **The two mechanisms are fully independent and can both be active on the same document**: if Settings → Signatories separately requires, say, the Admin role to sign every Meeting Minutes, that role-based slot and the document's own named Chairman/Secretary slots all open together, and the document only reaches `FINAL` once every one of them — of either kind — is signed. A field left as free text (no matching system user) simply never gets a slot at all, and never counts toward "fully signed."
4. **Frontend (`GenerateDocumentPage.jsx`)** — the Chairperson/Secretary fields on all three templates changed from plain text inputs to a new `PersonPicker` component: a dropdown of active system users (`usersAPI.getAllUsers`), with a toggle to switch to a plain text box for someone not in the system. Selecting a user stores `${field}_user_id` + `${field}_name`; typing a name stores only `${field}_name`. The Attendees (Meeting Agenda) and Members Present (Meeting Minutes) fields, previously a single free-text blob, became a repeatable list of the same picker (`PersonListField`) — purely cosmetic on the printed document (no signature implications), matching what was asked for attendee fields generally. `exportUtils.js`'s `meetingAgendaTemplate`/`meetingMinutesTemplate`/`resolutionTemplate` read the new shape via two small helpers, `personName(data, key)` (checks `${key}_name` first, falls back to the old plain `${key}` string so already-generated documents from before this version keep rendering correctly) and `personListNames(value)` (same backward-compatible fallback for the attendee-list fields). Meeting Agenda's template gained a `.signature-section` (Chairperson/Secretary blank lines) for the first time, matching Minutes/Resolutions, since a named Chairman/Secretary on an agenda can now be a real required signer.

No change to how a signature is actually *rendered onto* a document — same as before this version (29.6/30.6), the real recorded signature image only ever appears in the Signatures modal, never baked into the client-rendered template HTML; the printed document always shows a blank line and the person's printed name regardless of digital signing status.

Verified end-to-end against a real embedded Postgres (`@electric-sql/pglite`): `ensurePersonSignatureSlots` is idempotent; a Meeting Minutes document with a named Chairman and Secretary opens exactly those two person-based slots (labelled `'Chairman'`/`'Secretary'`); a real Secretary-role holder who was *not* the person named on that specific document is correctly rejected from signing it; the actually-named secretary (holding zero relevant role) signs successfully; the document reaches `FINAL` only once both named people have signed; `getMyPendingSignatures` surfaces the person-based slot for a user with no matching role at all; a Resolution with a free-typed (non-system) Chairperson opens no slot for that field and is never blocked by it, while its named Secretary still must sign; and role-based + person-based slots opened on the same document both have to be signed before it finalises.
...
##### 33.7 Capital Goal Calls — "call on shares" (v1.43.0)

**Purpose.** Layered directly on top of the Capital Goals module above rather than replacing it: every goal created from v1.43.0 onward is call-based — instead of a Treasurer simply watching free-form contributions accumulate against a target, each shareholder is offered an equal suggested share of the current month's target and explicitly *pledges* an amount (their own choice: equal, less, more, or zero), which stays pending until a Treasurer approves the actual payment. Approving a pledge payment **is** the act of recording the money's arrival — the same "approval = posting" pattern already used by Requisitions/Fines/Payment Confirmations — and is what actually counts toward the goal's progress and increases the member's shares. A goal's yearly total is split into equal **monthly calls**; shareholders pledge against one month at a time, never against the year total directly.

**Data model.** Three new columns on `capital_goals`: `goal_type` (`PRIMARY`/`SECONDARY`, NULL on any pre-v1.43.0 goal — that NULL is exactly what `isCallBased` checks throughout the codebase), `fiscal_year`, `call_deadline_day` (1–28, so it applies safely to every month including February). A partial unique index enforces **exactly one PRIMARY goal per fiscal year**; any number of SECONDARY goals can run alongside it in the same year, working identically in every other respect. Four new tables: `capital_goal_monthly_calls` (one row per month of a goal's range — period, monthly_target, iteration1_deadline, iteration2_deadline, status: ITERATION_1 / ITERATION_2 / FULFILLED); `capital_goal_pledges` (one row per shareholder's pledge into one monthly call, one iteration — iteration, currency_id, pledged_amount, amount_settled, status: PENDING / PARTIAL / FULFILLED / REJECTED); `capital_goal_pledge_payments` (one row per approved settlement of a pledge — amount, account_id, paid_date, `fine_id` if a late fine was auto-assigned); `capital_goal_payment_applications` (append-only, mirrors `side_fund_payment_applications`' own oldest-first-cascade design, Section 26 — records exactly which period(s) a single iteration-2 payment actually cleared, since one payment can span several backlogged months).

**Baseline & the two-iteration flow.** For each monthly call, the *baseline* is `monthly_target ÷ (number of active shareholders)` — active meaning "holds the Shareholder role," the same eligibility rule `GET /users/shareholders` already uses, not `shares_held > 0`. A shareholder pledges any amount they like: equal to the baseline (happy to contribute their equal share), less (including zero — never penalized), or more. **Iteration 1** runs until `call_deadline_day`. A subtle capping rule governs settlement during iteration 1, re-evaluated **fresh at every single approval**, not decided once in advance: if *every* shareholder's current non-rejected iteration-1 pledge is at or above the baseline, each person's settlement is capped to the baseline — the excess above baseline is only ever offered up in iteration 2. If even one shareholder is below baseline (or hasn't pledged), everyone's pledge settles in full as pledged, uncapped. Because this is evaluated per-approval rather than decided once, it self-corrects as pledges come in in any order, can produce harmless overshoot past 100% of the month's target, and never retroactively undoes a payment already approved.

If the month isn't fully met by the iteration-1 deadline, **iteration 2** opens automatically (scheduled job, see below) and stays open for however many days Settings → Capital Call Fines currently specifies (defaults to 7, v1.48.0). Only shareholders who pledged **above** the baseline in iteration 1 are eligible to pledge again in iteration 2 — the second round exists specifically to let the members who were capped or who over-pledged make good on the remaining balance; a target can end up met at less than 100%, exactly 100%, or more than 100%, and a goal that's never fully met just carries its shortfall forward (below). **No late fine ever applies to an iteration-2 payment**, no matter how late it's actually settled — iteration 2 very often exists specifically to cover *other* members' iteration-1 defaults, so fining it would be punishing the wrong person.

**Carry-forward backlog.** A month's unresolved shortfall doesn't disappear at the iteration-2 deadline — it rolls forward and stacks cumulatively onto every later month's own iteration-2 window until fully covered, however many periods back that takes. An eligible member's iteration-2 combined target is therefore *their own current month's remaining shortfall* **plus** the full accumulated backlog from every prior unresolved month. A single iteration-2 payment can span several backlogged periods at once — allocated **oldest-period-first** (`capital_goal_payment_applications`, the exact cascading pattern `sideFundService.applySideFundPayment` already established for Side Fund, Section 26), with any amount left over after every real shortfall is exhausted landing on the current month.

**Fines.** Only an iteration-1 default is ever fined, and only on the unpaid/late *portion* actually settled late — not the whole pledge. Computed once, at the moment of actual late settlement (no separate escalating background job). The rate/grace period is Admin-configurable (Settings → Capital Call Fines, v1.48.0 — see 33.7.1 below); out of the box it defaults to **5%** if settled within 7 days of `call_deadline_day`, **10%** after that (and the second-round window defaults to 7 days too), matching the original hardcoded values exactly. Reuses the `fines` table's existing `CONTRIBUTION_FAILURE` shape as-is (no schema change) — the fine amount is recorded as ordinary income to the company, tracked completely independently of the member's capital contribution; it never itself counts toward their shares.

**Currency & shares.** A shareholder picks whatever active currency they like when pledging — not locked to the goal's own currency. At approval, the Treasurer must pick a receiving account **in that same currency** (hard-rejected otherwise, no silent conversion — the same "never guess at money" rule used everywhere else in this system); if that currency differs from the goal's own, the amount is converted via `sharePricingService.getExchangeRateOn`/`convertToShareCurrency` at the rate in effect on the payment date. The approved (converted) amount is credited through the exact same `creditShareholderContribution` core (`transactionsController.js`) every other capital-contribution entry point already uses — shares increase by the normal share-price/exchange-rate math, no separate code path.

**Per-goal stats.** `computeGoalContributionStats` (`capitalGoalCallService.js`) gives each shareholder their own running total/percentage toward a goal, plus their biggest and smallest single settled payment, and names the single top contributor across the whole goal — shown with names, deliberately unlike the anonymous grid below, since this is "my own standing plus one public fact," not a pledge-by-pledge breakdown of the group.

**Anonymous status grid.** `GET /monthly-calls/:id/status` is open to every authenticated member and returns only per-shareholder colour-coded cells (`PAID` / `PARTIALLY_PAID` / `PLEDGED` / `DEFAULTED` / `NOT_RESPONDED`) — no names, no amounts, exactly one cell per active shareholder — so the whole group can see where a month stands without anyone's individual pledge being identifiable.

**Scheduled job (`jobs/scheduler.js`).** `scheduleCapitalGoalCallDeadlines` runs daily at 00:30, two independent sweeps: `processIteration1Deadline` (any ITERATION_1 call whose deadline has passed and isn't fully met opens iteration 2, computing the combined target/eligible-member list at that moment) and `processIteration2Deadline` (any ITERATION_2 call whose own 7-day window has passed gets marked FULFILLED or carries its shortfall forward into the next month's backlog, per above).

**API endpoints** (`cms/src/routes/capitalGoals.js`, same prefix, reusing `CAPITAL_GOAL_VIEW`/`CAPITAL_GOAL_MANAGE` — no new permissions):

| Method | Path | Permission | Description |
|---|---|---|---|
| POST | `/monthly-calls/:monthlyCallId/pledges` | any authenticated member | Submit a pledge (iteration defaults to 1) |
| PATCH | `/pledges/:id` | own pledge only | Edit — only while nothing's settled yet |
| POST | `/pledges/:id/reject` | `CAPITAL_GOAL_MANAGE` | Reject — only while nothing's settled |
| POST | `/pledges/:id/approve` | `CAPITAL_GOAL_MANAGE` | Approve = record the payment, issue shares, auto-fine if late |
| GET | `/my-calls` | any authenticated member | My own pledges + which open calls I can still pledge into |
| GET | `/monthly-calls/:id` | `CAPITAL_GOAL_VIEW` | One call's own period/target/deadlines + parent goal info |
| GET | `/monthly-calls/:id/pledges` | `CAPITAL_GOAL_MANAGE` | Every pledge for one call — the approval queue |
| GET | `/monthly-calls/:id/status` | any authenticated member | Anonymous colour-coded grid, no names/amounts |
| GET | `/:id/monthly-calls` | `CAPITAL_GOAL_VIEW` | Every monthly call under one goal |
| GET | `/:id/stats` | `CAPITAL_GOAL_VIEW` | My contribution total/%/biggest/smallest + the top contributor |

**Frontend.** `CapitalGoalsPage.jsx`'s "New Goal" modal now also collects `goal_type`/`fiscal_year`/`call_deadline_day`; a "My Capital Calls" button links to the new personal dashboard. `CapitalGoalDetailPage.jsx` locks target/currency/dates once a goal is call-based (only title/description stay editable — the monthly schedule is already fixed), and gained a "My Contribution to This Goal" + "Top Contributor" card and a Monthly Capital Calls table linking into each call. New **`MyCapitalCallsPage.jsx`** (`/capital-goals/my-calls`) — every open call I can still pledge into (baseline shown, currency picker, submit/edit) plus my full pledge history. New **`CapitalCallDetailPage.jsx`** (`/capital-goals/monthly-calls/:id`) — the anonymous status grid (everyone) plus the Treasurer's approval queue with a currency-matched account picker (Approve/Reject).

**Known issues / open items.**
- No email/bell notification when iteration 2 opens, when a fine is auto-assigned, or as a call's deadline approaches — pull-only for now (visit "My Capital Calls"), same open item already noted for the base Capital Goals module in 33.6.
- `ShareholderDashboard.jsx` doesn't yet surface open calls or outstanding pledges directly — reachable today via the Sidebar → Capital Goals → "My Capital Calls" button.

Migration `migration_v1.43.0.sql`. Verified end-to-end against a real embedded Postgres (`@electric-sql/pglite`) driving the actual shipped controller handlers via `require.cache` substitution — goal + monthly call generation, duplicate-primary-per-year rejection, baseline capping (both branches), iteration-2 eligibility including the zero-eligible edge case, no-fine-ever on iteration 2, 5%/10% fine tiers with exact amounts and real `fines` rows, oldest-first carry-forward cascade across multiple periods with exact per-period application rows, currency-mismatch rejection, cross-currency FX conversion with correct share issuance, and the per-goal contribution stats/leaderboard.

##### 33.7.1 Activation, and Admin-configurable fines (v1.48.0)

**The problem.** A goal created before v1.43.0 — `goal_type IS NULL`, what the codebase calls a "legacy" goal — was always documented to never automatically receive a monthly call schedule (schema.sql's own comment on `capital_goals.goal_type`), but this meant a company's real, live "Capital Drive 2026" goal (created pre-v1.43.0, still `ACTIVE`) had no `capital_goal_monthly_calls` rows at all: `CapitalGoalDetailPage.jsx`'s entire Monthly Capital Calls section is gated on `isCallBased` and simply didn't render, and `MyCapitalCallsPage.jsx` correctly, but unhelpfully, showed nothing open for every single shareholder — reported directly as "the capital calling system isn't active yet as planned... no way for any shareholder to make a pledge... no way for the members to see what is the current month." The "Monthly Breakdown" table on the same page still rendered something, via `computeGoalProgress`'s separate legacy branch (an evenly-split `generate_series` over ordinary `shareholder_contributions`), which is what made the bug easy to miss from the UI alone — it looked populated, just not call-based.

**The fix — opt-in activation, not a silent auto-migration.** New `POST /capital-goals/:id/activate-call-schedule` (`CAPITAL_GOAL_MANAGE`) turns a legacy goal into a call-based one on purpose, preserving its existing history/target/currency: if the goal is still legacy, it requires `goal_type`/`fiscal_year`/`call_deadline_day` in the request body (same validation as creating a new goal, including the one-PRIMARY-per-fiscal-year rule) and sets them; either way, it then calls the existing `generateMonthlyCallsForGoal` to build the full monthly schedule. Immediately afterward, a new **catch-up sweep** (`catchUpMonthlyCalls`) runs the exact same due-deadline logic as the daily cron (`processIteration1Deadline`/`processIteration2Deadline`) synchronously, scoped to just this one goal — without it, a goal activated mid-year would sit with every already-past month still showing `ITERATION_1` (technically "open" but already past its own deadline) until the next 00:30 cron tick; the sweep means whichever period is genuinely current opens (or is correctly mid-iteration-2) right away. Rejects outright if the goal already has any monthly call rows — activation only ever runs once. `CapitalGoalDetailPage.jsx` gained an "Activate Capital Calls" button (only shown when needed) with an explanatory banner and a modal collecting the extra fields for a legacy goal; `CapitalGoalsPage.jsx`'s list shows an amber "Calls not activated" badge next to any `ACTIVE` legacy goal's title. Separately, `MyCapitalCallsPage.jsx`'s empty state was also fixed to distinguish "you've already pledged into everything open" / "nothing you're currently eligible for" / "nothing open right now" instead of one uninformative message for all three.

**Configurable fines.** The four values governing late-payment fines — grace period, the two fine percentages, and the iteration-2 window length — used to be hardcoded module-level constants in `capitalGoalCallService.js` (`ITERATION1_GRACE_DAYS`, `ITERATION2_WINDOW_DAYS`, `FINE_PERCENTAGE_WITHIN_GRACE`, `FINE_PERCENTAGE_AFTER_GRACE`), flagged directly: "I would like... that the fines can be set and not hardcoded in the system." Every other hardcoded-looking fine value in the system turned out to already be admin-entered per instance — `POST /fines`'s `fine_percentage` for a manually-assigned `CONTRIBUTION_FAILURE` fine is typed in by the Treasurer on the Fines page itself, not a constant — so this module's automatic late-capital-call fine was the one genuinely hardcoded case. New single-row `capital_call_fine_settings` table (`grace_days`, `fine_percentage_within_grace`, `fine_percentage_after_grace`, `iteration2_window_days`), the exact same "one editable row, same shape as `savings_settings`" convention already used for the savings interest rate — seeded with the original hardcoded values, so behaviour is completely unchanged until an Admin deliberately edits it. `capitalGoalCallService.getFineSettings(client)` reads the live row on every call (no caching, mirroring how `savings_settings` is read fresh by `savingsController.getSavingsSettings`) — both `approvePledgePayment` (the fine percentage) and `processIteration1Deadline` (the iteration-2 window) now call it instead of touching a constant. New `GET`/`PATCH /capital-goals/fine-settings` (`CAPITAL_GOAL_VIEW` to read — transparency about what a late payment would cost; `CAPITAL_GOAL_MANAGE` to change), and a new "Capital Call Fines" tab in Settings.

Migration `migration_v1.48.0.sql`. Verified end-to-end against `@electric-sql/pglite`: the original activation test suite (legacy-goal detection, required-field validation, correct schedule + catch-up generation, re-activation rejection, `getMyPledges` and `getGoalById` both reflecting the newly-open current month) re-confirmed unaffected by this round's changes; new tests cover the fine-settings default row matching the exact prior hardcoded values, out-of-range validation (a percentage over 100 rejected), an Admin edit persisting and being immediately visible both through the settings endpoint and through `capitalGoalCallService.getFineSettings(client)` called the same way `approvePledgePayment`/`processIteration1Deadline` actually call it, `processIteration1Deadline` computing its iteration-2 deadline using the newly-configured window (not the old hardcoded 7) via the real production code path, and reverting back to the original defaults.
...
#### 36. Fines & Penalties (v1.37.0)

##### 36.1 Purpose

Treasury can assign a fine/penalty to a shareholder — special income to the company, separate from any capital contribution, tracked in the member's outstanding balances and Portfolio. Requested directly: "the treasury can assign to a shareholder a fine that gets listed in their portfolio and dues... can be paid in any account but following the currency of posting... entered by the treasurer directly or through requisition after payment." A fine carries a categorised reason (contribution failure, meeting violation, general) and, for contribution failures specifically, the fine amount is computed automatically from what was defaulted on rather than entered by hand.

##### 36.2 Data model

**`fines`**: id, reference_id, user_id (the fined member), reason (`CONTRIBUTION_FAILURE` / `MEETING_VIOLATION` / `GENERAL`), description, currency_id, amount, default_deadline, defaulted_amount, fine_percentage (the three CONTRIBUTION_FAILURE-only fields — a CHECK constraint enforces they're all present together or all absent, matching the reason), status (`OUTSTANDING` / `PAID`), account_id, transaction_id, paid_date, payment_description, cleared_by, cleared_at, assigned_by, created_at. Widened `requisitions.requisition_type` (new `FINE_PAYMENT` value) and `requisitions.fine_id` (new column, nullable, only set for that type) and `transactions.inflow_type` (new `FINE_PAYMENT_IN` value). New permissions `FINE_VIEW`/`FINE_MANAGE`, starting ungranted for every role including Admin, per this system's standing convention.

##### 36.3 Business rules / key logic

- **Reason-driven form, server-computed amount.** `POST /fines` (`FINE_MANAGE`) requires a `reason` from the fixed three-value list plus a free-text `description`. For `MEETING_VIOLATION`/`GENERAL`, the Treasurer enters `amount` directly. For `CONTRIBUTION_FAILURE`, the Treasurer instead enters `default_deadline`, `defaulted_amount`, and `fine_percentage` — the amount actually stored is computed server-side as `defaulted_amount × (fine_percentage / 100)`, never trusted from the client, so it can never drift from its own inputs (e.g. failing to pay 150 by 15.08.2026 at 8% always posts exactly 12, no matter what a tampered request claims).
- **Currency-matched clearing, no silent conversion.** `finesService.clearFine` — the single shared crediting core, called from both `finesController.clearFineDirect` (Treasurer clears it directly) and a `FINE_PAYMENT` branch in `requisitionsController.approveRequisition` (Treasurer approves a member's own acknowledgement request) — requires the receiving account's `currency_id` to exactly match the fine's own `currency_id`, and hard-rejects with a clear error otherwise. No FX conversion is attempted, the same "never guess at money, hard-stop instead" rule already established for `sharePricingService`'s FX-coverage guard (Section 14) — a fine posted in EUR can be paid into any EUR account, but never automatically converted into a UGX one.
- **Two ways to clear a fine, one core.** (1) Treasurer directly: `PATCH /fines/:id/clear`, only a receiving account, paid date, and description are required — a system-managed "Fines & Penalties" category is auto-provisioned on first use via new `categoryService.getOrCreateCategory` (extracted from `categories.js`'s own path-rebuild logic), so the Treasurer is never prompted to pick or pre-configure one. (2) Member-initiated acknowledgement: a new `FINE_PAYMENT` requisition type, created by the fined member themselves once they've already paid externally, naming exactly which fine it settles via `requisitions.fine_id`. Unlike Savings/Side Fund's acknowledgement types (Section 9), which auto-resolve a single target automatically, Fines needs this explicit reference because a member can hold several simultaneous outstanding fines, possibly in different currencies — there's no safe "oldest first" or generic-credit cascade the way Side Fund dues has. `createRequisition`/`editRequisition` validate the referenced fine exists, belongs to the requester, and is still `OUTSTANDING` before accepting it; `approveRequisition`'s `FINE_PAYMENT` branch requires a Treasurer-chosen `account_id` at approval time (like the plain EXPENSE fallthrough, since — unlike Side Fund/Savings — there's no single canonical account to resolve automatically) and calls `finesService.clearFine` under the hood.
- **Self-service visibility, no permission needed.** `GET /fines/me` is open to any authenticated, role-assigned, consented member (same router-level gate as every other finance route — `authenticate → requireAssignedRole → requireConsent → blockFinanceRestricted`) with no `FINE_VIEW`/`FINE_MANAGE` check — a member always sees their own fines, mirroring `GET /side-fund/dues/me` and `GET /requisitions/me`. Treasury oversight (`GET /fines` — every fine, filterable by status/member) requires `FINE_VIEW`; assigning and clearing require `FINE_MANAGE`.

##### 36.4 API endpoints (`cms/src/routes/fines.js`, prefix `/api/fines`)

| Method | Path | Access | Description |
|---|---|---|---|
| GET | `/me` | Any authenticated, role-assigned, consented member | My own fines, all statuses |
| GET | `/` | `FINE_VIEW` | Every fine, filterable by `status`/`user_id` |
| POST | `/` | `FINE_MANAGE` | Assign a fine to a member |
| PATCH | `/:id/clear` | `FINE_MANAGE` | Clear a fine directly (account, paid date, description) |

A fine can also be cleared indirectly via `POST /requisitions` (`requisition_type: 'FINE_PAYMENT'`, `fine_id`) followed by `POST /requisitions/:id/approve` (`account_id` required) — see Section 9.

##### 36.5 Frontend

- **`FinesPage.jsx`** (`/fines`) — one page, two tabs, same self-service/oversight split as Side Fund (Section 10.5): "My Fines" (everyone, always visible, no permission needed) with a red banner totalling outstanding fines per currency and a "Request Acknowledgement" action on any `OUTSTANDING` row (opens a small modal that creates a `FINE_PAYMENT` requisition scoped to that specific fine); "All Fines" (only shown if `FINE_VIEW` is held) with a "Clear Fine" action on any `OUTSTANDING` row for `FINE_MANAGE` holders. The Assign Fine modal's form is fully conditional on `reason` — picking `CONTRIBUTION_FAILURE` swaps the plain amount field for deadline/defaulted-amount/percentage fields with a live-calculated preview of the resulting fine amount.
- **`RequisitionsPage.jsx`** gained `FINE_PAYMENT` recognition throughout — a "Fine" badge on the relevant rows, correct account-required approval flow (same path as EXPENSE, since Fines needs a Treasurer-chosen currency-matched account, unlike the account-free Contribution/Side Fund acknowledgement types) — though the actual creation of a `FINE_PAYMENT` requisition happens from the Fines page's "Request Acknowledgement" action, not from Requisitions' own generic "Submit Requisition" form (which has no way to pick a specific fine).
- Sidebar nav entry "Fines" (`ExclamationTriangleIcon`), gated the same way as Savings/Side Fund: `hasFinancialAccess('FINE_VIEW') && !isAdminOfficer` — visible to the default financial roles or anyone separately granted `FINE_VIEW`, hidden from purely performative roles (Section 3).
- **Member Portfolio** (`MemberPortfolioPage.jsx`, Section 23.7) gained a "Fines & Penalties" stat tile (outstanding total per currency) and a full fines history table, backed by new `portfolioService.getFinesSection()`.

##### 36.6 Known issues / open items

- The Assign Fine member picker (`AssignFineModal`) lists every active member, not filtered to the Shareholder role specifically — the backend doesn't enforce the fined member must hold Shareholder either (`createFine` only checks the account exists and is active). In practice fines are expected to be assigned to shareholders per the original request, but nothing currently blocks assigning one to any active member.
- No bulk-clear or bulk-assign flow (unlike Side Fund's Bulk Pay Dues, Section 10.3) — each fine is assigned and cleared one at a time.
- Clearing a fine posts a single CREDIT transaction with no reversal path yet — an incorrectly cleared fine currently has no dedicated "undo," the same limitation flagged for several other one-way postings in this system (Section 7).

---

#### 37. Member Deposit Tracking (v1.38.0; activation switch + parent account v1.38.1; reversal fix v1.40.1)

##### 37.1 Purpose

Tracks how much each shareholder has "as deposit" in the company — deliberately NOT a separate pool like the Side Fund. Requested directly: "this amount is held in the account it is parented by unlike the side fund it is not separate therefore its amount is spendable through the parent account. It is only there to track how much each member has as deposit in the company... does not contribute to ones shareholding and each shareholder's deposit amount cannot be zero unless otherwise excused." **v1.38.1 correction:** the initial v1.38.0 build let the account be chosen freely per entry and had no on/off switch — this was flagged directly: "like the side fund, the deposit functionality should be a feature that is turned on or off if the company has this arrangement or not and attached to a specific account." Deposits now follow the Side Fund's exact "optional, activated, single parented account" shape (`is_active`/`parent_account_id`, chosen once in Settings, never per entry) — the one remaining difference from the Side Fund is that this parent account is still NOT a separate envelope (no `current_balance` counter); every deposit stays a completely ordinary, spendable transaction into that one account. Funded two ways — sliced out of a Transactions contribution (alongside the existing Side Fund/Savings slices) or entered as a standalone inflow — and, on exit, refunded with a deduction that depends on how the member is leaving.

##### 37.2 Data model

**`deposit_config`**: singleton (id=1), **is_active** (BOOLEAN, off by default — v1.38.1), **parent_account_id** (the one account every deposit is posted into and refunded from — v1.38.1), target_amount (the single company-wide target every member's own balance is compared against), currency_id (derived automatically from the parent account's own currency, no longer chosen independently), updated_by, updated_at. **`deposit_balances`**: user_id (PK), balance (running per-member total, already normalized into `deposit_config.currency_id`), updated_at. **`deposit_excusals`**: user_id (PK), excused_by, excused_at, reason — presence of a row is the whole signal; a member with a row is excused from the "cannot be zero" expectation. **`deposit_entries`**: id, user_id, source (`CONTRIBUTION_SLICE` / `STANDALONE`), account_id (always the configured parent account), transaction_id, amount/currency_id (as originally posted), normalized_amount/exchange_rate_used (as converted into `deposit_config`'s currency at credit time — in practice almost always rate=1, since the currency is derived from the same parent account), entry_date, recorded_by, created_at, **is_reversed/reversed_at/reversed_by** (v1.40.1 — set when the linked transaction is reversed; the row is kept, never deleted, so history stays complete) — the full audit trail behind the running `deposit_balances` total. **`deposit_exit_events`**: id, user_id, exit_type (`MUTUAL_AGREEMENT` / `FORCED`), deduction_percentage, gross_balance, deduction_amount, net_payout, source_account_id (always the configured parent account, recorded per event for its own audit trail), transaction_id, payment_ack_id, processed_by, processed_at, notes — a CHECK constraint enforces `deduction_percentage = 5` for MUTUAL_AGREEMENT and `>= 50 AND <= 100` for FORCED, exactly matching the brief. Widened `transactions.inflow_type` (`DEPOSIT_CONTRIBUTION_IN`, `DEPOSIT_REFUND_OUT`) and `payment_acknowledgements.source_type` (`DEPOSIT_REFUND`). New permissions `DEPOSIT_VIEW`/`DEPOSIT_MANAGE`. `migration_v1.38.1.sql` adds `is_active`/`parent_account_id` (both `ADD COLUMN IF NOT EXISTS`, idempotent) to the `deposit_config` table shipped in `migration_v1.38.0.sql`.

##### 37.3 Business rules / key logic

- **Not an envelope, deliberately.** Unlike the Side Fund's `current_balance` counter layered on top of a real account, a deposit has no parallel "how much of this account is earmarked" column — the posted transaction is a completely ordinary credit into the configured parent account, fully spendable/commingled from that moment on. `deposit_balances.balance` is purely a tracking number, reconstructable at any time from `deposit_entries`.
- **Optional, activated, single parent account — v1.38.1.** Like the Side Fund, deposits are off by default (`deposit_config.is_active = FALSE`) and only usable once an Admin/Treasurer chooses one `parent_account_id` and flips it on in Settings (`updateDepositConfig`, mirroring `sideFundController.updateSideFundConfig`'s exact validation shape — activating without a parent account throws a clear error). Every deposit, from either entry point, is posted into that one account; the account is never chosen per transaction. The one place Deposits still differs from the Side Fund: there is no "can't change the parent account while it holds a balance" guard, since there's no pooled envelope balance to protect — changing the parent account any time is safe (it just changes where the NEXT deposit lands; past `deposit_entries` rows keep the account they actually posted to).
- **Two entry points, one shared crediting core.** `transactionsController.creditDepositContribution` is called from BOTH `recordContribution`'s optional `deposit_amount` slice AND `depositsController.createStandaloneDeposit` — the same "one core, two entry points" shape as `creditSideFundContribution`/`creditShareholderContribution`. It lives in `transactionsController.js` itself (not a separate service file) specifically so it can call `postTransaction` directly without a circular require — `depositService.js` holds only the non-crediting exit-refund logic, and imports `postTransaction` FROM `transactionsController.js` one-directionally. Neither entry point accepts an account parameter any more (v1.38.1) — `creditDepositContribution` resolves `deposit_config.parent_account_id` internally and throws `'Deposits are not active for this company yet...'` if the feature isn't activated.
- **Normalized at credit time, never re-normalized later.** Because a single company-wide target exists in one currency but the parent account could in principle be changed to a different currency later, `creditDepositContribution` converts each credit into `deposit_config.currency_id` via `sharePricingService.getExchangeRateOn` (date-effective, same "hard stop rather than guess" rule as everywhere else FX is involved) and stores both the original and normalized amounts on the `deposit_entries` row. In practice this is a same-currency, rate=1 no-op almost always, since `currency_id` is now derived automatically from the parent account rather than chosen independently.
- **Does not touch shareholding.** `creditDepositContribution` never calls `recalculateShareholding()` — deposits are explicitly excluded from what counts toward a member's shares, per the brief.
- **Exit refund — deduction depends on exit type, account is automatic (v1.38.1).** `depositService.computeExitRefund`/`processExitRefund` mirror `sideFundController.computeExitPayout`/`removeMember`'s exact shape (a pure computation usable both for a read-only preview and the real processing, a two-leg posting — DEBIT the parent account / CREDIT Savings — followed by a Payment Acknowledgement for two-party sign-off). As of v1.38.1 the debit leg auto-uses `deposit_config.parent_account_id`, exactly like `removeMember` auto-uses `side_fund_config.parent_account_id` — the Treasurer no longer chooses a source account at refund time. The deduction itself is either a fixed 5% (`MUTUAL_AGREEMENT`) or an admin-entered percentage that must be >= 50% (`FORCED`), rather than Side Fund's expense-share formula. `deposit_balances.balance` is always zeroed on exit, even when the net payout works out to zero (a 100% FORCED deduction is allowed).
- **"Cannot be zero unless excused" is monitoring, not enforcement.** Nothing in the schema blocks a balance from reaching zero — the only way it decreases is a deliberate one-time exit refund (or, as of v1.40.1, a reversal — see below). `deposit_excusals` and the `below_target`/`is_excused` flags surfaced on `GET /deposits`, `GET /deposits/me`, and the Member Portfolio are reporting signals for Treasury, not a hard gate on any transaction.
- **Fixed (v1.40.1): reversing a deposit's transaction now correctly rolls back its balance.** Reported directly: "the deposit amount remains unchanged even when a reversal is initiated." `POST /transactions/:id/reverse` (`transactionsController.reverseTransaction`) is a generic, ledger-only reversal — it always posts the opposite-direction ledger entry and adjusts `accounts.current_balance`, but previously only special-cased ONE side table: `shareholder_contributions`, via a dedicated `transactions.contribution_id` column. Deposits have no such linking column (unlike contribution_id) — they're only identifiable via `inflow_type = 'DEPOSIT_CONTRIBUTION_IN'` — so a deposit's `deposit_balances`/`deposit_entries` were silently left untouched by any reversal. `reverseTransaction` now matches on that `inflow_type`, looks up the linked `deposit_entries` row via `transaction_id`, decrements `deposit_balances.balance` by that entry's `normalized_amount`, and marks the entry `is_reversed = TRUE` (with `reversed_at`/`reversed_by`) — the entry row itself is kept, not deleted, so `GET /deposits/me`'s entry history stays complete (shown struck-through with a "Reversed" badge on `DepositsPage.jsx`). Guarded against going negative: if the member's current balance is lower than the entry being reversed (e.g. some of it was already paid out via an exit refund since), the reversal is rejected with a clear error rather than violating the `non_negative_deposit_balance` CHECK. The identical gap (a transaction-side reversal never rolling back a side-table effect) also existed for Side Fund and Savings contributions, which use the same "no linking column, `inflow_type`-only" shape — fixed the same way in v1.41.0, see Section 10.6/11.6.

##### 37.4 API endpoints (`cms/src/routes/deposits.js`, prefix `/api/deposits`)

| Method | Path | Access | Description |
|---|---|---|---|
| GET | `/settings` | Any authenticated, role-assigned, consented member | View activation status, parent account, and target |
| PATCH | `/settings` | `DEPOSIT_MANAGE` | Activate/deactivate, set the parent account, update the target amount |
| GET | `/me` | Any authenticated, role-assigned, consented member | My own balance, target comparison, and entry history |
| GET | `/` | `DEPOSIT_VIEW` | Every shareholder's standing, with below-target/excused flags |
| POST | `/` | `DEPOSIT_MANAGE` | Standalone deposit entry (member, amount, date only — account is automatic) |
| GET | `/excusals` | `DEPOSIT_MANAGE` | List excused members |
| PUT | `/excusals/:userId` | `DEPOSIT_MANAGE` | Excuse a member |
| DELETE | `/excusals/:userId` | `DEPOSIT_MANAGE` | Clear an excusal |
| GET | `/:userId/exit-preview` | `DEPOSIT_MANAGE` | Read-only refund breakdown before confirming |
| PATCH | `/:userId/exit-refund` | `DEPOSIT_MANAGE` | Process the exit refund (account is automatic) |

A deposit can also be funded via `POST /transactions/contributions` with an optional `deposit_amount` field — see Section 2.5/11.3's contribution-slice pattern. This checkbox only appears once deposits are activated.

##### 37.5 Frontend

- **`DepositsPage.jsx`** (`/deposits`) — shows a "Deposits aren't active yet" banner (identical wording pattern to the Side Fund's own inactive-state banner) until an Admin/Treasurer activates it; once active, "My Deposit" (everyone) shows balance, target, standing (on target / below target / excused), and full entry history, and "All Members" (`DEPOSIT_VIEW`) lists every shareholder with a below-target/excused badge and, for `DEPOSIT_MANAGE` holders, Excuse/Clear Excusal and Exit Refund actions. The Settings modal gained an is-active toggle and a parent-account select (mirroring `SideFundPage.jsx`'s own Settings modal exactly); the Record Deposit and Exit Refund modals both dropped their account-choice fields, since the account is now automatic. The Exit Refund modal still fetches the same `computeExitRefund` breakdown the backend uses before confirming, mirroring Side Fund's exit-payout preview modal exactly.
- **`TransactionsPage.jsx`'s `ContributionModal`** gained a third, independent "This contribution includes a deposit portion" checkbox, following the identical checkbox-gated pattern as the existing Side Fund/Savings portions (Section 11.3) — as of v1.38.1 this checkbox is itself gated behind a fetched `depositActive` flag (`depositsAPI.getSettings()`), the same way the side fund checkbox is already gated behind `sideFundActive`.
- Sidebar nav entry "Deposits" (`ArchiveBoxIcon`), gated the same way as Fines/Savings/Side Fund: `hasFinancialAccess('DEPOSIT_VIEW') && !isAdminOfficer`.
- **Member Portfolio** (`MemberPortfolioPage.jsx`, Section 23.7) gained a Deposit Balance stat tile (red when below target) and a full deposit entry history table, backed by new `portfolioService.getDepositSection()`.

##### 37.6 Known issues / open items

- `GET /deposits` only lists members currently holding the Shareholder role — a member who held it in the past (and may still carry a `deposit_balances` row from when they did) drops out of the oversight list once the role is revoked, though their own `GET /deposits/me` still works.
- No bulk excuse/standalone-entry flow (unlike Side Fund's Bulk Pay Dues) — each action is one member at a time.
- The exit refund's Savings credit leg assumes the member already has (or can auto-get) a Savings balance row — consistent with every other payout-to-savings flow in this system, but there's no separate check surfaced if Savings itself has never been set up company-wide.
- **Fixed in v1.41.0** — Side Fund and Savings had the identical reversal gap Deposits had before v1.40.1; both are now covered by their own `reverseTransaction` branches — see Section 10.6 and Section 11.6.
- **Fixed in v1.41.2 — historical reversals from before v1.40.1 never had their deposit balance corrected.** Reported directly: "the amount that is spendable through its parent account ... need to have a way of ammending historical reversals since the current one doesn't change the amount despite the reversal. money leaves the account but the member still holds the figure amount of the reversal." v1.40.1 fixed `reverseTransaction` so a deposit reversal FROM NOW ON correctly decrements `deposit_balances.balance` — it did nothing for a deposit already reversed before that fix existed, so any such member kept showing a deposit balance no longer backed by real money in the parent account (the ledger/account balance itself was always correct — only this per-member tracking figure was stale). Unlike Side Fund/Savings' equivalent backfill (`migration_v1.41.1.sql`), this one has no unrecoverable class of case at all: `deposit_entries` has always been one permanent row per deposit linked to its transaction via `transaction_id`, never a shared/overwritten row the way `side_fund_dues` is, so every historical reversal can be traced to the exact entry and amount involved. `migration_v1.41.2.sql` finds every `DEPOSIT_CONTRIBUTION_IN` transaction marked `is_reversed = TRUE` whose linked `deposit_entries` row is still `is_reversed = FALSE` (i.e. predates the fix), decrements that member's `deposit_balances.balance` by the entry's `normalized_amount`, and marks the entry reversed — guarded the same way a live reversal is (clamped to 0 with a `RAISE NOTICE` if the member's balance is already lower than the entry, e.g. some was already paid out via an exit refund since). Safe to run more than once.

---

#### 38. Payment Confirmations (v1.39.0)

##### 38.1 Purpose

The mirror image of Payment Acknowledgements (Section 35). That feature always creates its row the INSTANT a payment has ALREADY been posted — a post-hoc paper trail the recipient signs off on afterward, never gating the money movement itself. Payment Confirmations is the opposite order, requested directly: "the treasury do a similar thing as fines under acknowledges where an entry to anyone in the system can be made/posted to them in order to acknowledge the money they received defining how they received with means including Cash, Bank transfer, mobile money (MTN/AIRTEL/OTHER) where the former (bank transfer and mobile money) require entry of the transaction ID filled in. Then as soon as the recipient confirms it, it is recorded as a transaction in its respect." Treasury posts an entry FIRST, documenting who was paid, how much, and how — nothing hits the ledger yet. Only once the recipient reviews it and confirms does the real transaction get posted; if they dispute it instead, no transaction is ever created and Treasury cancels the entry and reissues a corrected one. It replaces Service Fees' old instant-post payment flow (Section 28) — a service fee payment now goes through this same pending-until-confirmed flow rather than posting the moment the Treasurer records it.

##### 38.2 Data model

**`payment_confirmations`**: id, reference_id, source_type (`GENERAL_PAYMENT` / `SERVICE_FEE_PAYMENT` — a CHECK list, deliberately easy to extend later, same shape as `payment_acknowledgements.source_type`), source_id (NULL for `GENERAL_PAYMENT`; `service_fee_agreements.id` for `SERVICE_FEE_PAYMENT`), account_id (which account will be debited once confirmed), category_id, payer_id (who recorded the entry — always Treasury), recipient_id (who must confirm), amount, currency_id (derived from `account_id`), payment_method (`CASH` / `BANK_TRANSFER` / `MOBILE_MONEY`), mobile_money_provider (`MTN` / `AIRTEL` / `OTHER`, required only when payment_method is Mobile Money), external_reference (the transaction ID — required for Bank Transfer/Mobile Money, forbidden for Cash; both rules enforced by CHECK constraints, not just app-level validation), purpose, entry_date, status (`PENDING_CONFIRMATION` / `CONFIRMED` / `DISPUTED` / `CANCELLED`), transaction_id (NULL until CONFIRMED), confirmation_note, confirmed_at, dispute_reason, disputed_at, cancellation_reason, cancelled_by, cancelled_at, created_at. Widened `transactions.inflow_type` (new `GENERAL_PAYMENT_OUT` value — `SERVICE_FEE_OUT` already existed and is reused for confirmed `SERVICE_FEE_PAYMENT` entries). No new permissions — reuses `PAYMENT_ACK_VIEW`/`PAYMENT_ACK_MANAGE` since this lives inside the same Payment Acknowledgements page as a second tab.

##### 38.3 Business rules / key logic

- **Confirmation creates the transaction — the opposite of Payment Acknowledgements.** `paymentConfirmationsController.confirmPayment` is the only place `postTransaction()` is ever called for this feature — nothing is posted at creation time. This deliberately inverts `payment_acknowledgements`' own documented invariant ("auto-creates one row the instant a payment actually pays out — never created by hand"): here, the row IS created by hand, first, and the payment only actually happens once the recipient acts.
- **Payment method validation, enforced twice.** `validatePaymentMethod()` throws a clear error before the database ever sees a mismatched combination (Mobile Money without a provider, Cash with a transaction ID, Bank Transfer/Mobile Money without one) — and the same rules are also enforced as CHECK constraints on the table itself, so a bad combination can never be inserted even by a future code path that forgets to call the validator.
- **Two source types, one shared insert core.** `createPaymentConfirmationRow()` (internal, not a route) is called from both `createPaymentConfirmation` (the real `POST /` route, Treasury pays anyone ad hoc, `GENERAL_PAYMENT`) and `createServiceFeePaymentConfirmation()` (an internal helper called from `serviceFeesController.recordPayment`, `SERVICE_FEE_PAYMENT` — recipient/account/category/currency are all derived from the agreement, never re-entered). Same "one core, multiple entry points" shape used throughout this codebase (Side Fund/Savings/Deposit crediting, Payment Acknowledgements' own `createPaymentAcknowledgement`).
- **Confirming a service fee payment also inserts its `service_fee_payments` row — deferred, not skipped.** Before v1.39.0, `recordPayment` inserted that row immediately alongside the transaction. Now it's inserted inside `confirmPayment` at the moment the transaction actually posts, keeping `service_fee_payments` an accurate record of payments that actually happened (a pending, unconfirmed, or disputed entry never appears in an agreement's payment history) rather than payments Treasury merely claimed were made.
- **Two references per completed flow, mirroring Payment Acknowledgements.** The confirmation row itself gets its own reference at creation time (module `PACK`, category `GEN`/`SVC`, record type `PAYMENT_CONFIRMATION`); the eventual transaction gets a completely separate reference at confirm time (the account's own module code, record type `TRANSACTION`) — the same "one reference for the paper trail, one for the money movement" split `payment_acknowledgements` already established.
- **No "reopen" step, unlike Payment Acknowledgements.** A disputed Payment Acknowledgement can be reopened back to PENDING_ACK because the underlying payment already happened — reopening just asks the recipient to look again. Here, nothing has happened yet for a disputed entry, so there's nothing to "look again" at with the same data; Treasury's only remedy is `POST /:id/cancel` (from PENDING_CONFIRMATION or DISPUTED, `PAYMENT_ACK_MANAGE`) followed by recording a fresh, corrected entry. A CONFIRMED entry can never be cancelled here — it has already posted a real transaction, out of scope for this feature the same way no other module in this system offers a one-click ledger reversal (Section 7).
- **Self-service access is not permission-gated**, mirroring Payment Acknowledgements exactly — any authenticated, role-assigned, consented user (Administrative Officer included) can view/confirm/dispute their own entries; only creating a `GENERAL_PAYMENT` entry, viewing everyone's, and cancelling require `PAYMENT_ACK_MANAGE`/`PAYMENT_ACK_VIEW`.

##### 38.4 API endpoints (`cms/src/routes/paymentConfirmations.js`, prefix `/api/payment-confirmations`)

| Method | Path | Access | Description |
|---|---|---|---|
| POST | `/` | `PAYMENT_ACK_MANAGE` | Record a general ad hoc payment entry (PENDING_CONFIRMATION) |
| GET | `/my` | Any authenticated user | My own entries (all statuses) |
| POST | `/:id/confirm` | Recipient only | Confirm receipt — posts the real transaction (PENDING_CONFIRMATION → CONFIRMED) |
| POST | `/:id/dispute` | Recipient only | Flag an issue, reason required (PENDING_CONFIRMATION → DISPUTED), no transaction posted |
| GET | `/` | `PAYMENT_ACK_VIEW` | Every entry, filterable by status/source_type |
| POST | `/:id/cancel` | `PAYMENT_ACK_MANAGE` | Cancel a not-yet-confirmed entry |
| GET | `/:id` | Recipient, payer, or `PAYMENT_ACK_VIEW` | Full detail |

A service fee payment is created via the existing `POST /service-fees/agreements/:id/pay` (Section 28.4), now additionally requiring `payment_method` (and `mobile_money_provider`/`external_reference` where applicable) in the request body — see Section 28.3's updated flow.

##### 38.5 Frontend

- **`PaymentAcknowledgementsPage.jsx`** gained a third tab, "Payment Confirmations", alongside the existing "My Acknowledgements"/"All (Treasury)" tabs — everyone sees their own entries awaiting confirmation (with Confirm/Dispute actions) and full history; `PAYMENT_ACK_VIEW` holders additionally see every entry in the system with status filter chips and a Cancel action; `PAYMENT_ACK_MANAGE` holders get a "New Payment" button opening `CreatePaymentConfirmationModal` (recipient, account, category, amount, date, payment method with the Cash/Bank Transfer/Mobile Money toggle, conditional provider/transaction ID fields, purpose).
- **`ServiceFeesPage.jsx`'s `RecordPaymentModal`** gained the same payment-method fields (Cash/Bank Transfer/Mobile Money toggle, conditional provider/transaction ID) and now shows an amber notice that the entry is pending until the fee recipient confirms it, instead of confirming a balance was moved immediately.
- No new sidebar nav entry — this lives entirely inside the existing "Payment Acknowledgements" page.

##### 38.6 Known issues / open items

- `GENERAL_PAYMENT`'s recipient picker (`CreatePaymentConfirmationModal`) lists every active user system-wide, not filtered to any particular role — appropriate since "anyone in the system" was the explicit brief, but worth knowing if a future refinement wants to narrow it.
- No automated reminder job for an entry sitting in PENDING_CONFIRMATION for a long time, same limitation already flagged for Payment Acknowledgements (Section 35.6) — a one-time notification only, relying on the recipient to act on it from their bell.
- A disputed entry has no structured "resolution" record beyond the dispute_reason text itself and the eventual cancel action — same limitation as Payment Acknowledgements' own disputes (Section 35.6).

---

#### 39. Bonds & Investments Overhaul (v1.40.0; face value repayment, frequency rescheduling, and settlement auto-funding v1.42.0; schedule-collapse bug actually fixed v1.42.1)

##### 39.1 Purpose

Six related fixes/additions to the Investments module (Section 8), all requested together: bonds bought at a discount or premium to face value weren't representable; a coupon that actually paid out differently from the scheduled amount had no way to be corrected; a bond's first coupon date sometimes isn't known until after settlement and couldn't be changed once set; money spent on an investment's operational transactions wasn't counted as "Spent" even though it left the funding account; spend beyond the planned budget had no tracking; and there was no way to formally wind an investment down mid-term with a proper sign-off and closing report. None of these are new modules — they're corrections and extensions to the existing `investments`/`bond_coupons`/`investment_transactions` tables and `investmentsController.js`.

##### 39.2 Data model — new columns (`migration_v1.40.0.sql`)

**`investments`** gained:
- `settlement_value` (NUMERIC, nullable) — the actual price paid for a BOND when it differs from `face_value` (bought at a discount or premium). Purely informational — the coupon schedule's gross/tax/net math is always computed on `face_value`, never on `settlement_value`. `settlement_percentage` (`settlement_value / face_value × 100`) and `settlement_discount_amount` (`face_value − settlement_value`, positive = discount, negative = premium) are derived at read time in `getInvestmentById`/never stored.
- `supplementary_budget` (NUMERIC, default 0) — auto-tracked running total of spend that pushed `actual_expenditure` past `planned_budget`. Never set directly — see 39.4.
- `status_before_termination`, `termination_requested_by/at`, `termination_reason`, `records_confirmed_by/at`, `termination_approved_by/at`, `termination_report` — the mid-term termination workflow's state (see 39.6).
- `status` CHECK widened to add `PENDING_TERMINATION` and `TERMINATED`, alongside the original `PENDING/ACTIVE/ON_HOLD/COMPLETED/CANCELLED`.

**`bond_coupons`** gained `actual_gross_amount`, `actual_tax_amount`, `actual_net_amount` (all nullable — NULL means paid exactly as scheduled), plus `adjusted_by`/`adjusted_at`. The original `gross_amount`/`tax_amount`/`net_amount` columns are **never overwritten** — they stay the original forecast forever, so the schedule always shows expected vs. actual side by side.

##### 39.3 Bond coupon actual-payment override + date gating

`payBondCoupon` (`PATCH /:id/coupons/:couponId/pay`) now takes an optional `actual_gross_amount`. When supplied: `taxAmount = round2(actualGross × tax_withholding_rate / 100)`, `netAmount = actualGross − taxAmount` — recalculated from the bond's own withholding rate, not copied from the scheduled coupon. The `investment_returns` row posts the **actual** gross (not the scheduled one), and `bond_coupons.actual_gross_amount/actual_tax_amount/actual_net_amount/adjusted_by/adjusted_at` are set; every other coupon in the schedule is untouched. Without `actual_gross_amount`, behavior is unchanged from before v1.40.0 — the scheduled amounts post as-is.

**Date gating (both paths):** a coupon can only be marked paid on or after its own `due_date` — `payBondCoupon` compares `coupon.due_date` against today (server-side, in addition to the frontend hiding both buttons for a not-yet-due coupon) and throws `badRequest` if it's still in the future. This exists specifically so a future payment can never be approved early, per the explicit requirement.

Frontend (`BondScheduleCard` in `InvestmentDetailPage.jsx`): each PENDING coupon shows "Mark Paid" and "Record Actual Payment" side by side once due (`c.due_date <= today`), or "Not due yet" before then. "Record Actual Payment" opens `RecordActualCouponPaymentModal`, which live-previews the auto-calculated tax/net as the gross amount is typed. A PAID coupon whose `actual_gross_amount` is set shows an amber "Actual: X gross, Y net" line under its Paid badge.

##### 39.4 Settlement value / discount-premium display

Accepted as an optional `settlement_value` field on both `POST /investments` and `PATCH /investments/:id` (BOND only). `getInvestmentById`/`getAllInvestments` compute `settlement_percentage` and `settlement_discount_amount` via `CASE` expressions alongside the existing `roi_percentage` pattern — no new stored aggregate. `BondScheduleCard` shows a blue info panel ("Settlement Value: X (Y% of face value) — bought at a discount/premium of Z / bought at par") whenever `settlement_value` is set, directly under the coupon summary tiles, explicitly noting coupon payments stay on the full face value. `CreateInvestmentModal` (`InvestmentsPage.jsx`) gained the matching input field.

##### 39.5 Spent-tracking fix + supplementary budget

**The bug:** `recordInvestmentTransaction` (`POST /:id/transactions`, the "Record Operational Transaction" button) posted its EXPENSE/TAX entries to the general ledger via `postTransaction` and inserted into `investment_transactions`, but never touched `investments.actual_expenditure` — so money that genuinely left the funding account for the investment's sake was invisible in the investment's own "Spent" figure. The same gap existed in `payBondCoupon`'s own tax-withholding leg (a `investment_transactions` TAX insert done inline, not through `recordInvestmentTransaction`).

**The fix:** both places now also update `actual_expenditure` — EXPENSE and TAX entries only; INFLOW is income, not spend, and is deliberately left out (it's already visible via `operating_budget.total_income`). This merges capital funding and operational spend into a single "Spent" total, per explicit choice during design (rather than tracking them as two separate figures).

**Supplementary budget:** every time `actual_expenditure` grows (`fundInvestment`, `recordInvestmentTransaction`'s EXPENSE/TAX branch, and `payBondCoupon`'s tax branch), a shared helper `computeSupplementaryOverage(plannedBudget, currentExpenditure, amount)` works out how much of *that specific increase* falls beyond `planned_budget` and adds it to `supplementary_budget` — correctly handling a single funding call that's partly within budget and partly beyond it, and whether the investment was already over budget going in. No separate manual "add supplementary budget" action exists — it's detected automatically, per the explicit design choice made during clarification. Shown as a 5th stat tile on `InvestmentDetailPage.jsx`'s header (amber when > 0).

##### 39.6 First coupon date AND frequency rescheduling (frequency added v1.42.0)

`PATCH /investments/:id/coupon-schedule` (`updateCouponSchedule`, `INVESTMENT_MANAGE`) — sets or corrects `first_coupon_date` **and/or `coupon_frequency`** on an already-ACTIVE bond and regenerates the **entire** coupon schedule from whichever changed (or both) via the same `generateBondCouponSchedule()` used at creation time, so every later coupon date recalculates automatically. Either field can be omitted to keep its current value, but at least one must be supplied. Only usable while **no coupon has been paid or marked missed yet** (`SELECT COUNT(*) WHERE status != 'PENDING'` must be 0 — widened in v1.42.1 to also cover a coupon marked `MISSED` by the termination workflow, Section 39.8, which is a real historical record too and shouldn't be silently erasable by a reschedule) — once a coupon is settled one way or another, the schedule is locked (both fields, same single guard); this is a narrower, purpose-built endpoint rather than loosening `editInvestment`'s existing PENDING-only guard, since that function's bond-schedule regeneration does an unconditional `DELETE FROM bond_coupons` that would be unsafe post-approval. Frontend: an "Edit Coupon Date / Frequency" link next to the coupon rate badge on `BondScheduleCard` (hidden once any coupon has been paid), opening a modal with both a date picker and a frequency dropdown.

**v1.42.0 — first (incomplete) attempt at fixing the schedule collapsing to one row.** Reported directly: "the date recalculation failed when i set the first coupon date and now only shows one date of the final coupon payment and amount." The diagnosis at the time was that the periodic-coupon loop (`while (dueDate < maturity)` in `generateBondCouponSchedule()`) never runs when its anchor date is on/after maturity, leaving only the always-forced final coupon — so v1.42.0 added validation rejecting that case with a clear error. **This was real but insufficient — see v1.42.1 below, which found and fixed the actual root cause and superseded this diagnosis entirely.**

**v1.42.1 — found and fixed the ACTUAL root cause; the v1.42.0 fix never worked.** The user reported the exact same symptom again immediately after v1.42.0 shipped, regardless of what first coupon date or frequency was entered — which meant the v1.42.0 diagnosis was wrong. The real cause: `generateBondCouponSchedule()` builds its dates with `new Date(`${someDate}T00:00:00Z`)`, which assumes `someDate` is a plain `"YYYY-MM-DD"` string. `updateCouponSchedule` (and `editInvestment`) always pass the investment's own `start_date`/`expected_end_date` straight from a `SELECT * FROM investments` row — and node-postgres returns a `DATE` column as a native JS `Date` object, not a string. The template literal silently stringifies that `Date` object (e.g. `"Wed Jun 18 2043 02:00:00 GMT+0200 (...)"`), which `new Date(...)` cannot parse, producing an `Invalid Date`. Every comparison against an `Invalid Date` evaluates to `false` — including the periodic loop's `while (dueDate < maturity)` (zero iterations, so only the forced final coupon survives) **and** the v1.42.0 anchor-validation guard itself (`anchor >= maturity` is also `false` against an `Invalid Date`, so it never fired). This is why the v1.42.0 fix looked correct on paper but changed nothing in practice — the exact failure it was meant to catch was silently masked before it ever reached that check. Fixed properly this time: `bondSchedule.js` now normalizes `issueDate`/`maturityDate`/`firstCouponDate` (accepting a `Date` object, a bare date string, or a full ISO timestamp string) into a plain date string immediately, before any comparison, and throws a clear error if a date still can't be parsed after that — turning any future date-shape problem into a loud failure instead of a silent one-coupon schedule. The v1.42.0 anchor validation itself was legitimate and is kept; it just needed dates it could actually compare. **Retroactively fixable after all** — unlike the earlier (incorrect) assumption that a collapsed schedule couldn't be recovered because the original first coupon date was unknown, the real bug never touched what was *stored*; `investments.first_coupon_date`/`coupon_frequency` always held the value actually entered; it only broke *regenerating the schedule from it*. `backfill_v1.42.1_bond_schedules.js` (run once, after deploying — no schema change, so no migration file this time) regenerates every BOND's schedule from its own currently-stored fields using the now-fixed generator: a no-op for any bond that was already correct, a full recovery for any bond that had collapsed, and it also happens to fix an `AT_MATURITY`-frequency bond that was silently given `NaN` amounts by the exact same underlying bug (a different symptom, identical cause) — deliberately skips any bond with a coupon already `PAID` or `MISSED`, same boundary the live endpoint itself enforces. Verified against a real embedded Postgres (`@electric-sql/pglite`) driving the actual `updateCouponSchedule` handler end-to-end with a `Date`-object investment row (reproducing the bug exactly), confirming a 33-coupon semi-annual schedule now generates correctly, the anchor-too-late guard still correctly rejects genuinely bad input, and the widened PAID/MISSED lock holds.

##### 39.7 Profit/loss flag

`performance_status` — `NOT_YET_FUNDED` / `PROFITABLE` / `LOSING` / `BREAK_EVEN` — derived via a `CASE` on `total_returns` vs. `actual_expenditure`, identical inputs to the existing `roi_percentage` calculation, computed fresh on every read in both `getAllInvestments` and `getInvestmentById` (no new stored column, same reasoning as `roi_percentage` itself). Shown as a small label under the ROI% figure on the investment header.

##### 39.8 Mid-term termination workflow

Three steps, each its own endpoint, deliberately **not** reachable via the generic `PATCH /:id/status` (that endpoint's controller explicitly rejects `PENDING_TERMINATION`/`TERMINATED` even though its own route validator already excludes them — defense in depth against the workflow being bypassed):

1. **Request** — `POST /:id/terminate/request` (`INVESTMENT_MANAGE`, body: `reason`). Only from `ACTIVE`/`ON_HOLD`. Snapshots the current status into `status_before_termination`, sets `status='PENDING_TERMINATION'`, records who/when/why, notifies the responsible person by bell + email.
2. **Confirm records** — `POST /:id/terminate/confirm-records`. Only the investment's own `responsible_user_id` can confirm (or, if none is set, anyone holding `INVESTMENT_APPROVE`, so the workflow never gets stuck on a missing responsible person) that every return, expense, and transaction against the investment is up to date. **Returns and operational transactions can still be recorded while `PENDING_TERMINATION`** (`MUTABLE_INVESTMENT_STATUSES = ['ACTIVE', 'PENDING_TERMINATION']`, checked in `recordInvestmentTransaction` and `payBondCoupon`; `recordReturn` never had a status gate to begin with) — specifically so anything missing can be caught up before this step. Capital funding (`fundInvestment`) stays `ACTIVE`-only; injecting new capital into an investment that's being wound down doesn't make sense.
3. **Approve & close** — `POST /:id/terminate/approve` (`INVESTMENT_APPROVE`, same permission as initial investment approval; optional `closing_note`). Requires step 2 done first. Sets `status='TERMINATED'`, `actual_end_date=CURRENT_DATE`, and writes `termination_report` — a plain-text summary stating the reason, planned budget, total spent (flagging how much was supplementary), total returns, and the **result: a profit or loss of `|total_returns − actual_expenditure|`**. Any coupon still `PENDING` at this point is flipped to `MISSED` so the schedule reads accurately on a closed bond.

**Reject** — `POST /:id/terminate/reject` (`INVESTMENT_MANAGE` or `INVESTMENT_APPROVE`, optional `reason`) — usable any time while `PENDING_TERMINATION`, restores `status` to exactly `status_before_termination` and clears every termination_* field, so an abandoned request leaves no trace beyond the audit log.

No new permissions — reuses `INVESTMENT_MANAGE` (request/reject) and `INVESTMENT_APPROVE` (final approve, and confirm-records' no-responsible-person fallback), the same minimal-footprint reasoning already used throughout this module.

Frontend (`InvestmentDetailPage.jsx`): an amber "Termination under review" card while `PENDING_TERMINATION` (reason, requested-by, records-confirmed status, and the Confirm/Approve/Reject actions gated to exactly who the backend would accept); a "Termination Report" card once `TERMINATED`, showing the generated report text and who closed it. `RequestTerminationModal`/`ApproveTerminationModal`/`RejectTerminationModal` handle the three actions; a "Terminate Investment" link appears whenever the investment is eligible.

##### 39.10 Face value repaid alongside the final coupon (v1.42.0)

Reported directly: "generally all bond investments payout the facevalue amount together with the last coupon wasn't included before which it should." Before v1.42.0, `payBondCoupon` only ever credited a coupon's interest — the bond's principal (`face_value`) was never paid back to the funding account at maturity, in any code path. Fixed: when the coupon being paid is the **final** one for that investment (`coupon_number = MAX(coupon_number)` for that `investment_id` — checked by number, not by date, so it stays correct even if the schedule was later rescheduled per 39.6), `payBondCoupon` now ALSO posts a second CREDIT into `returns_account_id` for the full `face_value`, recorded as its own `investment_returns` row with a new `return_type = 'PRINCIPAL'` (widened CHECK, alongside the existing `DIVIDEND/PROFIT_SHARE/CAPITAL_GAIN/INTEREST/RENTAL/OTHER`). Always credited at face value, never `settlement_value` — a bond returns its full principal at maturity regardless of what was paid to acquire it; the discount/premium (39.4) only ever affected the acquisition cost.

**Deliberately excluded from `investments.total_returns`.** That figure drives `roi_percentage`/`performance_status` (39.7) — folding the face value in would make a matured bond's ROI look wildly and wrongly inflated the moment it matures, since getting your own principal back isn't profit. The `PRINCIPAL` return row is still fully visible in the investment's Returns history for a complete audit trail; it just doesn't feed the profit/loss math.

**Auto-completion.** A bond that's just had its final coupon AND its face value repaid has nothing left to do — if it was `ACTIVE` at the moment of that payment, it's automatically flipped to `COMPLETED` (`actual_end_date` set to the payment date). Deliberately does NOT apply if the investment is `PENDING_TERMINATION` — that workflow (39.8) owns the final status in that case, not a coupon payment. `payBondCoupon`'s response now includes `is_final_coupon`, `principal_repaid`, and `completed_at_maturity` so the frontend can show all of this clearly rather than it looking like an ordinary coupon payment.

**Retroactive backfill.** Since this is a pure CREDIT (money coming in), it can never cause a negative balance, so it was safe to apply automatically to previously created investments — see 39.13.

##### 39.11 Settlement value auto-funded on approval (v1.42.0)

Reported directly: "I would like that once the bond is approved the settlement value is recorded and deducted from the holding account automatically (no negatives allowed of course) that the investment is activated already but the update later rule stands if the settlement value is not known at first entry / creation." Before v1.42.0, `approveInvestment` only ever flipped `status` from `PENDING` to `ACTIVE` — it never touched money at all; funding always required a separate, fully manual `POST /:id/fund` call for whatever amount the Treasurer typed in.

**On approval:** if the bond's `settlement_value` is already known (set at creation, or edited in while still `PENDING` via `editInvestment`), `approveInvestment` now automatically funds the investment for that exact amount in the same transaction as the status flip — reusing the exact same funding logic as a manual `fundInvestment` call (new shared internal helper `postInvestmentFunding()`, extracted out of `fundInvestment` so both call sites share one code path rather than drifting apart). Guarded by `postTransaction`'s own existing negative-balance/floor-limit check: if the funding account can't cover it, the **entire approval fails and rolls back** — the investment stays `PENDING`, nothing is partially funded, nothing is activated. This is what "no negatives allowed" means in practice — the check isn't a soft warning, it blocks the approval outright.

**"Update later" — new endpoint for post-approval.** If `settlement_value` genuinely isn't known yet at approval time, the investment still activates (approval is never blocked on it), exactly per the stated rule. Since `editInvestment` only works while `PENDING`, a brand new endpoint, `PATCH /investments/:id/settlement-value` (`setSettlementValue`, `INVESTMENT_MANAGE`), lets it be filled in once the bond is `ACTIVE` — which immediately triggers the same guarded auto-funding. **Locked once `actual_expenditure > 0`** (funded by ANY means — the auto-funding above, this endpoint, or even a plain manual `fundInvestment` call) — the same "can't rewrite money that's already moved" principle used for the coupon schedule locking after the first coupon is paid (39.6). Frontend: an amber "Settlement value wasn't known when this bond was approved..." banner with a "Record Settlement Value" button on `BondScheduleCard`, shown only while `status = 'ACTIVE'` and `actual_expenditure = 0`.

##### 39.12 Known issues / open items

- The termination report is plain text, not a structured/printable document like `paymentAcknowledgementTemplate()` or `receiptTemplate()` — displayed on the page as-is, no dedicated print layout yet.
- `confirmTerminationRecords`'s no-responsible-person fallback lets **any** `INVESTMENT_APPROVE` holder confirm records, not a specifically-designated stand-in — acceptable since it only fires when the investment genuinely has nobody assigned, but worth revisiting if that turns out to happen often.
- Supplementary budget is tracked only at the investment level, not per-project, mirroring how `actual_expenditure` itself already isn't split across projects for a multi-project investment beyond the explicit `project_id` on `fundInvestment`.
- The first-coupon-date schedule-collapse bug (39.6) — v1.42.0's fix didn't actually work (the check it added was itself silently bypassed by the same underlying bug); v1.42.1 found the real cause and fixed it properly, and it turned out to be fully retroactively recoverable after all via `backfill_v1.42.1_bond_schedules.js`, not just fixed going forward as first assumed.

##### 39.13 Retroactive backfill for previously created bonds (v1.42.0)

Reported directly: "I would like that these changes affects even previously created investments." A one-time script, `cms/backfill_v1.42.0_bonds.js`, run the same way as `run_migration.js` (`cd cms && node backfill_v1.42.0_bonds.js` — same `.env` connection settings), does two things safely and idempotently:

1. **Face value repayment (39.10)** for any BOND whose final coupon was already marked `PAID` before this version existed and has no `PRINCIPAL` return recorded yet — credits face value now, exactly as `payBondCoupon` would for a coupon paid today, and auto-completes the investment if it's still `ACTIVE`. Always safe (a pure credit).
2. **Settlement auto-funding (39.11)** for any `ACTIVE` BOND that already has a `settlement_value` recorded but has never been funded by any means (`actual_expenditure` still exactly 0) — funds it now, the same guarded way approval would have. A bond whose account genuinely can't afford it today is skipped and reported, not forced through.

Deliberately does **not** touch any investment that already has `actual_expenditure > 0` — that money already moved via the old fully-manual funding flow (the only option before this version existed), and re-funding it here would double-charge the account. Safe to run more than once: a second run finds nothing left to do in either category (both scripts' candidate queries naturally exclude anything already corrected). Does not attempt the first-coupon-date backfill — see 39.12's note on why that one needs a human. Verified against a real embedded Postgres (`@electric-sql/pglite`) driving the actual production `payBondCoupon`/`approveInvestment`/`setSettlementValue` handlers directly (not just the SQL in isolation), including the negative-balance atomic-rollback case and re-running twice for idempotency.
...
- **Backend silently restarting roughly every 16 minutes** (Sections 5.9, 22.6) — fixed in **1.35.2**. The global rate limiter (200 req/IP/15min, meant for brute-force/DDoS protection) also applied to `/health`, so Render's own liveness-probe traffic to that endpoint eventually crossed the limit and started getting `429`. Render reads a failing health check as "this instance is unresponsive" and restarts it — a clean, error-free restart from the app's own perspective, since nothing was actually wrong, which is exactly why it was invisible in the app's own logs and initially suspected to be an out-of-memory kill (Render's Metrics → Memory graph ruled that out: flat around 20%, no sawtooth). Fixed with a `skip: (req) => req.path === '/health'` on the global limiter — `/health` is an infrastructure probe, not user traffic, and should never be capable of tripping a DDoS guard. This directly caused v1.35.1's forced-logout symptom too: any page load or token refresh landing in the ~1-2 second restart window got treated as a real auth failure until that was separately fixed.
- **A role could never be re-assigned to someone who'd previously held, then lost, that exact role** (Section 23.6) — fixed in **1.35.3**. `user_roles` had a table-wide `UNIQUE(user_id, role_id)` since the original schema; the app's own "already holds this role?" check only looks at active rows, so it let the attempt through, and the INSERT then collided with the constraint anyway because a revoked row for that pairing still counted. Surfaced via the generic "A record with this value already exists" message. Fixed by replacing the constraint with a partial unique index scoped to active rows only (`WHERE revoked_at IS NULL`), via `migration_v1.35.3.sql`.
- **Several read-only financial endpoints, and the Dashboard/TopBar that call them, were reachable by every role including "performative" ones (Secretary, Assistant Secretary, Coordinator)** (Section 3) — fixed in **1.36.0**. `GET /accounts/summary` and 7 similar endpoints checked only `authenticate`, on a historical "any real member" assumption that predates the current financial-vs-performative role policy; the default Dashboard and TopBar balance strip rendered the results with no client-side permission check either. Fixed with a new `requireFinancialAccess`/`FINANCIAL_ROLES` allow-list (Treasurer, Assistant Treasurer, Shareholder, Director, Admin by default, or an explicit permission grant) applied server-side to all 8 endpoints and mirrored client-side (`hasFinancialAccess`) to gate what the Dashboard/TopBar/Sidebar even fetch or render. See Section 3's "Financial vs. performative roles" writeup for the full mechanics.
- **Transfer Approve button shown without a role-specific client-side check** (Section 4.3.6) — fixed in **1.36.0**, same release as the above. Any holder of `FINANCE_TRANSFER_APPROVE` saw the Approve button even when the backend would reject their specific approval (Primary→Secondary needs Treasurer, Secondary→Primary needs Director) — a real instance of "buttons shouldn't appear if a person doesn't hold that role," not just a UX rough edge. `TransfersPage.jsx`'s new `canApprove(row)` mirrors `transfersController.approveTransfer`'s exact role check per transfer type. Reject was deliberately left on the broader `FINANCE_TRANSFER_APPROVE` check, matching `rejectTransfer`'s own (type-agnostic) backend rule.
...
| 1.49.0 | **Dark-mode chart contrast/modernization, system-wide (Section 2.4 addendum).** Reported directly, with screenshots: charts looked poor in dark mode — washed-out axis labels, a stray bright rectangle around bar/line charts, and a stark white tooltip box popping out of an otherwise dark card. Root cause: v1.27.0's dark mode is entirely CSS-driven (`darkMode: 'media'` + `dark:` Tailwind classes), but every chart in the system is drawn by recharts as plain SVG with colors passed as JS prop values (`stroke="#f0f0f0"`, an unstyled `<Tooltip>`) — there is no `dark:` class equivalent for an SVG `stroke` attribute or a `contentStyle` object, so those values were simply never re-picked for dark mode at all; CartesianGrid's very light `#f0f0f0` grid lines (invisible-by-design on a white card) became a glaring white box on a dark one, unstyled axis tick text kept its default medium-gray (unreadable on dark), and Tooltip fell back to its default plain-white background. New `useChartTheme()` hook (`cms-frontend/src/hooks/useChartTheme.js`) — the app's first JS-side dark-mode read, via a live `matchMedia('(prefers-color-scheme: dark)')` listener — returns a resolved, mode-appropriate palette (`primary`/`success`/`danger`/`warning`/`accent`/`neutral`, each shifted a shade brighter in dark mode for contrast, e.g. green-600 → emerald-400) plus ready-to-spread prop bundles: `gridProps` (theme-correct stroke, horizontal-only lines for a cleaner look), `axisTick`, `tooltipProps` (themed background/border/text matching the card, replacing the plain-white default), and `legendProps`. Applied to every recharts instance in the system — `InvestmentDetailPage.jsx` (Budget Usage donut, Returns Over Time, Budget vs Spent by Project), `AccountsPage.jsx` (Balance Over Time area, Flow Breakdown donut), `MmfDetailPage.jsx` (Balance Over Time line, Interest vs Fees bars), `LoanDetailPage.jsx` (Principal Repaid donut, Repayments Over Time, Outstanding Balance line), `CapitalGoalDetailPage.jsx` (Expected vs Actual cumulative line) — replacing every hardcoded hex color and unstyled Tooltip/Legend with the shared theme. Pie/donut slices also gained a card-colored stroke between segments (a cleaner "cut-out" look than a flat abutting fill) and small padding-angle gaps, a modest modernization alongside the contrast fix itself. Code-only, no schema migration, no new permissions. Verified via `eslint`/Babel parse checks on every changed file (zero errors); a full production build was attempted but exceeded the sandbox's command timeout — not run to completion, so a final visual check in a real browser is recommended before relying on this in production. |
| 1.51.1 | **Fixed: Requisitions returned a 422 "Validation failed" on nearly every submission that left an optional date blank (Section 21).** Reported directly from local backend warning logs (`POST /api/requisitions` 422s repeating every submission). Root cause: express-validator's plain `.optional()` only skips validation when a field is `undefined` — an empty string is still "present" — so `required_by_date`/`contribution_date`/`fine_id` sent as `''` (the normal case whenever those fields aren't used, e.g. every `EXPENSE` requisition with no "Required By" date entered) reached `isISO8601()`/`isInt()` and failed. Fixed with `optional({ checkFalsy: true })` on all three fields, on both `POST /requisitions` and `PATCH /requisitions/:id`; `RequisitionsPage.jsx` also now coerces `required_by_date` to `undefined` when blank, matching how `contribution_date` already worked. Code-only, no schema migration. |
| 1.51.0 | **Dashboard current-quarter widget; capital goals: mid-year effective-date adoption, historical read-only accounting, admin on/off toggle, direct contributions auto-attributed.** Requested directly, two parts, with an explicit invitation for clarifying questions — a research subagent gathered full context first, then four clarifying questions were asked via structured multiple-choice, all answered with the recommended option: a direct (non-pledge) contribution auto-settles the contributor's own open pledge first, topping up anonymously if there's no pledge or money left over; a historical (pre-effective-date) month is a read-only aggregate — no fake pledge/payment rows, marked `CLOSED` immediately; a direct contribution that happens to settle a pledge LATE is fined under the exact same rules as a manual approval, no exemption; and the tracking toggle is a full pause (hides the UI, stops the cron, blocks every write endpoint) rather than a partial one. **Current quarter widget**: new `GET /settings/fiscal-quarters/current` (any authenticated user) reuses `quarterAnalyticsService`'s fiscal-match-with-calendar-fallback `quarterForDate` logic (rather than the bare `fiscalService.getQuarterForDate`, which has no fallback) — both `DashboardPage.jsx` and `ShareholderDashboard.jsx` now show it inline next to today's date. **Effective date of adoption**: new nullable `capital_goals.effective_from` (must be the 1st of a month; NULL means "same as `start_date`," zero behavior change for every pre-v1.51.0 goal) lets a PRIMARY (or SECONDARY) goal be created or activated mid-year without penalizing months that already happened — `generateMonthlyCallsForGoal` now generates any period before `effective_from`'s own period already `CLOSED`, skipping iteration entirely. That historical period's "collected" figure (`getPeriodCollected`/`getHistoricalPeriodCollected`) is computed on the fly from real `shareholder_contributions` already recorded that month, converted into the goal's own currency the same way a live payment tranche already is (best-effort — an unconvertible currency pair is silently skipped, never thrown on) — never a fake pledge/payment row. "No penalty system applies" to a historical month's own shortfall: `getCarryForwardBacklog` now excludes every historical period entirely, so it never becomes backlog live-period shareholders are asked to make up via iteration 2. `computeGoalContributionStats`/`CapitalGoalDetailPage.jsx`'s monthly breakdown and `capitalGoalCallsController`'s monthly-call list/detail endpoints all fold the historical aggregate in alongside live figures, visually flagged "Historical" in the UI (read-only, no pledge link). Both `CapitalGoalsPage.jsx`'s create form and `CapitalGoalDetailPage.jsx`'s Activate Capital Calls modal gained an optional "Effective Date of Adoption" date picker. **Primary goal no longer compulsory**: nothing in the system ever required a PRIMARY goal to exist — confirmed and left as-is; the toggle below is the actual mechanism for "capital goals are optional." **Capital Goal Tracking on/off toggle**: new singleton `capital_goal_settings` (`tracking_enabled`, default `TRUE` — zero behavior change until an Admin deliberately flips it). Off is a full pause: `Sidebar.jsx` hides the Capital Goals nav item, both dashboards hide their capital-goal widgets, `scheduler.js`'s daily deadline sweep returns immediately without touching anything, and every write endpoint (`createGoal`/`updateGoal`/`cancelGoal`/`completeGoal`/`activateCallSchedule`/`submitPledge`/`editPledge`/`rejectPledge`/`approvePledgePayment`) rejects with a clear "Capital Goal Tracking is currently turned off" error — reads and all existing data are completely untouched either way. New `GET`/`PATCH /capital-goals/settings/tracking` (view open to `CAPITAL_GOAL_VIEW`, edit to `CAPITAL_GOAL_MANAGE`); the toggle itself lives at the top of Settings' renamed "Capital Goals" tab (previously "Capital Call Fines"). **Direct contributions count toward the PRIMARY goal**: any ordinary (non-pledge) capital contribution — via Transactions → Record Contribution, or a `CONTRIBUTION_ACKNOWLEDGEMENT` requisition being approved — now also attributes toward that shareholder's own PRIMARY-goal monthly call for its own live period, via new `attributeDirectContributionToPrimaryGoal` (`capitalGoalCallService.js`), called from a lazy `require()` inside `creditShareholderContribution` (a top-level require would have created a circular dependency, since `capitalGoalCallService.js` already requires `transactionsController.js` for the reverse direction) and wrapped in try/catch so a capital-goal bookkeeping hiccup can never fail or roll back an otherwise ordinary, already-recorded contribution. It finds-or-creates-and-settles the contributor's own iteration-1 pledge for that month (growing an existing pledge or creating a system-generated one sized to the payment if there's a shortfall or none at all), through a newly extracted `settleApprovedContributionIntoPledge` core shared with the Treasurer's manual `approvePledgePayment` — same lateness/fine logic either way (a late direct contribution is fined identically to a late manual approval) — but deliberately skipping the baseline-cap rule, which exists to keep PLEDGE promises fair while other members haven't paid yet, not to throttle money that's already unconditionally in the bank. A capital-call settlement itself (`approvePledgePayment`) passes a new `skipCapitalGoalAutoAttribution: true` to `creditShareholderContribution` to avoid double-attributing the same money. Silently does nothing when tracking is off, the date falls in a historical period, or there's no ACTIVE call-based PRIMARY goal covering that date. Migration `migration_v1.51.0.sql`. Verified end-to-end against `@electric-sql/pglite` driving the real shipped controller/service handlers (31 assertions, all passing): mid-year `effective_from` generates the correct CLOSED/live split; the historical aggregate matches real contribution totals; carry-forward backlog correctly excludes historical periods; the tracking toggle blocks `submitPledge`/`createGoal` while off and the same flag scheduler.js checks is confirmed disabled/re-enabled; a direct contribution with no existing pledge creates and fully settles a system-generated one; a direct contribution against an existing open pledge settles into it rather than duplicating; a historical-period direct contribution creates no pledge at all while still updating the read-only aggregate; a late direct-contribution settlement is correctly flagged late and fined; and the calendar-quarter fallback and a configured fiscal quarter's precedence both resolve correctly. Frontend changes verified via `eslint` (zero errors, six pre-existing unrelated warnings) on every touched file; a full production build was not attempted (same sandbox command-timeout constraint noted in v1.49.0/v1.50.0) — a final visual check in a real browser is recommended before relying on this in production. |
| 1.50.0 | **Accounts market-history charts; Transactions/Transfers analytics, quarter summaries and CSV export; Dashboard current capital call widget.** Requested directly, in four parts, with an explicit instruction to ask clarifying questions before implementing — four questions were asked via structured multiple-choice and all four were answered with the recommended option: keep currencies strictly separate everywhere (never sum/convert across currencies in any chart, summary, or export); resolve "quarter" per-date against Settings → Fiscal Quarters, falling back to an ordinary calendar quarter for any date a fiscal quarter doesn't cover; a single CSV export button per page, exporting every row matching the currently-applied filters (clearing filters first yields the full ledger); and the Dashboard's capital call widget shows only the current open call belonging to the year's one PRIMARY capital goal, not every open call across every active goal. New shared `quarterAnalyticsService.js` (`loadFiscalQuarters`/`quarterForDate`/`bucketAndSummarize`) is the one engine both Transactions' and Transfers' analytics endpoints call, so the fiscal/calendar quarter logic exists in exactly one place. New dependency-free `utils/csv.js` (RFC4180-style escaping, UTF-8 BOM for Excel) backs both exports — no new npm dependency. **Accounts page**: `AccountsPage.jsx` gained a `MarketHistoryCharts` section (share price over time via `sharesAPI.getHistory`, one exchange-rate line per currency pair via `exchangeRatesAPI.getHistory`) rendered above the account list, hidden entirely if there's under 2 points to trend. **Transactions page**: new `GET /transactions/analytics` (Income vs Expense by currency, plus most/least income and expense quarter, honoring the same filters as the ledger) and `GET /transactions/export` (full filtered CSV, capped at 20,000 rows) — both factored through a new shared `buildTransactionFilters` extracted from the existing `getTransactions` handler so filtering can never drift between the ledger, the analytics, and the export. `TransactionsPage.jsx` renders the new `IncomeExpenseCharts` section above the Filters card and adds an "Export CSV" button alongside the pre-existing "Print" (ledger PDF) action. **Transfers page**: `buildTransferFilters` gained `account_id` (matching either leg) and a `from_date`/`to_date` range, additive to the pre-existing `status`/`transfer_type` filters; new `GET /transfers/analytics` (deliberately restricted to `status = 'POSTED'` only — the sole status where money has actually, finally moved, so a "largest transfer" or a volume trend is never built from a transfer that never happened) returns per-currency monthly volume + most/least-active quarter, an exchange-rate-over-time series per currency pair, total bank charges by currency, and the single largest/smallest completed transfer per currency; `GET /transfers/export` is NOT restricted to POSTED — it's a ledger-style export showing every status, matching how the Transactions export also shows every status. `TransfersPage.jsx` gained a Filters card (previously had none), the new `TransferAnalyticsCharts` section, and an "Export CSV" button. **Dashboard**: both `DashboardPage.jsx` (staff-facing) and `ShareholderDashboard.jsx` (shown to shareholder-only users, who are exactly who this matters most to) gained a `CurrentCapitalCallCard` widget — the PRIMARY capital goal's currently-open monthly call, if any, sourced from the existing per-user `GET /capital-goals/my-calls` endpoint (no new backend route; that endpoint was already open to any authenticated member, not gated behind `CAPITAL_GOAL_VIEW`) and linking straight to `/capital-goals/my-calls`, the actual pledging page. Renders nothing if there's no PRIMARY goal or it has no call currently open, which also covers un-activated legacy goals automatically. **Bug found and fixed during verification**: `getTransferAnalytics`'s exchange-rate-trend and largest/smallest date fields, and both CSV exports' `Date` column, built their date string via `String(value_date).slice(0, 10)` — but a `DATE` column comes back from the database as a native JS `Date` object, not a string, so `String(aDateObject)` produces `"Mon Jan 05 2026 00:00:00 GMT..."` rather than `"2026-01-05"`; slicing that wrong string to 10 characters silently produced garbage like `"Mon Jan 05"` in both the CSV downloads and the exchange-rate chart's x-axis. Fixed by routing all four sites through the existing `normalizeDateInput()` helper (`utils/dateUtils.js`), the same helper `quarterAnalyticsService.js` already used correctly for this exact reason. No schema migration — code-only. Verified end-to-end against `@electric-sql/pglite`: `quarterForDate`'s fiscal-match-with-per-date-calendar-fallback behavior directly; `bucketAndSummarize`'s currency separation and most/least-quarter picks; the Transactions analytics and CSV endpoints against real inserted multi-currency rows (unfiltered and `account_id`-filtered); the Transfers analytics endpoint's POSTED-only restriction (an `AWAITING_APPROVAL` transfer correctly excluded from analytics but correctly present in the CSV export), its either-leg `account_id` filter, its date-range filter, its largest/smallest and total-charges figures against hand-computed expected values, and — after the fix above — that every date appearing in analytics output and CSV rows is a proper `YYYY-MM-DD` string rather than a stringified `Date` object; the pre-existing `getTransfers` ledger endpoint confirmed still working with the new filters layered on. Frontend changes verified via `eslint` (zero errors/warnings) on every touched file; a full production build was not attempted (same sandbox command-timeout constraint noted in v1.49.0) — a final visual check in a real browser is recommended before relying on this in production. |
| 1.48.0 | **Capital Call activation for legacy goals, and Admin-configurable capital call fines (Section 33.7.1).** Reported directly, with screenshots of the live system: "the capital calling system isn't active yet as planned. There is no way for any shareholder to make a pledge... no way for the members to see what is the current month... I would like that the capital system starts functioning immediately and that the fines can be set and not hardcoded." Root cause: the live "Capital Drive 2026" goal predates v1.43.0 (`goal_type IS NULL`, a "legacy" goal) — by original, documented design a legacy goal never automatically gets a monthly call schedule, so every symptom (no Monthly Capital Calls section, an empty My Capital Calls page for every member) traced to the same cause, not a bug in the pledge/scheduler pipeline itself (confirmed correct via a clean PGlite repro on a goal created the normal way). New `POST /capital-goals/:id/activate-call-schedule` (`CAPITAL_GOAL_MANAGE`) opt-in-activates a legacy goal (collecting `goal_type`/`fiscal_year`/`call_deadline_day` if still unset) or regenerates a call-based goal's missing schedule, then immediately runs a new `catchUpMonthlyCalls` sweep — the same due-deadline logic as the daily cron, scoped to just this goal — so already-elapsed months open (or correctly reach iteration 2) right away instead of waiting for the next cron tick; rejects if the goal already has a schedule. `CapitalGoalDetailPage.jsx` gained an "Activate Capital Calls" button + explanatory banner; `CapitalGoalsPage.jsx`'s list flags any un-activated `ACTIVE` legacy goal with an amber badge; `MyCapitalCallsPage.jsx`'s empty state now distinguishes "already pledged into everything open" from "nothing eligible" from "nothing open." Separately: the four values governing automatic late-capital-call fines (grace days, the two fine percentages, the iteration-2 window) were hardcoded constants in `capitalGoalCallService.js` — every other fine value in the system turned out to already be admin-entered per instance, so this was the one real case. New single-row `capital_call_fine_settings` table (same convention as `savings_settings`), seeded with the exact prior hardcoded values; `getFineSettings(client)` reads it live (no caching) from both `approvePledgePayment` and `processIteration1Deadline`. New `GET`/`PATCH /capital-goals/fine-settings` (view open to `CAPITAL_GOAL_VIEW`, edit to `CAPITAL_GOAL_MANAGE`) and a new "Capital Call Fines" tab in Settings. Migration `migration_v1.48.0.sql`. Verified end-to-end against `@electric-sql/pglite`: the full pre-existing activation test suite re-confirmed passing unchanged; new tests cover the fine-settings default row, out-of-range rejection, an Admin edit persisting and being immediately visible through both the settings endpoint and the exact `getFineSettings(client)` call site `approvePledgePayment`/`processIteration1Deadline` use, `processIteration1Deadline` computing its iteration-2 deadline off the newly-configured window through the real production code path, and reverting to the original defaults. |
| 1.47.0 | **Full monitoring visibility for Treasurers/Admins across Acknowledgements, Fines, Requisitions, Service Fees; a Service Fee agreement detail page (Sections 21, 35, 38).** Requested directly: Treasurers/Admins should be able to see all pending transactions under Acknowledgements, Fines, Requisitions, and Service Fee payments for every member — even without rights to approve beyond their own scope, just to monitor whether something has been actioned — plus a detail page under each Service Fee agreement showing its payment history over time, with any adjustment to the monthly amount showing its effective change date and full change history. Of the four modules named, only Requisitions actually had a bug: `RequisitionsPage.jsx`'s "All Requisitions" tab was gated on a combined `canApprove` flag requiring BOTH `FINANCE_TRANSACTION_CREATE` and `FINANCE_VIEW_ALL`, when the backend's own `GET /requisitions` route only ever required `FINANCE_VIEW_ALL` — so an Admin/Treasurer holding only the view permission never saw the tab populate. Fixed with a new, separate `canViewAll` flag (`FINANCE_VIEW_ALL` alone) gating the tab/data-load, while the existing `canApprove` flag is untouched and still gates the Approve/Reject buttons exactly as before — no new approval rights, per the clarifying answer that this should guarantee monitoring visibility only. Payment Acknowledgements/Confirmations and Fines were already correctly gated on a single view permission with no such bug. Service Fees was the real gap — the only one of the five "pending transaction" modules still gated by hardcoded `requireRoles(['Admin','Treasurer','Assistant Treasurer'])` instead of the permission system every other module uses. New `SERVICE_FEE_VIEW`/`SERVICE_FEE_MANAGE` permissions replace every `requireRoles` gate in `routes/serviceFees.js` (VIEW for list/read endpoints, MANAGE for create/edit/pay/terminate/reimbursement review) and `ServiceFeesPage.jsx`'s `hasRole(...)` checks — split into `canView*`/`canManage*` pairs so the Agreements/Reimbursements tabs are visible to anyone holding VIEW while the actual action buttons still require MANAGE. New `service_fee_agreement_amendments` table (mirrors `loan_received_rate_amendments`): `updateAgreement` now requires a `reason` and `effective_from` whenever `monthly_amount` is actually changing, and records the change as an append-only row rather than silently overwriting the figure; `getAgreementById` now also returns this history alongside `payments`. New `ServiceFeeAgreementDetailPage.jsx` (`/service-fees/agreements/:id`, wired from clickable rows in `ServiceFeesPage.jsx`'s Agreements table, mirroring how Loans' list links to `LoanDetailPage.jsx`) relocates Record Payment/Amend/Terminate onto the single agreement's own page (same actions, same permission gates, nothing new) and shows the payment history and the new amendment history together, plus a Download Summary button using a new `serviceFeeAgreementTemplate` (`exportUtils.js`, mirrors `loanTemplate`). Migration `migration_v1.47.0.sql`. Verified end-to-end against `@electric-sql/pglite` driving the real shipped `updateAgreement`/`getAgreementById` handlers: a notes-only update records no amendment; an amount change without a reason/effective date is rejected; an amount change with both is recorded correctly (previous/new amount, reason, effective date) and reflected in the updated agreement and in `getAgreementById`'s returned history; resending the same amount a second time does not create a duplicate amendment row; the migration re-applies cleanly on top of an already-migrated schema. |
| 1.46.0 | **Four fixes requested together: remove documents from the archive, stop Portfolio Summary from saving a document, PDF + brief summary on monthly report emails, and a mandatory Preview step before Generate (Sections 16.6, 17.3, 21.3).** (1) New `deleteDocument` (`DELETE /documents/:id`, `DOCUMENT_DELETE` permission) soft-removes a document already in the Company Archive (either regular-lifecycle `ARCHIVED`, or a direct archive upload that never went through that step) — a new `DELETED` status value, excluded from every list view the same way `SUPERSEDED` already was; never a real row-delete, since `documents.id` is referenced by several enforced foreign keys (grants, loans, staff grants, audit engagement documents, report log) plus two unenforced polymorphic links (`document_signatures`, `document_stamps_applied`) that a hard delete would silently orphan. (2) `MemberPortfolioPage.jsx`'s redundant "Generate Portfolio Summary" button (which called `documentsAPI.generate()`, persisting a document every time someone printed their own portfolio) was removed entirely — the existing "Print" action, which was already purely client-side, is now the only action on the page. (3) `sendGeneralReportToAllMembers`/`sendIndividualReportsToAllMembers` (`reportService.js`) now render the full report to a PDF (new shared `pdfService.renderHtmlToPdfBuffer`, factored out of `certificateService.js`'s existing Puppeteer code) and attach it, with the email body shrunk to a short bullet-point summary instead of the entire report dumped inline — share-certificate emails already worked this way and needed no change. (4) `GenerateDocumentPage.jsx`'s single Generate step split into Preview (local render only, no backend call, safe to click repeatedly while adjusting fields) and a separate Confirm & Save to Library button that disables itself once it succeeds — the fix for clicking Generate twice creating the same document twice, and, since a preview is now shown before anything is saved, also directly delivers "let the system show a preview... in case they may have to change something." No new schema beyond the widened `documents.status` CHECK — `migration_v1.46.0.sql`. Verified end-to-end against `@electric-sql/pglite`: delete-eligibility for both archive paths, double-delete rejection, and correct exclusion from both the general and Company-Archive-filtered document lists; report emails carry exactly one PDF attachment each with a summary-only body, the General report's PDF rendered once total and the Personal report's once per member. |
| 1.45.0 | **Person-specific signatories from template attendee fields (Section 29.9).** Requested directly: on the Meeting Minutes/Agenda/Resolution templates, the Chairperson/Secretary/attendee fields should be fillable from a dropdown of system users, or a typed free-text name for someone not in the system; picking a real system user as Chairman or Secretary should mean that specific person's signature is recorded once they approve the document. `document_signatures.required_role_id` is now nullable, with a new `required_user_id` + `position_title` pair covering the person-based case (a slot naming one specific person, regardless of role) alongside the existing role-based case — a CHECK constraint keeps exactly one of the two set, and two partial unique indexes replace the old composite `UNIQUE` constraint. New `ensurePersonSignatureSlots()` (idempotent, mirrors `ensureSignatureSlots`); `signSlot` now matches a caller by either their held roles or their own user ID; `getSignatureStatus`/`getMyPendingSignatures`/`notifyPendingSignatories` all extended to surface person-based slots the same way role-based ones already were. Confirmed via clarifying questions before building: the named person still has to actively sign it themselves (nothing auto-signs); naming them makes them the required signer for that one document even without holding the matching role; applies to Meeting Minutes, Meeting Agenda, and Resolutions; a free-typed (non-system) name is printed-only and never blocks finalisation. `documentsController.approveDocument` reads `chairperson_user_id`/`secretary_user_id` out of the document's own `template_data` and opens person-based slots alongside any separately-configured role-based ones — both must be satisfied before the document reaches `FINAL`. Frontend: the Chairperson/Secretary fields on all three templates became a new `PersonPicker` (dropdown of active users, with a toggle to type a free name instead); Attendees/Members Present became a repeatable `PersonListField` of the same picker (cosmetic only, no signature implications). `exportUtils.js` templates read the new shape via `personName()`/`personListNames()` helpers, both backward-compatible with documents generated before this version. Verified end-to-end against `@electric-sql/pglite` driving the real shipped handlers: idempotent slot creation; a real Secretary-role holder who wasn't the person named on a specific document is correctly rejected while the actually-named person (holding no relevant role) signs successfully; the document only finalises once every named person has signed; a free-typed Chairperson never creates a slot or blocks finalisation; role-based and person-based slots on the same document both have to clear before it's `FINAL`. Migration `migration_v1.45.0.sql`. |
| 1.44.0 | **Signature system: role-wide access, visibility gating, unified pending-signature view (Section 29.8).** Requested directly against the existing v1.23.0 Digital Consent & Multi-Signatory Approval module: signatures should appear on every approved document per the roles a member holds; with several directors, any of them can sign and their own name/signature shows (this part was already true — a slot belongs to the role, not a person); every document needing a signature, including share certificates, should surface in one place to sign from; a document can't reach the general Documents view until every signatory has approved; the signer's name/date auto-fill (already true); redrawing a signature must never alter an already-signed document (already true — signing snapshots the file). What was actually fixed: (1) `POST /documents/:id/sign` no longer requires the `DOCUMENT_APPROVE` permission on top of being a configured signatory role — a role added in Settings → Signatories can now always sign, matching how certificate-round signing already worked; (2) `GET /certificates/rounds/:id` was hard-restricted to Treasurer/Assistant Treasurer/Admin, meaning a Director-only (or any other role) signatory was notified but had no page that would load — now also open to a current pending signatory of that specific round, while the full round list stays management-only; (3) new `GET /documents/pending-signatures` (`signatureService.getMyPendingSignatures`) and a new "Pending My Signature" tab on the Documents page — the first place documents and certificate rounds needing your signature appear together, with a badge count, reusing a generalized `SignaturesModal`; (4) `getAllDocuments` now hides a `DRAFT` document with any open signature slot from everyone except its creator, an Admin, or a current pending signatory — it reappears for everyone the instant the last required signature lands; (5) `SIGNABLE_DOCUMENT_TYPES` widened from 4 to all 14 document types (mirroring the already-wider `STAMPABLE_DOCUMENT_TYPES`), now a single source of truth in `signatureService.js` rather than a separately-drifting copy in `settingsController.js` — purely opt-in, nothing changes for a type until an Admin configures required roles for it. No new tables — a widened `signature_requirements.document_type` CHECK constraint (`migration_v1.44.0.sql`) plus route/controller changes. Verified end-to-end against `@electric-sql/pglite` driving the real shipped handlers: a newly-eligible type enforces correctly; any of two directors can sign with zero permissions granted, recording that specific director; the second director is correctly rejected once the slot's filled; visibility hiding/reveal works for creator/Admin/pending-signatory/outsider; `getMyPendingSignatures` tracks both documents and rounds correctly; a Director-only round signatory can now view and sign where an outsider still can't. |
| 1.43.0 | **Capital Goal Calls — "call on shares" (Section 33.7).** Layered on top of Capital Goals (Section 33): every new goal is now either the year's one PRIMARY (general) goal or an alongside SECONDARY goal, split into equal monthly calls that shareholders pledge against (equal to, less than, more than, or zero of an equal suggested baseline share — never penalized for pledging low or zero) rather than a Treasurer just watching free-form contributions land. Approving a pledge payment records the money's arrival, credits it through the same `creditShareholderContribution` core as every other contribution path, and increases shares by the normal share-price/exchange-rate math. A month not fully met by its deadline opens a second iteration for 7 more days, open only to members who pledged above the baseline in round one, with an unresolved shortfall carrying forward cumulatively (oldest-first cascade, mirroring Side Fund's own payment-application pattern) until fully covered — no fine ever applies to a second-iteration settlement. A first-iteration default is fined 5% (within 7 days of the deadline) or 10% (after) on the unpaid portion only, recorded independently of the member's capital contribution via the existing Fines module. Pledges can be made in any currency, converted at approval time; the receiving account must match the pledge's own currency. New anonymous, colour-coded status grid (no names/amounts) lets every member see where a month stands; a separate per-goal stats view shows each member their own running total/percentage/biggest/smallest contribution and names the single top contributor. New tables `capital_goal_monthly_calls`/`capital_goal_pledges`/`capital_goal_pledge_payments`/`capital_goal_payment_applications`; new `capital_goals.goal_type`/`fiscal_year`/`call_deadline_day` plus a partial unique index enforcing one PRIMARY goal per fiscal year. New pages `MyCapitalCallsPage.jsx` (`/capital-goals/my-calls`) and `CapitalCallDetailPage.jsx` (`/capital-goals/monthly-calls/:id`); `CapitalGoalsPage.jsx`/`CapitalGoalDetailPage.jsx` extended accordingly. No new permissions — reuses `CAPITAL_GOAL_VIEW`/`CAPITAL_GOAL_MANAGE`. New daily scheduled job `scheduleCapitalGoalCallDeadlines` (iteration transitions). Verified end-to-end against `@electric-sql/pglite` driving the real shipped controller handlers, including baseline capping, iteration-2 eligibility, fine tiers, multi-period carry-forward with exact application rows, currency-mismatch rejection, and cross-currency FX share issuance. Migration `migration_v1.43.0.sql`. |
| 1.42.1 | **Fixed the REAL cause of the bond coupon schedule collapsing to one row — v1.42.0's fix never actually worked (Section 39.6).** Reported again immediately after v1.42.0 shipped, regardless of what date/frequency was entered — proving the earlier diagnosis was wrong. Actual cause: `generateBondCouponSchedule()` parses dates via `new Date(`${d}T00:00:00Z`)`, which assumes a plain string; `updateCouponSchedule`/`editInvestment` always pass the investment's own `start_date`/`expected_end_date` straight from a DB row, and node-postgres returns `DATE` columns as JS `Date` objects — the template literal silently stringifies the object into something unparseable, producing an `Invalid Date`. Every comparison against an `Invalid Date` is `false`, so the periodic loop never ran AND v1.42.0's own anchor-validation guard never fired either (masked by the same bug it was meant to catch) — which is why that fix changed nothing. Fixed by normalizing every date input (`Date` object, bare string, or ISO timestamp) to a plain date string up front, plus a loud error if a date still can't be parsed. Also widened the reschedule lock from `status='PAID'` to `status != 'PENDING'`, so a coupon marked `MISSED` by the termination workflow can't be silently erased by a reschedule either. Unlike the earlier (incorrect) assumption that this couldn't be fixed retroactively, the stored `first_coupon_date`/`coupon_frequency` values were never actually wrong — only regenerating from them was broken — so `backfill_v1.42.1_bond_schedules.js` (no schema change, no migration file) safely regenerates every affected bond's schedule from its own stored fields: a no-op where already correct, a full recovery where collapsed, and it also fixes an `AT_MATURITY` bond that was silently getting `NaN` amounts from the identical root cause. Verified against `@electric-sql/pglite` driving the real `updateCouponSchedule` handler with a genuine `Date`-object investment row, reproducing the exact bug and confirming the fix, the anchor guard, and the widened lock all now work correctly. |
| 1.42.0 | **Bond investment fixes: schedule-collapse bug, face value repayment, frequency rescheduling, settlement auto-funding (Sections 39.6, 39.10–39.13).** Reported directly, four issues together plus a retroactive-application request: "the date recalculation failed when i set the first coupon date and now only shows one date of the final coupon payment and amount. Plus generally all bond investments payout the facevalue amount together with the last coupon wasn't included before which it should. I would also like to be able to change the frequency as well after creation ... I would like that once the bond is approved the settlement value is recorded and deducted from the holding account automatically (no negatives allowed of course) ... I would like that these changes affects even previously created investments." (1) Root-caused the schedule collapse to `generateBondCouponSchedule()`'s periodic-coupon loop never executing when its anchor date (the first coupon date, or its default) lands on or after maturity — only the always-forced final coupon survived, silently wiping every other row; fixed by validating the anchor is strictly before maturity and rejecting it otherwise with a clear error (not retroactively fixable — see 39.6/39.12). (2) `payBondCoupon` now also repays the bond's full face value alongside its final coupon (by `coupon_number`, robust to rescheduling), recorded as a new `investment_returns.return_type = 'PRINCIPAL'` row that is deliberately excluded from `investments.total_returns` (would wrongly spike ROI on maturity) and auto-completes the investment if still ACTIVE. (3) `PATCH /:id/coupon-schedule` (previously first-coupon-date only) now also accepts `coupon_frequency`; both fields lock together once the first coupon is actually paid, same as before. (4) `approveInvestment` now auto-funds a bond's known `settlement_value` in the same transaction as approval, via a new shared `postInvestmentFunding()` helper (also now used by `fundInvestment`) — guarded by the same negative-balance check as any other posting, rolling back the entire approval (investment stays PENDING) if the funding account can't cover it; a new `PATCH /:id/settlement-value` (`setSettlementValue`) lets it be recorded later if unknown at approval time, activating funding immediately, and locks once funded by any means (`actual_expenditure > 0`). (5) `backfill_v1.42.0_bonds.js` (run once, after the migration, the same way as `run_migration.js`) retroactively applies (2) and (4) to previously created investments — crediting face value for any bond whose final coupon was already paid, and auto-funding any already-approved bond with a recorded-but-never-funded settlement value — deliberately skipping anything already funded by the old manual flow, safe to re-run. Verified end-to-end against a real embedded Postgres (`@electric-sql/pglite`) driving the actual production route handlers via `require.cache` substitution, including the atomic negative-balance rollback and idempotency on a second backfill run. Widened `investment_returns.return_type` CHECK (adds `PRINCIPAL`). Migration `migration_v1.42.0.sql`, then `node backfill_v1.42.0_bonds.js`. |
| 1.41.2 | **Reconcile historical Deposit balances for reversals from before v1.40.1 (Section 37.6).** Reported directly right after v1.41.1 shipped: "i was first focused on the deposit amount (security) ... this amount need to have a way of ammending historical reversals since the current one doesn't change the amount despite the reversal. money leaves the account but the member still holds the figure amount of the reversal." v1.40.1 only corrected `deposit_balances.balance` for reversals happening from then on; any deposit reversed before that fix existed still left the member's tracked balance too high, out of sync with the parent account's real money. Since `deposit_entries` has always been one permanent, non-overwritten row per deposit (unlike Side Fund's shared/cascading `side_fund_dues`), this backfill is fully recoverable with no unrecoverable class of case: `migration_v1.41.2.sql` finds every reversed `DEPOSIT_CONTRIBUTION_IN` transaction whose linked entry is still unmarked, decrements that member's balance by the entry's exact amount, and marks the entry reversed — clamped to 0 with a notice if the member's balance is already lower (e.g. from an exit refund since). Verified against `@electric-sql/pglite`, including the clamp case and re-running twice to confirm idempotency. Migration `migration_v1.41.2.sql`. |
| 1.41.1 | **Notification link backfill + historical Side Fund/Savings reversal reconciliation (Sections 10.6, 11.6, 18.3).** Two follow-ups reported right after v1.41.0 shipped. (1) A notification's `link` is baked into the row at creation time and never recomputed — so v1.41.0's link fix only affected *new* notifications; every notification already in the table kept bouncing to the Dashboard exactly as before (confirmed live with a screenshot). `migration_v1.41.1.sql` runs a one-time `UPDATE` over existing rows using the same corrected destinations, plus fixes two `transfersController.js` links (`TRANSFER_APPROVAL_PROGRESS`/`TRANSFER_REJECTED`) that v1.41.0's audit missed entirely. (2) v1.41.0's reversal fix only runs going forward — a Side Fund or Savings transaction reversed in the past, before that fix existed, was never corrected. The migration backfills what's safely recoverable: Side Fund's envelope balance (`side_fund_config.current_balance`, always exact — one aggregate number) and, for Savings, every deposit/handout linked via `approveSavingsDeposit`/`createFixedTermSavings`/handouts (fully recoverable — they already carried `transaction_id`). Two things are honestly reported rather than auto-corrected, since the data to do so precisely/safely doesn't exist: Side Fund's per-due detail for old cascading payments (`side_fund_dues.transaction_id` was last-write-wins, and `side_fund_member_credit` has no per-transaction history at all), and Savings deposits from the pre-fix `creditSavingsContribution` direct-credit path (wrote no per-entry record, no way to identify the member at all). Both print a `RAISE NOTICE` per affected case for manual review rather than guessing. Verified against a real embedded Postgres (`@electric-sql/pglite`, not just pg-mem) with synthetic pre-v1.41.0 scenarios covering every recoverable/unrecoverable case, run twice to confirm idempotency (no double-correction). Migration `migration_v1.41.1.sql`. |
| 1.41.0 | **Side Fund & Savings reversal rollback, plus notification link/layout fixes (Sections 10.6, 11.6, 18.3/18.6).** Continues v1.40.1's Deposits fix: Side Fund and Savings contributions/handouts had the identical gap — reversing their transaction correctly adjusted the ledger and account balance but left `side_fund_dues`/`side_fund_member_credit` and `savings_balances`/`member_savings`/`savings_handouts` completely untouched. Side Fund was the harder of the two (a payment can cascade across several dues and bank leftover as credit, and `side_fund_dues.transaction_id` is last-write-wins) — solved with a new append-only `side_fund_payment_applications` table, written by `applySideFundPayment` itself, giving `reverseTransaction` an exact record of what one transaction did regardless of how many dues/members it spanned (also correctly covers Bulk Pay Dues). Savings needed only new columns: `member_savings`/`savings_handouts` already carry `transaction_id` and a real `status` column, so both gained a `'REVERSED'` status value (mirroring the existing `shareholder_contributions` convention) plus `reversed_at`/`reversed_by`; `creditSavingsContribution` (the Record Contribution "Savings Portion" slice) now also writes a `member_savings` row so it can be identified at all — previously it updated only the aggregate balance. The two rarer "exit payout" legs crediting Savings (Side Fund exit, Deposit exit refund) are explicitly left out of scope — no user-identifying record to reverse against. Separately, requested together: notification `link`s that pointed at routes that don't exist were silently bouncing to the Dashboard via the router's catch-all — repointed to the nearest correct existing page (grants/events/certificates/requisitions/transactions/transfers); and the bell dropdown gained an overall height cap (`min(640px, calc(100vh - 88px))`) with its own scroll, since only its inner lists were capped before, leaving the footer unreachable on some phone screens. New table `side_fund_payment_applications`; widened `member_savings.status`/`savings_handouts.status` CHECKs; new `reversed_at`/`reversed_by` columns on both. Migration `migration_v1.41.0.sql`. |
| 1.40.1 | **Fixed: reversing a deposit's transaction left its deposit balance unchanged (Section 37.3/37.6).** Reported directly: "the deposit amount remains unchanged even when a reversal is initiated." `reverseTransaction` (`POST /transactions/:id/reverse`) is a generic, ledger-only reversal that previously only special-cased `shareholder_contributions` (via `transactions.contribution_id`) — deposits have no such linking column, only an `inflow_type`, so a deposit's `deposit_balances`/`deposit_entries` were silently left untouched by any reversal. Now matches on `inflow_type = 'DEPOSIT_CONTRIBUTION_IN'`, looks up the linked `deposit_entries` row via `transaction_id`, decrements `deposit_balances.balance` by that entry's `normalized_amount`, and marks the entry reversed (new `is_reversed`/`reversed_at`/`reversed_by` columns — the row is kept, not deleted, so history stays complete; shown struck-through with a "Reversed" badge on `DepositsPage.jsx`). Rejects the reversal with a clear error if the member's current balance is already lower than the entry being reversed (e.g. some of it was already paid out via an exit refund), rather than violating the `non_negative_deposit_balance` CHECK. The identical gap existed for Side Fund and Savings contributions (same "`inflow_type`-only, no linking column" shape) — fixed the same way in v1.41.0 above. Migration `migration_v1.40.1.sql`. |
| 1.40.0 | **Bonds & Investments overhaul (new Section 39).** Six related fixes/additions requested together: bond settlement value (bought at a discount/premium to face value — new `investments.settlement_value`, shown as a % of face value on the bond page; coupon math always stays on face value); a "Record Actual Payment" option on any due bond coupon (`payBondCoupon` now accepts `actual_gross_amount`, auto-recalculating tax from the bond's own withholding rate — only that one coupon is affected, `bond_coupons.actual_gross_amount/actual_tax_amount/actual_net_amount` store it alongside the original forecast) plus server-side date gating so neither "Mark Paid" nor "Record Actual Payment" can approve a coupon before its due date; a new `PATCH /:id/coupon-schedule` lets a bond's first coupon date be set/corrected after the fact (regenerating the whole schedule), usable until the first coupon is actually paid; fixed a bug where operational EXPENSE/TAX entries (`recordInvestmentTransaction`, and `payBondCoupon`'s own tax leg) left the funding account but were never added to `actual_expenditure` ("Spent") — now they are, merged into the same single Spent total as capital funding; a new `supplementary_budget` running total auto-detects and logs any spend that pushes `actual_expenditure` past `planned_budget`, with no separate manual step; a derived `performance_status` (PROFITABLE/LOSING/BREAK_EVEN/NOT_YET_FUNDED) shown next to ROI%; and a full mid-term termination workflow — request (`INVESTMENT_MANAGE`) → the investment's own responsible person confirms all records are up to date (returns/expenses can still be recorded during this review window) → a Treasurer/Director (`INVESTMENT_APPROVE`) gives final sign-off, generating a closing report stating the profit or loss and by how much, with any still-pending coupons marked MISSED. Two new statuses (`PENDING_TERMINATION`, `TERMINATED`), reachable only via the dedicated `/terminate/*` endpoints, never the generic status-update route. No new permissions. Migration `migration_v1.40.0.sql`. |
| 1.39.0 | **Payment Confirmations (new Section 38, Service Fees updated).** Requested directly: "the treasury do a similar thing as fines under acknowledges where an entry to anyone in the system can be made/posted to them in order to acknowledge the money they received defining how they received with means including Cash, Bank transfer, mobile money (MTN/AIRTEL/OTHER) where the former (bank transfer and mobile money) require entry of the transaction ID filled in. Then as soon as the recipient confirms it, it is recorded as a transaction in its respect." The mirror image of Payment Acknowledgements (Section 35), which always creates its row the instant a payment has already posted — this is the opposite order: Treasury records an entry FIRST (`payment_confirmations`, PENDING_CONFIRMATION), stating who was paid, how much, and how (Cash / Bank Transfer / Mobile Money — a transaction ID is required for the latter two, enforced by CHECK constraints as well as app-level validation); the real transaction is only posted (`paymentConfirmationsController.confirmPayment`) once the recipient reviews and confirms it. If they dispute it instead (reason required), no transaction is ever created — Treasury cancels the entry (`POST /:id/cancel`) and reissues a corrected one; there's no "reopen" step since nothing was posted to undo. Two source types: `GENERAL_PAYMENT` (Treasury pays any active user, ad hoc, via a new `POST /payment-confirmations` route) and `SERVICE_FEE_PAYMENT` (replaces Service Fees' old instant-post `recordPayment` flow entirely — recipient/account/category are derived from the agreement; confirming also inserts the `service_fee_payments` history row, deferred from creation time to confirm time). New table `payment_confirmations`; widened `transactions.inflow_type` (`GENERAL_PAYMENT_OUT` — `SERVICE_FEE_OUT` reused for confirmed service fee entries). No new permissions — reuses `PAYMENT_ACK_VIEW`/`PAYMENT_ACK_MANAGE` since it lives inside the existing Payment Acknowledgements page as a new "Payment Confirmations" tab (Confirm/Dispute for recipients, a "New Payment" modal and Cancel action for Treasury). `ServiceFeesPage.jsx`'s Record Payment modal gained the same Cash/Bank Transfer/Mobile Money fields. Migration `migration_v1.39.0.sql`. |
| 1.38.1 | **Fixed: Member Deposit Tracking shipped without an activation switch or a fixed account, unlike the Side Fund (Section 37).** Flagged directly: "like the side fund, the deposit functionality should be a feature that is turned on or off if the company has this arrangement or not and attached to a specific account. this hasn't been made that way." v1.38.0 let the Treasurer/member pick any account per deposit entry (with the contribution-slice deposit coupled to whatever account the capital-contribution slice itself targeted) and had no on/off switch — deposits "worked" the moment `deposit_config.currency_id` was set. `deposit_config` gained `is_active` (BOOLEAN, off by default) and `parent_account_id`, the same two columns `side_fund_config` already carries, with `currency_id` now derived automatically from that account rather than chosen independently. `creditDepositContribution` (`transactionsController.js`) no longer accepts an `accountId` parameter — it resolves and validates `deposit_config.parent_account_id`/`is_active` internally, throwing a clear "Deposits are not active for this company yet..." error if unconfigured; `recordContribution`'s account-coupling logic between the deposit slice and the capital contribution slice was removed entirely, since the deposit slice now resolves its own account independently, exactly like the side-fund slice already does. `depositService.processExitRefund` no longer accepts a `sourceAccountId` from the caller — the debit leg auto-uses `deposit_config.parent_account_id`, mirroring `sideFundController.removeMember`'s own auto-use of `side_fund_config.parent_account_id`. `updateDepositConfig` now mirrors `updateSideFundConfig`'s validation exactly (activating without a chosen parent account throws). The ONE respect Deposits still differs from the Side Fund, unchanged from v1.38.0 and reconfirmed as intentional: the parent account is not a separate envelope — no `current_balance` counter — every deposit stays a completely ordinary, spendable transaction into that one account. `DepositsPage.jsx`'s Settings modal gained an is-active toggle and parent-account select (mirroring `SideFundPage.jsx`'s own Settings modal); the Record Deposit and Exit Refund modals both dropped their now-unnecessary account-choice fields; the page shows a "Deposits aren't active yet" banner until configured, mirroring the Side Fund's own inactive-state banner. `TransactionsPage.jsx`'s "This contribution includes a deposit portion" checkbox is now gated behind a fetched `depositActive` flag, the same way the side fund checkbox is already gated behind `sideFundActive`. `migration_v1.38.1.sql` (`ADD COLUMN IF NOT EXISTS`, idempotent). |
| 1.38.0 | **Member Deposit Tracking (new Section, Transactions and Portfolio extended).** Deposits are a per-member running total, NOT a separate envelope like the Side Fund — money posted for a deposit is a completely normal transaction into whichever real account it was recorded against, fully spendable/commingled from that account, and does not contribute to shareholding. Funded two ways sharing one crediting core (`transactionsController.creditDepositContribution`): a third, independent optional `deposit_amount` slice on `POST /transactions/contributions` (alongside the existing Side Fund/Savings slices, always landing in the SAME account the capital contribution slice itself targets — resolved once, up front, whenever a deposit slice is present) and a standalone entry (`POST /deposits`, Treasurer/Admin, auto-provisions a "Member Deposits" category on first use). Each credit is normalized into a single company-wide target currency (`deposit_config`, admin-updatable target amount) via `sharePricingService.getExchangeRateOn`, so `GET /deposits`/`GET /deposits/me` can flag a member "below target" regardless of which currency any individual deposit was actually posted in; a member is expected to keep a nonzero balance unless explicitly excused (`deposit_excusals` — a monitoring/reporting flag, not a hard block on any transaction, since the schema itself never prevents a balance reaching zero). On exit, `depositService.computeExitRefund`/`processExitRefund` (mirroring `sideFundController`'s own exit-payout shape almost exactly — a two-leg DEBIT-source/CREDIT-Savings posting followed by a Payment Acknowledgement, `source_type DEPOSIT_REFUND`) refund the balance with a deduction: a fixed 5% for `MUTUAL_AGREEMENT`, or an admin-entered percentage (must be >= 50%) for `FORCED` — unlike Side Fund, there's no single canonical account to auto-debit, so the Treasurer chooses one explicitly at refund time. New tables `deposit_config`, `deposit_balances`, `deposit_excusals`, `deposit_entries`, `deposit_exit_events`; widened `transactions.inflow_type` (`DEPOSIT_CONTRIBUTION_IN`, `DEPOSIT_REFUND_OUT`) and `payment_acknowledgements.source_type` (`DEPOSIT_REFUND`); new permissions `DEPOSIT_VIEW`/`DEPOSIT_MANAGE`. New `DepositsPage.jsx` (`/deposits`, self-service "My Deposit" + Treasury "All Members"), a third checkbox-gated portion on `ContributionModal`, and a Deposit Balance section on the Member Portfolio (`portfolioService.getDepositSection`). Migration `migration_v1.38.0.sql`. |
| 1.37.0 | **Fines & Penalties (new Section, Requisitions extended).** Treasury can now assign a fine/penalty to a shareholder — special income to the company, tracked in the member's outstanding balances and Portfolio. `POST /fines` (`FINE_MANAGE`) takes a `reason` — `CONTRIBUTION_FAILURE`, `MEETING_VIOLATION`, or `GENERAL` — plus a free-text description; for `CONTRIBUTION_FAILURE`, the Treasurer enters the deadline of default, the amount defaulted on, and a fine percentage, and the amount is computed server-side (`defaulted_amount × fine_percentage / 100`) rather than trusted from the client — e.g. failing to pay 150 by 15.08.2026 at 8% posts a 12 fine. A fine can be paid into any account, but only in the same currency it was posted in — `finesService.clearFine` hard-rejects a currency mismatch rather than attempting any conversion, the same "never guess at money" rule used everywhere else in this system. New "Fines" page (self-service "My Fines" open to any member, no permission needed; "All Fines" oversight behind `FINE_VIEW`) lets a member either request the Treasurer acknowledge a payment they've already made externally — a new `FINE_PAYMENT` requisition type, referencing the specific fine via a new `requisitions.fine_id` column (unlike Savings/Side Fund's single-envelope acknowledgement types, a member can hold several simultaneous outstanding fines, possibly in different currencies, so the requisition must name exactly which one it settles) — or the Treasurer can clear it directly (`PATCH /fines/:id/clear`, only a receiving account/paid date/description needed; a system-managed "Fines & Penalties" category is auto-provisioned on first use via new `categoryService.getOrCreateCategory`, so the Treasurer is never prompted to pick one). Both paths share one crediting core, `finesService.clearFine` — called directly and from a new `FINE_PAYMENT` branch in `requisitionsController.approveRequisition` (the same "one core, two entry points" pattern as Side Fund/Savings contribution crediting). New `fines` table (with a CHECK enforcing the three `CONTRIBUTION_FAILURE` fields are all present together or all absent); widened `requisitions.requisition_type` (`FINE_PAYMENT`) and `transactions.inflow_type` (`FINE_PAYMENT_IN`); new permissions `FINE_VIEW`/`FINE_MANAGE`. Portfolio gained a Fines section (`portfolioService.getFinesSection`) with a per-currency outstanding total. Migration `migration_v1.37.0.sql`. |
| 1.36.0 | **Financial vs. performative role visibility, system-wide (Section 3).** Only Treasurer, Assistant Treasurer, Shareholder, Director, and Admin (`FINANCIAL_ROLES`) see company financial information by default now — every other role (Secretary, Assistant Secretary, Coordinator, Administrative Officer) is "performative": full access to what it's actually for, no account balances/share price/exchange rates/investment figures unless an Admin separately grants the permission. New `requireFinancialAccess(permission)` allow-list middleware (`middleware/auth.js`) applied to 8 endpoints that previously checked only `authenticate` — `GET /accounts/summary`, `savings/settings`, `side-fund/settings`, `shares/price(/history)`, `exchange-rates/current(/history)`, `investments/performance-summary`, `mmf/performance-summary` — each written under a pre-dating "any real member" assumption. Mirrored client-side via `AuthContext.js`'s `hasFinancialAccess(permission)`, used to gate `DashboardPage.jsx` (account balance StatCards, Best/Worst Investment card — previously rendered with no permission check at all), `TopBar.jsx` (the persistent balance strip, previously only excluded Auditor/Administrative Officer), and `Sidebar.jsx` (Savings/Side Fund links, previously `!isAdminOfficer`-only with no permission check). Also fixed in the same release: `TransfersPage.jsx`'s Approve button now uses a `canApprove(row)` check mirroring the backend's exact per-type role rule (Treasurer for Primary→Secondary, Director for Secondary→Primary) instead of the broader `FINANCE_TRANSFER_APPROVE` permission alone — closing CMS_BIBLE's own previously-open "buttons shouldn't appear if a role doesn't hold it" item. Code-only, no schema migration. |
| 1.35.3 | **Fixed: Assign Role failed with "A record with this value already exists" for any member being given back a role they'd previously held and lost (Section 23.6).** `user_roles` carried a table-wide `UNIQUE(user_id, role_id)` from the original schema — `assignRole`'s own duplicate check only looks at currently-active rows (`revoked_at IS NULL`), so it correctly allowed the attempt, but the INSERT then hit the constraint anyway because a revoked row for that same pairing still counted against it. A pre-existing latent bug, not something the permanent-deletion feature (v1.35.0) introduced — it surfaced now because deleting an account that held a role naturally leads to handing that same role to someone who'd held it before. Fixed by replacing the table-wide constraint with a partial unique index, `user_roles_active_role_unique ON (user_id, role_id) WHERE revoked_at IS NULL` — a member still can't hold the same role twice at once, but the same role can now be assigned, revoked, and re-assigned to the same person any number of times. `migration_v1.35.3.sql`, idempotent. |
| 1.35.2 | **Fixed: backend silently restarting roughly every 16 minutes on both companies' deployments (Section 7.1).** The global rate limiter (`server.js`, 200 requests/IP/15min, meant to catch brute-force/DDoS traffic) applied to every endpoint including `/health` — Render's own liveness-probe traffic to that one endpoint eventually crossed the limit on its own and started getting `429 Too Many Requests`. Render reads a failing health check as "this instance is unresponsive" and restarts it, which is a completely clean, error-free restart from the app's own perspective (nothing was actually broken), explaining why nothing showed up in the app's own logs and why an out-of-memory kill was the first suspect (ruled out via Render's Metrics → Memory graph — flat around 20%, no sawtooth pattern). Fixed with `skip: (req) => req.path === '/health'` on the global limiter. This was the root cause behind v1.35.1's forced-logout symptom too — any page load or token refresh landing in the ~1-2 second restart window was being treated as a real auth failure. Code-only, no schema migration. Needs deploying to both `cms-backend` and `cms-b-backend`. |
| 1.35.1 | **Fixed: a transient network error forced a full logout instead of just failing one request (Section 22.6).** `AuthContext.js`'s `initAuth()` and `axios.js`'s response interceptor both conflated "the server explicitly rejected this token" (401/403) with "we couldn't reach the server at all" (a network error, `err.response` undefined) — treating a plain connectivity blip identically to a genuinely expired session, wiping the cached user and hard-redirecting to `/login`. Both now only clear the session on an explicit 401/403; a network error just fails that one request and leaves the session alone. Reported as "continuous network error that always takes me to the login page," correlating with a backend restarting roughly every 16 minutes with a clean boot and no error in its own logs each time — any page load or token refresh landing in that restart window was getting logged out over nothing. Root cause of the restarts themselves is still open (Section 7.2) — most likely an OOM kill or failing health check on the `starter` plan; check Render's Metrics → Memory graph to confirm. Code-only, no schema migration. |
| 1.35.0 | **Permanent user deletion, for duplicate/unused registrations only (Section 23.8).** New `userDeletionService.js` — rather than hand-maintain a list of "every table that might reference a user" (roughly 90 foreign-key columns across roughly 60 tables in this schema), `getUserFootprint()` reads PostgreSQL's own `pg_constraint`/`pg_attribute` catalog for every foreign key pointing at `users(id)` and checks each one for rows belonging to the target user — this stays correct automatically as the schema grows, with no list to maintain. A small allowlist (`notifications`, `role_requests`, `user_roles`, `member_consents` — all deleted; `audit_log` — kept, `user_id` set to `NULL` rather than deleted, honouring its append-only design) is cleaned up automatically; every other table with even one matching row blocks the deletion entirely, with the specific table/column surfaced to the admin. New `GET /users/:id/deletion-check` (read-only preview) and `DELETE /users/:id` (re-validates inside its own transaction before doing anything irreversible) — same `USER_MANAGE` gate as the existing Deactivate action, which remains the correct, reversible tool for every situation this isn't. Frontend: a "Delete Permanently" action on the Users page opens a confirmation modal that runs the check immediately — the delete button only appears once the account comes back clean, otherwise the blocking tables are listed and Deactivate is suggested instead. Code-only, no schema migration (no new tables, no CHECK constraint changes — the introspection query works against the schema exactly as it already is). |
| 1.34.0 | **Member Portfolio page (Section 23.7) — one page with everything about a member's standing in the company.** New `GET /users/:id/portfolio` (same `isSelfOrHasPermission('USER_VIEW_ALL')` gate as the existing profile lookup) backed by `portfolioService.buildMemberPortfolio()`, which independently queries roles held, shareholding + contribution history, savings, dividends received, side fund standing (with overdue called out), every payment ever paid out to the member (unified through `payment_acknowledgements`, Section 35), and every transaction they've been involved in as beneficiary/recorder/approver. Reached from the Users page ("View Portfolio" on any row, for anyone with `USER_VIEW_ALL`) or by tapping your own photo/name/roles at the top of the Sidebar — previously static, not clickable — which now opens the same page scoped to yourself (`/portfolio`, self-view). Two ways to get a printable copy: an instant browser print (nothing saved), or "Generate Portfolio Summary", which saves a real audit-logged Document (`document_type` `FINANCIAL_REPORT_INDIVIDUAL` — already allowed by the schema's CHECK constraints, just never finished) through the same generic document-generation endpoint Receipts/Resolutions use, downloadable and re-printable later from the Documents module. `migration_v1.34.0.sql` seeds the one `document_templates` row this needs — no CHECK constraint changes required. Deliberately separate from the existing monthly individual report email pipeline (Section 17) — this is an always-current, all-time, every-domain snapshot, not a period-scoped report. |
| 1.33.0 | **Shareholding rewritten from raw contributed money to real priced units (Section 14, in full).** Previously `shares_held` was simply `SUM(shareholder_contributions.amount)` — no division by share price at all, so a member's "shares" were really just a running total of money paid in, with the share price acting only as a display-side valuation multiplier that never wrote back to the registry. Now every approved contribution buys a specific number of units, computed once and locked in permanently: `units = (amount × exchange rate on that contribution's date) ÷ share price on that same date` (the exchange-rate step is skipped, i.e. rate = 1, when the contribution's account is already in the share price's own currency — one code path covers both of the two formulas requested, not two separate ones). New `sharePricingService.js` provides the date-effective price/rate lookups (same range-query pattern as `primary_account_floor_limits`); new `computeShareUnitsPerUser()` (`transactionsController.js`) is the shared pure computation used by both the rewritten `recalculateShareholding()` (still auto-triggered on every contribution/reversal, same as before — only the math changed) and the new preview/commit pair below. `POST /transactions/contributions` gains an optional `account_id` (defaults to Primary) so a contribution's own currency is known; `creditShareholderContribution` now pre-checks FX coverage exists for that currency/date combination and rejects immediately if not, rather than failing silently later. New Admin-only `GET /shares/recalculate-preview` / `POST /shares/recalculate` endpoint pair — a one-time, explicitly reviewed full recompute (old vs proposed `shares_held`/`percentage` per member, largest change first) for bringing pre-existing historical contributions onto the new unit-price model; ordinary day-to-day contributions never need it. Frontend: `ContributionModal` (`TransactionsPage.jsx`) gained the Account selector; `AccountsPage.jsx` gained an Admin-only "Shareholding Recalculate" card opening the preview/commit modal. `migration_v1.33.0.sql` — idempotent, seeds each company's own founding share price and the founding EUR→UGX rate dated to its earliest approved contribution, so historical contributions predating anyone manually setting a price/rate have something valid to convert against; deliberately does not itself touch `shareholding_registry` (that's the Recalculate screen's job, after this migration runs). See Section 14.7 for the full before/after. |
| 1.32.6 | **Fixed: browser tab title and favicon always showed "React App" and the default Create React App logo, regardless of company (Section 4.34).** `public/index.html`/`manifest.json` were still the untouched CRA scaffolding defaults — no deployment had ever set these, so every company's build shared the exact same generic browser-chrome. Two-layer fix, matching the same build-time-baseline-plus-live-override pattern already used for in-app branding: `index.html`'s `<title>`/`<meta name="description">` now read `%REACT_APP_COMPANY_NAME%`, a Create React App-native placeholder substituted at BUILD time from the same env var `render.yaml`/`render.company-b.yaml` already set — correct from first paint, before any JS runs. `BrandingContext.js` then keeps `document.title` and a new `id="app-favicon"` `<link>`'s `href` in sync with the LIVE `company_name`/`logo_url` from Settings > Company afterwards, so renaming the company or uploading a new logo takes effect immediately without a rebuild. `manifest.json`'s name/short_name (used for "Add to Home Screen" on mobile) changed from "Create React App Sample" to a neutral "Company Management System" — not made build-time-dynamic, since CRA's `%VAR%` substitution only applies to `index.html`, not other files copied verbatim from `public/`. Code-only, no schema migration. |
| 1.32.5 | **Three UI/UX fixes: wide-screen space usage, unreachable Settings tabs on narrow screens, and idle auto-logout never firing on mobile (Sections 2.4, 4.29.6).** (1) Four pages (Profile, Settings, Generate Document, About & Manual) capped their entire content at `max-w-4xl`/`max-w-5xl` with no centering, leaving a large uncentered dead zone on wide monitors — widened to `max-w-6xl mx-auto` on all four. (2) The tab bar on Settings (and, found in the same sweep, Dividends/Documents/Loans/Requisitions/Users/Profile's own inner tabs) was a plain `flex` row with neither `flex-wrap` nor `overflow-x-auto` — on a narrow screen, tabs pushed past the visible width (Membership Agreement, Settings' last tab, was the one users actually got stuck unable to reach) with no way to scroll to them; all seven now scroll horizontally (`overflow-x-auto scrollbar-hidden`, matching the existing hidden-scrollbar convention from `DataTable.jsx`). (3) `useIdleLogout.js` (v1.28.2) only ever tracked a single scheduled `setTimeout`, reset unconditionally on every activity event with no record of *when* activity last happened — mobile browsers routinely suspend a backgrounded tab's timers while the phone is locked, and worse, the very touch that brings the app back to the foreground was itself treated as fresh "activity," blindly restarting a full new window with no check of how long the user had actually been away first. A session could therefore stay alive indefinitely through any number of lock/unlock cycles. Rewritten to track a real `lastActivity` timestamp and check *elapsed wall-clock time* (not "did a timer fire") in three places: every activity event, the timer's own tick, and — the actual mobile fix — the Page Visibility API's `visibilitychange` firing when a backgrounded tab becomes visible again, which now logs out immediately if the timeout had already passed while backgrounded rather than waiting for a fresh idle period to elapse in the foreground. Verified with a standalone state-machine simulation covering background-then-unlock, touch-only return, normal active use, genuine foreground idling, and a short gap that should NOT log out. Code-only, no schema migration. |
| 1.32.4 | **Fixed: pre-login pages (Login, Register, Forgot/Reset Password, Verify Email) always showed whichever logo/name was baked into the frontend build, not this deployment's actual configured branding (Section 4.34).** `GET /settings/company` required authentication, so a second company sharing this same codebase (e.g. Company B) had no way to fetch its own uploaded logo before signing in — those pages fell back to the single static `public/logo.png` file bundled identically into every deployment's build, which showed whichever company's logo happened to be sitting in that file in the repo at build time. Made `GET /settings/company` deliberately public (nothing it returns is sensitive — company identity/branding only); `BrandingContext.js` now fetches once on app mount regardless of login state instead of only once authenticated; all five pre-login pages switched from the hardcoded `/logo.png`/`REACT_APP_COMPANY_NAME` fallback to the same `branding.logo_url`/`branding.company_name` pattern already used by `ConsentPage`/`PendingApprovalPage`. Code-only, no schema migration. |
| 1.32.3 | **Fixed: uploaded images (logo, profile photos, signatures, stamps) never rendered across the frontend/backend split (Section 4.34).** Helmet's default `Cross-Origin-Resource-Policy: same-origin` header — set on every response since this app first added Helmet, well before the storage-service refactor — silently blocks a browser from loading a response as an `<img>` (or script/CSS/font) from a different origin. The frontend and backend are always separate origins on this two-service Render layout, so every `<img src="/uploads/...">` (Sidebar/TopBar logo, avatars, signatures, stamps) failed to render with no error surfaced anywhere — regular API `fetch()`/`axios` calls were unaffected since those are CORS-checked rather than CORP-checked, which is why the rest of the app worked normally throughout. Fixed by explicitly setting `Cross-Origin-Resource-Policy: cross-origin` on just the `/uploads/*splat` proxy route in `server.js` (these files are already access-control-free "public-style" assets by design — see `streamInline`'s own comment in `storageService.js`), leaving Helmet's default untouched for every other response. Code-only, no schema migration. |
