...
| POST | `/agreements/:id/pay` | Treasurer, Assistant Treasurer, Admin | Record a monthly payment (defaults to the standard monthly amount) — v1.39.0: creates a pending Payment Confirmation (Section 38) rather than posting immediately; also requires `payment_method` (Cash/Bank Transfer/Mobile Money) |
