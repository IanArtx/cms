...
#### 39. Bonds & Investments Overhaul (v1.40.0; schedule bug fix, face value repayment, frequency rescheduling, and settlement auto-funding v1.42.0)
...
`PATCH /investments/:id/coupon-schedule` (`updateCouponSchedule`, `INVESTMENT_MANAGE`) — sets or corrects `first_coupon_date` **and/or `coupon_frequency`** on an already-ACTIVE bond and regenerates the **entire** coupon schedule from whichever changed (or both) via the same `generateBondCouponSchedule()` used at creation time, so every later coupon date recalculates automatically. Either field can be omitted to keep its current value, but at least one must be supplied. Only usable while **no coupon has been paid yet** (`SELECT COUNT(*) WHERE status='PAID'` must be 0) — once real money has moved against coupon #1, the schedule is locked (both fields, same single guard); this is a narrower, purpose-built endpoint rather than loosening `editInvestment`'s existing PENDING-only guard, since that function's bond-schedule regeneration does an unconditional `DELETE FROM bond_coupons` that would be unsafe post-approval. Frontend: an "Edit Coupon Date / Frequency" link next to the coupon rate badge on `BondScheduleCard` (hidden once any coupon has been paid), opening a modal with both a date picker and a frequency dropdown.

**v1.42.0 — fixed a bug where the schedule could silently collapse to just one row.** Reported directly: "the date recalculation failed when i set the first coupon date and now only shows one date of the final coupon payment and amount." Root cause, in `generateBondCouponSchedule()` (`cms/src/utils/bondSchedule.js`): the periodic-coupon loop is `while (dueDate < maturity)`, anchored on `first_coupon_date` (or, if not supplied, `issueDate + one period`) — if that anchor date ends up on or after the maturity date, the loop simply never executes, and only the one forced final coupon (always pushed after the loop, landing exactly on the maturity date) survives. Nothing errored; the schedule just silently shrank to that single row, wiping out every other coupon that had existed before. Fixed by validating the anchor is strictly before the maturity date and throwing a clear error otherwise ("First coupon date must be before the maturity date...") — applies everywhere `generateBondCouponSchedule()` is called (`createInvestment`, `editInvestment`, `updateCouponSchedule`), so the same mistake can no longer be made silently through any of the three entry points. **Not retroactively fixed for already-affected bonds** — there's no way to safely guess what the correct historical first coupon date should have been for a bond whose schedule already collapsed; re-running "Edit Coupon Date / Frequency" on the affected investment, now that the validation is in place, is the way to correct it.
...
- The first-coupon-date schedule-collapse bug (39.6) is fixed going forward but not retroactively — any bond already affected needs its schedule manually re-entered via "Edit Coupon Date / Frequency" once the correct first coupon date is known.
