...
4. [Module Reference](#4-module-reference) — 30 modules, numbered sequentially (not nested) for simplicity:
   1. Accounts
   2. Transactions
   3. Transfers
   4. Exchange Rates
...
| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/upcoming` | any authenticated non-Auditor user | Next N days of events |
| GET | `/` | any authenticated non-Auditor user | Full paginated event list |
| POST | `/` | Secretary/Admin | Create an event |
| PATCH | `/:id` | Secretary/Admin | Edit/cancel |
...
- **Uploaded files**: not persistent across redeploys on the current setup (Section 4.26.5/4.26.8) — a [Render Persistent Disk](https://render.com/docs/disks) add-on or a move to external object storage (e.g. S3) is the fix once this matters operationally.
