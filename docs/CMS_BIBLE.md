...
#### 37. Member Deposit Tracking (v1.38.0; activation switch + parent account v1.38.1)
...
- **"Cannot be zero unless excused" is monitoring, not enforcement.** Nothing in the schema blocks a balance from reaching zero — the only way it decreases is a deliberate one-time exit refund. `deposit_excusals` and the `below_target`/`is_excused` flags surfaced on `GET /deposits`, `GET /deposits/me`, and the Member Portfolio are reporting signals for Treasury, not a hard gate on any transaction.
