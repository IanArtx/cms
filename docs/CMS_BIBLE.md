...
##### 23.5 Frontend
`cms-frontend/src/pages/users/UsersPage.jsx` — member directory table with role badges, active/inactive status, Admin-only "Edit"/"Assign Roles"/"Deactivate" actions, and (v1.34.0) a "View Portfolio" link on every row for anyone holding `USER_VIEW_ALL`; a personal "My Profile" page (`ProfilePage.jsx`) for self-service edits, accessible to every role including Auditor. The photo camera-icon button opens the OS file picker, then (v1.28.3) `PhotoCropModal` for framing before the cropped result is uploaded — `reloadProfile()` calls `AuthContext`'s `refreshUser()` afterward, so the TopBar/sidebar avatar update immediately without needing a page reload.
...
| `REACT_APP_VERSION` | `1.34.0` | `1.34.0` (kept in sync manually — a pure display string, nothing reads it programmatically) |
