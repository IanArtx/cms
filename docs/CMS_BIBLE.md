...
#### 39. Bonds & Investments Overhaul (v1.40.0)
...
##### 39.6 First coupon date rescheduling

New endpoint `PATCH /investments/:id/coupon-schedule` (`updateCouponSchedule`, `INVESTMENT_MANAGE`) — sets or corrects `first_coupon_date` on an already-ACTIVE bond and regenerates the **entire** coupon schedule from it via the same `generateBondCouponSchedule()` used at creation time, so every later coupon date recalculates automatically. Only usable while **no coupon has been paid yet** (`SELECT COUNT(*) WHERE status='PAID'` must be 0) — once real money has moved against coupon #1, the schedule is locked; this is a narrower, purpose-built endpoint rather than loosening `editInvestment`'s existing PENDING-only guard, since that function's bond-schedule regeneration does an unconditional `DELETE FROM bond_coupons` that would be unsafe post-approval. Frontend: an "Edit First Coupon Date" / "Set First Coupon Date" link next to the coupon rate badge on `BondScheduleCard`, hidden once any coupon has been paid.
