...
#### 37. Member Deposit Tracking (v1.38.0)

##### 37.1 Purpose

Tracks how much each shareholder has "as deposit" in the company — deliberately NOT a separate pool like the Side Fund. Requested directly: "this amount is held in the account it is parented by unlike the side fund it is not separate therefore its amount is spendable through the parent account. It is only there to track how much each member has as deposit in the company... does not contribute to ones shareholding and each shareholder's deposit amount cannot be zero unless otherwise excused." Funded two ways — sliced out of a Transactions contribution (alongside the existing Side Fund/Savings slices) or entered as a standalone inflow — and, on exit, refunded with a deduction that depends on how the member is leaving.

##### 37.2 Data model

**`deposit_config`**: singleton (id=1), target_amount (the single company-wide target every member's own balance is compared against), currency_id (the normalization currency), updated_by, updated_at. **`deposit_balances`**: user_id (PK), balance (running per-member total, already normalized into `deposit_config.currency_id`), updated_at. **`deposit_excusals`**: user_id (PK), excused_by, excused_at, reason — presence of a row is the whole signal; a member with a row is excused from the "cannot be zero" expectation. **`deposit_entries`**: id, user_id, source (`CONTRIBUTION_SLICE` / `STANDALONE`), account_id (whichever real account the money actually landed in), transaction_id, amount/currency_id (as originally posted), normalized_amount/exchange_rate_used (as converted into `deposit_config`'s currency at credit time), entry_date, recorded_by, created_at — the full audit trail behind the running `deposit_balances` total. **`deposit_exit_events`**: id, user_id, exit_type (`MUTUAL_AGREEMENT` / `FORCED`), deduction_percentage, gross_balance, deduction_amount, net_payout, source_account_id, transaction_id, payment_ack_id, processed_by, processed_at, notes — a CHECK constraint enforces `deduction_percentage = 5` for MUTUAL_AGREEMENT and `>= 50 AND <= 100` for FORCED, exactly matching the brief. Widened `transactions.inflow_type` (`DEPOSIT_CONTRIBUTION_IN`, `DEPOSIT_REFUND_OUT`) and `payment_acknowledgements.source_type` (`DEPOSIT_REFUND`). New permissions `DEPOSIT_VIEW`/`DEPOSIT_MANAGE`.

##### 37.3 Business rules / key logic

- **Not an envelope, deliberately.** Unlike the Side Fund's `current_balance` counter layered on top of a real account, a deposit has no parallel "how much of this account is earmarked" column — the posted transaction is a completely ordinary credit into whichever account it targeted, fully spendable/commingled from that moment on. `deposit_balances.balance` is purely a tracking number, reconstructable at any time from `deposit_entries`.
- **Two entry points, one shared crediting core.** `transactionsController.creditDepositContribution` is called from BOTH `recordContribution`'s optional `deposit_amount` slice AND `depositsController.createStandaloneDeposit` — the same "one core, two entry points" shape as `creditSideFundContribution`/`creditShareholderContribution`. It lives in `transactionsController.js` itself (not a separate service file) specifically so it can call `postTransaction` directly without a circular require — `depositService.js` holds only the non-crediting exit-refund logic, and imports `postTransaction` FROM `transactionsController.js` one-directionally.
- **The deposit slice always lands in the SAME account as the contribution slice.** `recordContribution` resolves the target account once, up front, whenever a `deposit_amount` slice is present, then passes that resolved account id explicitly into both `creditShareholderContribution` and `creditDepositContribution` — guaranteeing they can never disagree, unlike Side Fund/Savings, which always target their own single dedicated account regardless of what the capital contribution slice does.
- **Normalized at credit time, never re-normalized later.** Because a single company-wide target exists in one currency but individual deposits can be posted in any currency, `creditDepositContribution` converts each credit into `deposit_config.currency_id` via `sharePricingService.getExchangeRateOn` (date-effective, same "hard stop rather than guess" rule as everywhere else FX is involved) and stores both the original and normalized amounts on the `deposit_entries` row. Changing `deposit_config.currency_id` later does not retroactively re-convert history — only new activity compares against the new currency.
...
- **Exit refund — deduction depends on exit type.** `depositService.computeExitRefund`/`processExitRefund` mirror `sideFundController.computeExitPayout`/`removeMember`'s exact shape (a pure computation usable both for a read-only preview and the real processing, a two-leg posting — DEBIT a source account / CREDIT Savings — followed by a Payment Acknowledgement for two-party sign-off), with two differences: (1) there is no single canonical account to debit automatically, since deposits aren't siloed, so the Treasurer must choose a `source_account_id` explicitly at refund time; (2) the deduction is either a fixed 5% (`MUTUAL_AGREEMENT`) or an admin-entered percentage that must be >= 50% (`FORCED`), rather than Side Fund's expense-share formula. `deposit_balances.balance` is always zeroed on exit, even when the net payout works out to zero (a 100% FORCED deduction is allowed).
...
| Method | Path | Access | Description |
|---|---|---|---|
| GET | `/settings` | Any authenticated, role-assigned, consented member | View the company-wide target |
| PATCH | `/settings` | `DEPOSIT_MANAGE` | Update the target amount/currency |
...
| POST | `/` | `DEPOSIT_MANAGE` | Standalone deposit entry |
...
| PATCH | `/:userId/exit-refund` | `DEPOSIT_MANAGE` | Process the exit refund |

A deposit can also be funded via `POST /transactions/contributions` with an optional `deposit_amount` field — see Section 2.5/11.3's contribution-slice pattern.

##### 37.5 Frontend

- **`DepositsPage.jsx`** (`/deposits`) — "My Deposit" (everyone, always visible) shows balance, target, standing (on target / below target / excused), and full entry history; "All Members" (`DEPOSIT_VIEW`) lists every shareholder with a below-target/excused badge and, for `DEPOSIT_MANAGE` holders, Excuse/Clear Excusal and Exit Refund actions. The Exit Refund modal fetches the same `computeExitRefund` breakdown the backend uses before confirming, mirroring Side Fund's exit-payout preview modal exactly.
- **`TransactionsPage.jsx`'s `ContributionModal`** gained a third, independent "This contribution includes a deposit portion" checkbox, following the identical checkbox-gated pattern as the existing Side Fund/Savings portions (Section 11.3).
