...
- **Side Fund and Savings have the identical reversal gap** that Deposits had before v1.40.1 — `reverseTransaction` still has no branch for `SIDE_FUND_CONTRIBUTION_IN`/`SAVINGS_DEPOSIT_IN`, so reversing one of those transactions today still leaves `side_fund_dues`/`savings_balances` unchanged. Same root cause, not yet fixed — worth doing the same way if reported.
