...
| `REACT_APP_VERSION` | `1.35.1` | `1.35.1` (kept in sync manually — a pure display string, nothing reads it programmatically) |
...
- **Backend restarting roughly every 16 minutes with a clean boot and no error in its own logs each time** (Section 22.6, v1.35.1) — the frontend symptom (forced logout on a transient network error) was fixed this session, but *why* the backend itself keeps restarting was not root-caused, since diagnosing it needs Render's Metrics → Memory graph for the service, which isn't visible from this environment. Prime suspect given the `starter` plan's memory ceiling: an out-of-memory kill (the restart cadence would show as a sawtooth climbing to the plan's limit right before each restart) — check that graph first before investigating further in code.
